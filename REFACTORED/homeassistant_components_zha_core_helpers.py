...
from  import uniform
import re
from  import TYPE_CHECKING, Any, TypeVar
...
from  import HomeAssistant, State, callback
from  import config_validation as cv, device_registry as dr
from  import ConfigType
from  import CLUSTER_TYPE_IN, CLUSTER_TYPE_OUT, CUSTOM_CONFIGURATION, DATA_ZHA
from  import BINDABLE_CLUSTERS
if TYPE_CHECKING:
...
class BindingPair:
    ...
    def destination_address(self) -> zdo_types.MultiAddress:
        ...
        return 6lowpan.sixlowpan.IP6FieldLenField.addfield.MultiAddress(addrmode=3, ieee=self.target_ieee, endpoint=self.target_ep_id)

async def safe_read(cluster, attributes, allow_cache=True, only_cache=False, manufacturer=None):
    ...
    """
    try:
        (result, _) = await cluster.read_attributes(attributes, allow_cache=allow_cache, only_cache=only_cache, manufacturer=manufacturer)
        return result
    except Exception:
        return {}

async def get_matched_clusters(source_zha_device: ZHADevice, target_zha_device: ZHADevice) -> list[BindingPair]:
    ...
    for endpoint_id in source_clusters:
        for cluster_id in source_clusters[endpoint_id][CLUSTER_TYPE_OUT]:
            if cluster_id not in BINDABLE_CLUSTERS:
                continue
            if target_zha_device.nwk == 0:
                cluster_pair = BindingPair(source_cluster=source_clusters[endpoint_id][CLUSTER_TYPE_OUT][cluster_id], target_ieee=target_zha_device.ieee, target_ep_id=target_zha_device.device.application.get_endpoint_id(cluster_id, is_server_cluster=True))
            ...
            for t_endpoint_id in target_clusters:
                if cluster_id in target_clusters[t_endpoint_id][CLUSTER_TYPE_IN]:
                    cluster_pair = BindingPair(source_cluster=source_clusters[endpoint_id][CLUSTER_TYPE_OUT][cluster_id], target_ieee=target_zha_device.ieee, target_ep_id=t_endpoint_id)
                    clusters_to_bind.append(cluster_pair)
...
def cluster_command_schema_to_vol_schema(schema: CommandSchema) -> vol.Schema:
    ...
    return vol.Schema({vol.Optional(field.name) if field.optional else vol.Required(field.name): schema_type_to_vol(field.type) for field in schema.fields})
...
def schema_type_to_vol(field_type: Any) -> Any:
    ...
    if issubclass(field_type, zigpy.types.FixedIntType) or issubclass(field_type, enum.Flag) or issubclass(field_type, enum.Enum):
        return vol.All(vol.Coerce(int), vol.Range(field_type.min_value, field_type.max_value))
    return str

def convert_to_zcl_values(fields: dict[str, Any], schema: CommandSchema) -> dict[str, Any]:
    ...
    for field in schema.fields:
        ...
        if issubclass(field.type, enum.Flag) and isinstance(value, list):
            ...
            value = field.type[value.replace(' ', '_')] if isinstance(value, str) else field.type(value)
        else:
            value = field.type(value)
        _LOGGER.debug('Converted ZCL schema field(%s) value from: %s to: %s', field.name, fields[field.name], value)
...
def async_is_bindable_target(source_zha_device, target_zha_device):
    ...
    for endpoint_id in source_clusters:
        for t_endpoint_id in target_clusters:
            matches = set(source_clusters[endpoint_id][CLUSTER_TYPE_OUT].keys()).intersection(target_clusters[t_endpoint_id][CLUSTER_TYPE_IN].keys())
            if any((bindable in BINDABLE_CLUSTERS for bindable in matches)):
                return True
    return False

@callback
def async_get_zha_config_value(config_entry: ConfigEntry, section: str, config_key: str, default: _T) -> _T:
    """Get the value for the specified configuration from the ZHA config entry."""
    return config_entry.options.get(CUSTOM_CONFIGURATION, {}).get(section, {}).get(config_key, default)
...
def async_cluster_exists(hass, cluster_id, skip_coordinator=True):
    ...
    for zha_device in zha_devices:
        ...
        for clusters in clusters_by_endpoint.values():
            if cluster_id in clusters[CLUSTER_TYPE_IN] or cluster_id in clusters[CLUSTER_TYPE_OUT]:
                return True
    return False
...
def async_get_zha_device(hass: HomeAssistant, device_id: str) -> ZHADevice:
    ...
    try:
        ...
        _LOGGER.error('Unable to determine device IEEE for device with device id `%s`', device_id)
        raise KeyError(f'Unable to determine device IEEE for device with device id `{device_id}`.') from ex
...
def reduce_attribute(states: list[State], key: str, default: Any | None=None, reduce: Callable[..., Any]=mean_int) -> Any:
            ...
            for delay in itertools.chain(delays, [None]):
                try:
                    ...
                    if delay:
                        ...
                        cluster_handler.debug('%s: retryable request #%d failed: %s. Retrying in %ss', func.__name__, try_count, ex, round(delay, 1))
                        try_count += 1
                        await asyncio.sleep(delay)
                    else:
                        cluster_handler.warning('%s: all attempts have failed: %s', func.__name__, errors)
                        if raise_:
                            raise
        return wrapper
    return decorator
...
class ZHAData:
    ...
    platforms: collections.defaultdict[Platform, list] = dataclasses.field(default_factory=lambda : collections.defaultdict(list))
    gateway: ZHAGateway | None = dataclasses.field(default=None)
    device_trigger_cache: dict[str, tuple[str, dict]] = dataclasses.field(default_factory=dict)
