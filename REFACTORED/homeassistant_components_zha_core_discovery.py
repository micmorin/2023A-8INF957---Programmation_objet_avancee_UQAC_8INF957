...
import logging
from  import TYPE_CHECKING, cast
from  import CONF_TYPE, Platform
...
from  import async_entries_for_device
from  import ConfigType
from  import alarm_control_panel, binary_sensor, button, climate, cover, device_tracker, fan, light, lock, number, select, sensor, siren, switch
from  import const as zha_const, registries as zha_regs
from  import ClusterHandler, closures, general, homeautomation, hvac, lighting, lightlink, manufacturerspecific, measurement, protocol, security, smartenergy
from  import get_zha_data, get_zha_gateway
if TYPE_CHECKING:
    from  import ZhaEntity
    from  import ZHADevice
    from  import Endpoint
    from  import ZHAGroup
_LOGGER = logging.getLogger(__name__)

@callback
async def async_add_entities(_async_add_entities: AddEntitiesCallback, entities: list[tuple[type[ZhaEntity], tuple[str, ZHADevice, list[ClusterHandler]]]]) -> None:
    """Add entities helper."""
    if not entities:
        return
...
class ProbeEndpoint:
    ...
    def discover_entities(self, endpoint: Endpoint) -> None:
        ...
        _LOGGER.debug('Discovering entities for endpoint: %s-%s', str(endpoint.device.ieee), endpoint.id)
    ...
    def discover_by_device_type(self, endpoint: Endpoint) -> None:
        ...
        if platform and platform in zha_const.PLATFORMS:
            ...
            (platform_entity_class, claimed) = zha_regs.ZHA_ENTITIES.get_entity(platform, endpoint.device.manufacturer, endpoint.device.model, cluster_handlers, endpoint.device.quirk_id)
            if platform_entity_class is None:
                return
            endpoint.claim_cluster_handlers(claimed)
            endpoint.async_new_entity(platform, platform_entity_class, unique_id, claimed)

    @callback
    def discover_by_cluster_id(self, endpoint: Endpoint) -> None:
        """Process an endpoint on a zigpy device."""
        items = zha_regs.SINGLE_INPUT_CLUSTER_DEVICE_CLASS.items()
        single_input_clusters = {cluster_class: match for (cluster_class, match) in items if not isinstance(cluster_class, int)}
        remaining_cluster_handlers = endpoint.unclaimed_cluster_handlers()
        for cluster_handler in remaining_cluster_handlers:
            if cluster_handler.cluster.cluster_id in zha_regs.CLUSTER_HANDLER_ONLY_CLUSTERS:
                endpoint.claim_cluster_handlers([cluster_handler])
                continue
            platform = zha_regs.SINGLE_INPUT_CLUSTER_DEVICE_CLASS.get(cluster_handler.cluster.cluster_id)
            if platform is None:
    ...
    @staticmethod
    def probe_single_cluster(platform: Platform | None, cluster_handler: ClusterHandler, endpoint: Endpoint) -> None:
        ...
        (entity_class, claimed) = zha_regs.ZHA_ENTITIES.get_entity(platform, endpoint.device.manufacturer, endpoint.device.model, cluster_handler_list, endpoint.device.quirk_id)
    ...
    def handle_on_off_output_cluster_exception(self, endpoint: Endpoint) -> None:
        ...
        for (cluster_id, cluster) in endpoint.zigpy_endpoint.out_clusters.items():
            platform = zha_regs.SINGLE_OUTPUT_CLUSTER_DEVICE_CLASS.get(cluster.cluster_id)
            if platform is None:
                continue
            cluster_handler_class = zha_regs.ZIGBEE_CLUSTER_HANDLER_REGISTRY.get(cluster_id, ClusterHandler)
            cluster_handler = cluster_handler_class(cluster, endpoint)
            self.probe_single_cluster(platform, cluster_handler, endpoint)

    @staticmethod
    @callback
    def discover_multi_entities(endpoint: Endpoint, config_diagnostic_entities: bool=False) -> None:
        ...
        if config_diagnostic_entities:
            (matches, claimed) = zha_regs.ZHA_ENTITIES.get_config_diagnostic_entity(endpoint.device.manufacturer, endpoint.device.model, list(endpoint.all_cluster_handlers.values()), endpoint.device.quirk_id)
        else:
            (matches, claimed) = zha_regs.ZHA_ENTITIES.get_multi_entity(endpoint.device.manufacturer, endpoint.device.model, endpoint.unclaimed_cluster_handlers(), endpoint.device.quirk_id)
        endpoint.claim_cluster_handlers(claimed)
        for (platform, ent_n_handler_list) in matches.items():
            for entity_and_handler in ent_n_handler_list:
                _LOGGER.debug("'%s' component -> '%s' using %s", platform, entity_and_handler.entity_class.__name__, [ch.name for ch in entity_and_handler.claimed_cluster_handlers])
        for (platform, ent_n_handler_list) in matches.items():
            for entity_and_handler in ent_n_handler_list:
                if platform == cmpt_by_dev_type:
                    endpoint.async_new_entity(platform, entity_and_handler.entity_class, endpoint.unique_id, entity_and_handler.claimed_cluster_handlers)
                    break
                first_ch = entity_and_handler.claimed_cluster_handlers[0]
                endpoint.async_new_entity(platform, entity_and_handler.entity_class, f'{endpoint.unique_id}-{first_ch.cluster.cluster_id}', entity_and_handler.claimed_cluster_handlers)
...
class GroupProbe:
    ...
    def initialize(self, hass: HomeAssistant) -> None:
        ...
        self._hass = hass
        self._unsubs.append(async_dispatcher_connect(hass, zha_const.SIGNAL_GROUP_ENTITY_REMOVED, self._reprobe_group))
    ...
    def discover_group_entities(self, group: ZHAGroup) -> None:
        ...
        if len(group.members) < 2:
            _LOGGER.debug('Group: %s:0x%04x has less than 2 members - skipping entity discovery', group.name, group.group_id)
            return
        ...
        for domain in entity_domains:
            ...
            if entity_class is None:
                continue
            zha_data.platforms[domain].append((entity_class, (group.get_domain_entity_ids(domain), f'{domain}_zha_group_0x{group.group_id:04x}', group.group_id, zha_gateway.coordinator_zha_device)))
        async_dispatcher_send(self._hass, zha_const.SIGNAL_ADD_ENTITIES)

    @staticmethod
    def determine_entity_domains(hass: HomeAssistant, group: ZHAGroup) -> list[Platform]:
        ...
        for member in group.members:
            if member.device.is_coordinator:
                continue
            entities = async_entries_for_device(entity_registry, member.device.device_id, include_disabled_entities=True)
            all_domain_occurrences.extend([cast(Platform, entity.domain) for entity in entities if entity.domain in zha_regs.GROUP_ENTITY_DOMAINS])
        ...
        _LOGGER.debug('The entity domains are: %s for group: %s:0x%04x', entity_domains, group.name, group.group_id)
        return entity_domains
PROBE = ProbeEndpoint()
