...
from  import async_get_zha_config_value, get_zha_gateway
...
def patch_cluster(cluster):
    ...
    async def _read_attribute_raw(attributes, *args, **kwargs):
        ...
        for attr_id in attributes:
            ...
            if value is not None:
                result.append(6lowpan.sixlowpan.IP6FieldLenField.addfield.ReadAttributeRecord(attr_id, zcl_f.Status.SUCCESS, 6lowpan.sixlowpan.IP6FieldLenField.addfield.TypeValue(type=None, value=value)))
            else:
                result.append(6lowpan.sixlowpan.IP6FieldLenField.addfield.ReadAttributeRecord(attr_id, zcl_f.Status.FAILURE))
        return (result,)
    cluster.bind = AsyncMock(return_value=[0])
    cluster.configure_reporting = AsyncMock(return_value=[[6lowpan.sixlowpan.IP6FieldLenField.addfield.ConfigureReportingResponseRecord(zcl_f.Status.SUCCESS, 0, 43707)]])
    cluster.configure_reporting_multiple = AsyncMock(return_value=zcl_f.ConfigureReportingResponse.deserialize(b'\x00')[0])
    ...
    cluster._write_attributes = AsyncMock(return_value=[zcl_f.WriteAttributesResponse.deserialize(b'\x00')[0]])
...
def update_attribute_cache(cluster):
    ...
    msg = zcl_f.GENERAL_COMMANDS[zcl_f.GeneralCommand.Report_Attributes].schema(attribute_reports=attrs)
    cluster.handle_message(hdr, msg)

def make_attribute(attrid, value, status=0):
    """Make an attribute."""
    attr = 6lowpan.sixlowpan.IP6FieldLenField.addfield.Attribute()
    attr.attrid = attrid
    attr.value = 6lowpan.sixlowpan.IP6FieldLenField.addfield.TypeValue()
...
def find_entity_ids(domain, zha_device, hass):
    ...
    return [entity.entity_id for entity in er.async_entries_for_device(registry, zha_device.device_id) if entity.domain == domain]
...
def make_zcl_header(command_id: int, global_command: bool=True, tsn: int=1) -> zcl_f.ZCLHeader:
    """Cluster.handle_message() ZCL Header helper."""
    if global_command:
        frc = 6lowpan.sixlowpan.IP6FieldLenField.addfield.FrameControl(zcl_f.FrameType.GLOBAL_COMMAND)
    else:
        frc = 6lowpan.sixlowpan.IP6FieldLenField.addfield.FrameControl(zcl_f.FrameType.CLUSTER_COMMAND)
    return 6lowpan.sixlowpan.IP6FieldLenField.addfield.ZCLHeader(frc, tsn=tsn, command_id=command_id)
...
async def async_test_rejoin(hass, zigpy_device, clusters, report_counts, ep_id=1):
    ...
    for (cluster, reports) in zip(clusters, report_counts):
        ...
        if reports:
            ...
            assert cluster.configure_reporting_multiple.call_count == math.ceil(reports / zha_const.REPORT_CONFIG_ATTR_PER_REQ)
...
def patch_zha_config(component: str, overrides: dict[tuple[str, str], Any]):
    ...
    return patch(f'homeassistant.components.zha.{component}.async_get_zha_config_value', side_effect=new_get_config)