...
from  import async_call_later
from  import registries
from  import REPORT_CONFIG_ASAP, REPORT_CONFIG_BATTERY_SAVE, REPORT_CONFIG_DEFAULT, REPORT_CONFIG_IMMEDIATE, REPORT_CONFIG_MAX_INT, REPORT_CONFIG_MIN_INT, SIGNAL_ATTR_UPDATED, SIGNAL_MOVE_LEVEL, SIGNAL_SET_LEVEL, SIGNAL_UPDATE_DEVICE
from  import AttrReportConfig, ClientClusterHandler, ClusterHandler, parse_and_log_command
...
class AnalogInput(ClusterHandler):
    ...
    REPORT_CONFIG = (6lowpan.sixlowpan.IP6FieldLenField.addfield(attr='present_value', config=REPORT_CONFIG_DEFAULT),)
...
class AnalogOutput(ClusterHandler):
    ...
    REPORT_CONFIG = (6lowpan.sixlowpan.IP6FieldLenField.addfield(attr='present_value', config=REPORT_CONFIG_DEFAULT),)
    ZCL_INIT_ATTRS = {'min_present_value': True, 'max_present_value': True, 'resolution': True, 'relinquish_default': True, 'description': True, 'engineering_units': True, 'application_type': True}
...
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(general.ApplianceControl.cluster_id)
...
class BasicClusterHandler(ClusterHandler):
    ...
    BATTERY = 3
    BIND: bool = False
    POWER_SOURCES = {UNKNOWN: 'Unknown', 1: 'Mains (single phase)', 2: 'Mains (3 phase)', BATTERY: 'Battery', 4: 'DC source', 5: 'Emergency mains constantly powered', 6: 'Emergency mains and transfer switch'}

    def __init__(self, cluster: zigpy.zcl.Cluster, endpoint: Endpoint) -> None:
        """Initialize Basic cluster handler."""
        super().__init__(cluster, endpoint)
        if 6lowpan.sixlowpan.IP6FieldLenField.addfield(self) and self.cluster.endpoint.endpoint_id == 2:
            self.ZCL_INIT_ATTRS = self.ZCL_INIT_ATTRS.copy()
            self.ZCL_INIT_ATTRS['trigger_indicator'] = True
        elif self.cluster.endpoint.manufacturer == 'TexasInstruments' and self.cluster.endpoint.model == 'ti.router':
            self.ZCL_INIT_ATTRS = self.ZCL_INIT_ATTRS.copy()
...
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(general.DeviceTemperature.cluster_id)
...
class LevelControlClusterHandler(ClusterHandler):
    ...
    CURRENT_LEVEL = 0
    REPORT_CONFIG = (6lowpan.sixlowpan.IP6FieldLenField.addfield(attr='current_level', config=REPORT_CONFIG_ASAP),)
    ZCL_INIT_ATTRS = {'on_off_transition_time': True, 'on_level': True, 'on_transition_time': True, 'off_transition_time': True, 'default_move_rate': True, 'start_up_current_level': True}
...
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(general.MultistateOutput.cluster_id)
...
class OnOffClusterHandler(ClusterHandler):
    ...
    REPORT_CONFIG = (6lowpan.sixlowpan.IP6FieldLenField.addfield(attr='on_off', config=REPORT_CONFIG_IMMEDIATE),)
    ZCL_INIT_ATTRS = {'start_up_on_off': True}

    def __init__(self, cluster: zigpy.zcl.Cluster, endpoint: Endpoint) -> None:
        """Initialize OnOffClusterHandler."""
        super().__init__(cluster, endpoint)
        self._off_listener = None
        if self.cluster.endpoint.model not in ('TS011F', 'TS0121', 'TS0001', 'TS0002', 'TS0003', 'TS0004'):
            return
        try:
    ...
    def matches(cls, cluster: zigpy.zcl.Cluster, endpoint: Endpoint) -> bool:
        ...
        return not (cluster.endpoint.device.manufacturer == 'Konke' and cluster.endpoint.device.model in ('3AFE280100510001', '3AFE170100510001'))
    ...
    async def turn_on(self) -> None:
        ...
        if result[1] is not Status.SUCCESS:
            raise 6lowpan.sixlowpan.IP6FieldLenField.addfield(f'Failed to turn on: {result[1]}')
    ...
    async def turn_off(self) -> None:
        ...
        if result[1] is not Status.SUCCESS:
            raise 6lowpan.sixlowpan.IP6FieldLenField.addfield(f'Failed to turn off: {result[1]}')
    ...
    def cluster_command(self, tsn, command_id, args):
        ...
        if cmd in ('off', 'off_with_effect'):
            ...
            if should_accept == 0 or (should_accept == 1 and bool(self.on_off)):
                ...
                self.cluster.update_attribute(self.ON_OFF, t.Bool.true)
                if on_time > 0:
                    self._off_listener = 6lowpan.sixlowpan.IP6FieldLenField.addfield(self._endpoint.device.hass, on_time / 10, self.set_to_off)
        elif cmd == 'toggle':
    ...
    def attribute_updated(self, attrid: int, value: Any, _: Any) -> None:
        ...
        if attrid == self.ON_OFF:
            self.async_send_signal(f'{self.unique_id}_{SIGNAL_ATTR_UPDATED}', attrid, 'on_off', value)
...
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(general.OnOffConfiguration.cluster_id)
...
class Ota(ClientClusterHandler):
    ...
    @callback
    def cluster_command(self, tsn: int, command_id: int, args: list[Any] | None) -> None:
        """Handle OTA commands."""
...
class PollControl(ClusterHandler):
    ...
    CHECKIN_INTERVAL = 55 * 60 * 4
    CHECKIN_FAST_POLL_TIMEOUT = 2 * 4
    LONG_POLL = 6 * 4
    _IGNORED_MANUFACTURER_ID = {4476}
...
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(general.PowerConfiguration.cluster_id)
class PowerConfigurationClusterHandler(ClusterHandler):
    """Cluster handler for the zigbee power configuration cluster."""
    REPORT_CONFIG = (6lowpan.sixlowpan.IP6FieldLenField.addfield(attr='battery_voltage', config=REPORT_CONFIG_BATTERY_SAVE), 6lowpan.sixlowpan.IP6FieldLenField.addfield(attr='battery_percentage_remaining', config=REPORT_CONFIG_BATTERY_SAVE))
    ...
    def async_initialize_cluster_handler_specific(self, from_cache: bool) -> Coroutine:
        ...
        return self.get_attributes(attributes, from_cache=from_cache, only_cache=from_cache)
