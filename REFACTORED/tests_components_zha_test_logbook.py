...
from  import CONF_DEVICE_ID, CONF_UNIQUE_ID, Platform
...
from  import MockRow, mock_humanify
ON = 1
OFF = 0
...
async def test_zha_logbook_event_device_no_triggers(hass: HomeAssistant, mock_devices) -> None:
    ...
    assert events[0]['name'] == 'FakeManufacturer FakeModel'
    assert events[0]['domain'] == 'zha'
    assert events[0]['message'] == "Shake event was fired with parameters: {'test': 'test'}"
    assert events[1]['name'] == 'FakeManufacturer FakeModel'
    assert events[1]['domain'] == 'zha'
    assert events[1]['message'] == "Zha Event was fired with parameters: {'test': 'test'}"
...
async def test_zha_logbook_event_device_no_device(hass: HomeAssistant, mock_devices) -> None:
    """Test ZHA logbook events without device and without triggers."""
    hass.config.components.add('recorder')
    assert await async_setup_component(hass, 'logbook', {})
    events = mock_humanify(hass, [MockRow(ZHA_EVENT, {CONF_DEVICE_ID: 'non-existing-device', COMMAND: COMMAND_SHAKE, 'device_ieee': '90:fd:9f:ff:fe:fe:d8:a1', CONF_UNIQUE_ID: '90:fd:9f:ff:fe:fe:d8:a1:1:0x0006', 'endpoint_id': 1, 'cluster_id': 6, 'params': {'test': 'test'}})])
