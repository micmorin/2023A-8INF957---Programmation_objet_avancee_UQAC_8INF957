...
from  import CLUSTER_HANDLER_ANALOG_INPUT, CLUSTER_HANDLER_BASIC, CLUSTER_HANDLER_DEVICE_TEMPERATURE, CLUSTER_HANDLER_ELECTRICAL_MEASUREMENT, CLUSTER_HANDLER_HUMIDITY, CLUSTER_HANDLER_ILLUMINANCE, CLUSTER_HANDLER_LEAF_WETNESS, CLUSTER_HANDLER_POWER_CONFIGURATION, CLUSTER_HANDLER_PRESSURE, CLUSTER_HANDLER_SMARTENERGY_METERING, CLUSTER_HANDLER_SOIL_MOISTURE, CLUSTER_HANDLER_TEMPERATURE, CLUSTER_HANDLER_THERMOSTAT, SIGNAL_ADD_ENTITIES, SIGNAL_ATTR_UPDATED
from  import get_zha_data
from  import SMARTTHINGS_HUMIDITY_CLUSTER, ZHA_ENTITIES
from  import ZhaEntity
if TYPE_CHECKING:
    from  import ClusterHandler
    from  import ZHADevice
PARALLEL_UPDATES = 5
BATTERY_SIZES = {0: 'No battery', 1: 'Built in', 2: 'Other', 3: 'AA', 4: 'AAA', 5: 'C', 6: 'D', 7: 'CR2', 8: 'CR123A', 9: 'CR2450', 10: 'CR2032', 11: 'CR1632', 255: 'Unknown'}
CLUSTER_HANDLER_ST_HUMIDITY_CLUSTER = f'cluster_handler_0x{SMARTTHINGS_HUMIDITY_CLUSTER:04x}'
...
class Sensor(ZhaEntity, SensorEntity):
    ...
    def __init__(self, unique_id: str, zha_device: ZHADevice, cluster_handlers: list[ClusterHandler], **kwargs: Any) -> None:
        """Init this sensor."""
        super().__init__(unique_id, zha_device, cluster_handlers, **kwargs)
        self._cluster_handler: ClusterHandler = cluster_handlers[0]

    @classmethod
    def create_entity(cls, unique_id: str, zha_device: ZHADevice, cluster_handlers: list[ClusterHandler], **kwargs: Any) -> Self | None:
        """Entity Factory.

        Return entity if it is a supported configuration, otherwise return None
        """
        cluster_handler = cluster_handlers[0]
        if cls._attribute_name in cluster_handler.cluster.unsupported_attributes or cls._attribute_name not in cluster_handler.cluster.attributes_by_name:
            return None
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self.async_accept_signal(self._cluster_handler, SIGNAL_ATTR_UPDATED, self.async_set_state)
    ...
    def formatter(self, value: int | enum.IntEnum) -> int | float | str | None:
        ...
        if self._decimals > 0:
            return round(float(value * self._multiplier) / self._divisor, self._decimals)
        return round(float(value * self._multiplier) / self._divisor)

@MULTI_MATCH(cluster_handler_names=CLUSTER_HANDLER_ANALOG_INPUT, manufacturers='Digi', stop_on_match_group=CLUSTER_HANDLER_ANALOG_INPUT)
...
@MULTI_MATCH(cluster_handler_names=CLUSTER_HANDLER_ELECTRICAL_MEASUREMENT, stop_on_match_group=CLUSTER_HANDLER_ELECTRICAL_MEASUREMENT, models={'VZM31-SN', 'SP 234', 'outletv4'})
class ElectricalMeasurement(Sensor):
    ...
    def formatter(self, value: int) -> int | float:
        ...
        multiplier = getattr(self._cluster_handler, f'{self._div_mul_prefix}_multiplier')
...
@MULTI_MATCH(cluster_handler_names=CLUSTER_HANDLER_ELECTRICAL_MEASUREMENT, stop_on_match_group=CLUSTER_HANDLER_ELECTRICAL_MEASUREMENT)
...
@MULTI_MATCH(generic_ids=CLUSTER_HANDLER_ST_HUMIDITY_CLUSTER, stop_on_match_group=CLUSTER_HANDLER_HUMIDITY)
@MULTI_MATCH(cluster_handler_names=CLUSTER_HANDLER_HUMIDITY, stop_on_match_group=CLUSTER_HANDLER_HUMIDITY)
...
@MULTI_MATCH(cluster_handler_names=CLUSTER_HANDLER_SMARTENERGY_METERING, stop_on_match_group=CLUSTER_HANDLER_SMARTENERGY_METERING)
class SmartEnergyMetering(Sensor):
    ...
    unit_of_measure_map = {0: UnitOfPower.WATT, 1: UnitOfVolumeFlowRate.CUBIC_METERS_PER_HOUR, 2: UnitOfVolumeFlowRate.CUBIC_FEET_PER_MINUTE, 3: f'100 {UnitOfVolumeFlowRate.CUBIC_METERS_PER_HOUR}', 4: f'US {UnitOfVolume.GALLONS}/{UnitOfTime.HOURS}', 5: f'IMP {UnitOfVolume.GALLONS}/{UnitOfTime.HOURS}', 6: UnitOfPower.BTU_PER_HOUR, 7: f'l/{UnitOfTime.HOURS}', 8: UnitOfPressure.KPA, 9: UnitOfPressure.KPA, 10: f'1000 {UnitOfVolume.GALLONS}/{UnitOfTime.HOURS}', 11: 'unitless', 12: f'MJ/{UnitOfTime.SECONDS}'}
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if (status := self._cluster_handler.status) is not None:
            if isinstance(status, enum.IntFlag):
                attrs['status'] = str(status.name if status.name is not None else status.value)
            else:
...
class SmartEnergySummation(SmartEnergyMetering):
    ...
    unit_of_measure_map = {0: UnitOfEnergy.KILO_WATT_HOUR, 1: UnitOfVolume.CUBIC_METERS, 2: UnitOfVolume.CUBIC_FEET, 3: f'100 {UnitOfVolume.CUBIC_FEET}', 4: f'US {UnitOfVolume.GALLONS}', 5: f'IMP {UnitOfVolume.GALLONS}', 6: 'BTU', 7: UnitOfVolume.LITERS, 8: UnitOfPressure.KPA, 9: UnitOfPressure.KPA, 10: f'1000 {UnitOfVolume.CUBIC_FEET}', 11: 'unitless', 12: 'MJ'}

    def formatter(self, value: int) -> int | float:
        """Numeric pass-through formatter."""
        if self._cluster_handler.unit_of_measurement != 0:
            return self._cluster_handler.summa_formatter(value)
        cooked = float(self._cluster_handler.multiplier * value) / self._cluster_handler.divisor
        return round(cooked, 3)

@MULTI_MATCH(cluster_handler_names=CLUSTER_HANDLER_SMARTENERGY_METERING, models={'TS011F', 'ZLinky_TIC'}, stop_on_match_group=CLUSTER_HANDLER_SMARTENERGY_METERING)
...
@MULTI_MATCH(cluster_handler_names=CLUSTER_HANDLER_SMARTENERGY_METERING, models={'ZLinky_TIC'})
...
@MULTI_MATCH(cluster_handler_names='voc_level', models='lumi.airmonitor.acn01', stop_on_match_group='voc_level')
class PPBVOCLevel(Sensor):
    """VOC Level sensor."""
    _attribute_name = 'measured_value'
    _attr_device_class: SensorDeviceClass = SensorDeviceClass.VOLATILE_ORGANIC_COMPOUNDS_PARTS
    _attr_state_class: SensorStateClass = SensorStateClass.MEASUREMENT
    _decimals = 0
    _multiplier = 1
...
@MULTI_MATCH(cluster_handler_names=CLUSTER_HANDLER_THERMOSTAT, stop_on_match_group=CLUSTER_HANDLER_THERMOSTAT)
class ThermostatHVACAction(Sensor):
    ...
    def native_value(self) -> str | None:
        ...
        if self._cluster_handler.pi_heating_demand is None and self._cluster_handler.pi_cooling_demand is None:
    ...
    def _rm_rs_action(self) -> HVACAction | None:
        ...
        if (running_state := self._cluster_handler.running_state) is None:
            return None
        rs_heat = self._cluster_handler.RunningState.Heat_State_On | self._cluster_handler.RunningState.Heat_2nd_Stage_On
        if running_state & rs_heat:
            return HVACAction.HEATING
        rs_cool = self._cluster_handler.RunningState.Cool_State_On | self._cluster_handler.RunningState.Cool_2nd_Stage_On
        if running_state & rs_cool:
            return HVACAction.COOLING
        running_state = self._cluster_handler.running_state
        if running_state and running_state & (self._cluster_handler.RunningState.Fan_State_On | self._cluster_handler.RunningState.Fan_2nd_Stage_On | self._cluster_handler.RunningState.Fan_3rd_Stage_On):
...
@MULTI_MATCH(cluster_handler_names={CLUSTER_HANDLER_THERMOSTAT}, manufacturers='Sinope Technologies', stop_on_match_group=CLUSTER_HANDLER_THERMOSTAT)
class SinopeHVACAction(ThermostatHVACAction):
    ...
    def _rm_rs_action(self) -> HVACAction:
        ...
        if running_state and running_state & (self._cluster_handler.RunningState.Fan_State_On | self._cluster_handler.RunningState.Fan_2nd_Stage_On | self._cluster_handler.RunningState.Fan_3rd_Stage_On):
            return HVACAction.FAN
        if self._cluster_handler.system_mode != self._cluster_handler.SystemMode.Off and running_mode == self._cluster_handler.SystemMode.Off:
            return HVACAction.IDLE
        return HVACAction.OFF
...
@MULTI_MATCH(cluster_handler_names='tuya_manufacturer', manufacturers={'_TZE200_htnnfasr'})
