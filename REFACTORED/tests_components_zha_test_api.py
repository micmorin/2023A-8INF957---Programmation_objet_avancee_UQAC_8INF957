...
from  import AsyncMock, MagicMock, call, patch
import pytest
...
async def test_async_get_network_settings_active(hass: HomeAssistant, setup_zha) -> None:
...
async def test_async_get_network_settings_inactive(hass: HomeAssistant, setup_zha, zigpy_app_controller: ControllerApplication) -> None:
    ...
    with patch.dict('homeassistant.components.zha.core.const.RadioType._member_map_', ezsp=MagicMock(controller=controller, description='EZSP')):
        settings = await api.async_get_network_settings(hass)
    assert settings.network_info.channel == 20
    assert len(zigpy_app_controller.start_network.mock_calls) == 0

async def test_async_get_network_settings_missing(hass: HomeAssistant, setup_zha, zigpy_app_controller: ControllerApplication) -> None:
...
async def test_change_channel(hass: HomeAssistant, setup_zha, zigpy_app_controller: ControllerApplication) -> None:
...
async def test_change_channel_auto(hass: HomeAssistant, setup_zha, zigpy_app_controller: ControllerApplication) -> None:
