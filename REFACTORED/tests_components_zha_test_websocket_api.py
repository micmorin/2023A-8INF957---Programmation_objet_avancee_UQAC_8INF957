...
from  import unhexlify
from  import deepcopy
from  import TYPE_CHECKING
from  import ANY, AsyncMock, MagicMock, call, patch
...
from  import const
from  import DOMAIN
from  import ATTR_CLUSTER_ID, ATTR_CLUSTER_TYPE, ATTR_ENDPOINT_ID, ATTR_ENDPOINT_NAMES, ATTR_IEEE, ATTR_MANUFACTURER, ATTR_MODEL, ATTR_NEIGHBORS, ATTR_QUIRK_APPLIED, ATTR_TYPE, BINDINGS, CLUSTER_TYPE_IN, EZSP_OVERWRITE_EUI64, GROUP_ID, GROUP_IDS, GROUP_NAME
from  import ATTR_DURATION, ATTR_INSTALL_CODE, ATTR_QR_CODE, ATTR_SOURCE_IEEE, ATTR_TARGET_IEEE, ID, SERVICE_PERMIT, TYPE, async_load_api
from  import ATTR_NAME, Platform
from  import Context, HomeAssistant
from  import FIXTURE_GRP_ID, FIXTURE_GRP_NAME, SIG_EP_INPUT, SIG_EP_OUTPUT, SIG_EP_PROFILE, SIG_EP_TYPE
from  import BASE_CUSTOM_CONFIGURATION, CONFIG_WITH_ALARM_OPTIONS
...
def required_platform_only():
    ...
    with patch('homeassistant.components.zha.PLATFORMS', (Platform.ALARM_CONTROL_PANEL, Platform.SELECT, Platform.SENSOR, Platform.SWITCH)):
        yield

@pytest.fixture
async def device_switch(hass, zigpy_device_mock, zha_device_joined):
    """Test ZHA switch platform."""
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [general.OnOff.cluster_id, general.Basic.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.ON_OFF_SWITCH, SIG_EP_PROFILE: zigpy.profiles.zha.PROFILE_ID}}, ieee=IEEE_SWITCH_DEVICE)
...
async def device_ias_ace(hass, zigpy_device_mock, zha_device_joined):
    ...
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [security.IasAce.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.IAS_ANCILLARY_CONTROL, SIG_EP_PROFILE: zigpy.profiles.zha.PROFILE_ID}})
    zha_device = await zha_device_joined(zigpy_device)
    zha_device.available = True
    return zha_device
...
async def device_groupable(hass, zigpy_device_mock, zha_device_joined):
    ...
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [general.OnOff.cluster_id, general.Basic.cluster_id, general.Groups.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.ON_OFF_SWITCH, SIG_EP_PROFILE: zigpy.profiles.zha.PROFILE_ID}}, ieee=IEEE_GROUPABLE_DEVICE)
    zha_device = await zha_device_joined(zigpy_device)
    zha_device.available = True
    return zha_device
...
async def test_device_clusters(hass: HomeAssistant, zha_client) -> None:
    ...
    await zha_client.send_json({ID: 5, TYPE: 'zha/devices/clusters', ATTR_IEEE: IEEE_SWITCH_DEVICE})
...
async def test_device_cluster_attributes(zha_client) -> None:
    ...
    await zha_client.send_json({ID: 5, TYPE: 'zha/devices/clusters/attributes', ATTR_ENDPOINT_ID: 1, ATTR_IEEE: IEEE_SWITCH_DEVICE, ATTR_CLUSTER_ID: 6, ATTR_CLUSTER_TYPE: CLUSTER_TYPE_IN})
    msg = await zha_client.receive_json()
...
async def test_device_cluster_commands(zha_client) -> None:
    ...
    await zha_client.send_json({ID: 5, TYPE: 'zha/devices/clusters/commands', ATTR_ENDPOINT_ID: 1, ATTR_IEEE: IEEE_SWITCH_DEVICE, ATTR_CLUSTER_ID: 6, ATTR_CLUSTER_TYPE: CLUSTER_TYPE_IN})
    msg = await zha_client.receive_json()
    commands = msg['result']
...
async def test_list_devices(zha_client) -> None:
    ...
    for device in devices:
        ...
        await zha_client.send_json({ID: msg_id, TYPE: 'zha/device', ATTR_IEEE: device[ATTR_IEEE]})
        msg = await zha_client.receive_json()
        device2 = msg['result']
...
async def test_get_zha_config_with_alarm(hass: HomeAssistant, zha_client, device_ias_ace) -> None:
    """Test getting ZHA custom configuration."""
    await zha_client.send_json({ID: 5, TYPE: 'zha/configuration'})
    msg = await zha_client.receive_json()
    configuration = msg['result']
...
async def test_update_zha_config(zha_client, app_controller: ControllerApplication) -> None:
    ...
    with patch('bellows.zigbee.application.ControllerApplication.new', return_value=app_controller):
        await zha_client.send_json({ID: 5, TYPE: 'zha/configuration/update', 'data': configuration['data']})
        msg = await zha_client.receive_json()
        assert msg['success']
        await zha_client.send_json({ID: 6, TYPE: 'zha/configuration'})
        msg = await zha_client.receive_json()
...
async def test_device_not_found(zha_client) -> None:
    ...
    await zha_client.send_json({ID: 6, TYPE: 'zha/device', ATTR_IEEE: '28:6d:97:00:01:04:11:8c'})
    msg = await zha_client.receive_json()
    assert msg['id'] == 6
...
async def test_list_groupable_devices(zha_client, device_groupable, zigpy_app_controller) -> None:
    """Test getting ZHA devices that have a group cluster."""
    coordinator = zigpy_app_controller.get_device(nwk=0)
...
async def test_remove_group(zha_client) -> None:
    ...
    groups = msg['result']
    assert len(groups) == 1
    await zha_client.send_json({ID: 15, TYPE: 'zha/group/remove', GROUP_IDS: [FIXTURE_GRP_ID]})
    msg = await zha_client.receive_json()
    assert msg['id'] == 15
    assert msg['type'] == const.TYPE_RESULT
...
@pytest.fixture
async def app_controller(hass: HomeAssistant, setup_zha, zigpy_app_controller: ControllerApplication) -> ControllerApplication:
    """Fixture for zigpy Application Controller."""
    await setup_zha()
    zigpy_app_controller.permit.reset_mock()
    return zigpy_app_controller

@pytest.mark.parametrize(('params', 'duration', 'node'), (({}, 60, None), ({ATTR_DURATION: 30}, 30, None), ({ATTR_DURATION: 33, ATTR_IEEE: 'aa:bb:cc:dd:aa:bb:cc:dd'}, 33, zigpy.types.EUI64.convert('aa:bb:cc:dd:aa:bb:cc:dd')), ({ATTR_IEEE: 'aa:bb:cc:dd:aa:bb:cc:d1'}, 60, zigpy.types.EUI64.convert('aa:bb:cc:dd:aa:bb:cc:d1'))))
async def test_permit_ha12(hass: HomeAssistant, app_controller: ControllerApplication, hass_admin_user: MockUser, params, duration, node) -> None:
    """Test permit service."""
    await hass.services.async_call(DOMAIN, SERVICE_PERMIT, params, True, Context(user_id=hass_admin_user.id))
...
IC_TEST_PARAMS = (({ATTR_SOURCE_IEEE: IEEE_SWITCH_DEVICE, ATTR_INSTALL_CODE: '5279-7BF4-A508-4DAA-8E17-12B6-1741-CA02-4051'}, zigpy.types.EUI64.convert(IEEE_SWITCH_DEVICE), unhexlify('52797BF4A5084DAA8E1712B61741CA024051')), ({ATTR_SOURCE_IEEE: IEEE_SWITCH_DEVICE, ATTR_INSTALL_CODE: '52797BF4A5084DAA8E1712B61741CA024051'}, zigpy.types.EUI64.convert(IEEE_SWITCH_DEVICE), unhexlify('52797BF4A5084DAA8E1712B61741CA024051')))

@pytest.mark.parametrize(('params', 'src_ieee', 'code'), IC_TEST_PARAMS)
async def test_permit_with_install_code(hass: HomeAssistant, app_controller: ControllerApplication, hass_admin_user: MockUser, params, src_ieee, code) -> None:
...
IC_FAIL_PARAMS = ({ATTR_SOURCE_IEEE: IEEE_SWITCH_DEVICE, ATTR_INSTALL_CODE: '5279-7BF4-A508-4DAA-8E17-12B6-1741-CA02-4052'}, {ATTR_INSTALL_CODE: '5279-7BF4-A508-4DAA-8E17-12B6-1741-CA02-4051'}, {ATTR_SOURCE_IEEE: IEEE_SWITCH_DEVICE}, {ATTR_INSTALL_CODE: '5279-7BF4-A508-4DAA-8E17-12B6-1741-CA02-4051', ATTR_QR_CODE: 'Z:000D6FFFFED4163B$I:52797BF4A5084DAA8E1712B61741CA024051'}, {ATTR_SOURCE_IEEE: IEEE_SWITCH_DEVICE, ATTR_QR_CODE: 'Z:000D6FFFFED4163B$I:52797BF4A5084DAA8E1712B61741CA024051'}, {ATTR_QR_CODE: 'Z:000D6FFFFED4163B$I:52797BF4A5084DAA8E1712B61741CA024052'}, {ATTR_QR_CODE: 'G$M:751$S:357S00001579$D:000000000F350FFD%Z$A:04CF8CDF3C3C3C3C$I:52797BF4A5084DAA8E1712B61741CA024052'}, {ATTR_QR_CODE: '000D6FFFFED4163B|52797BF4A5084DAA8E1712B61741CA024052'})

@pytest.mark.parametrize('params', IC_FAIL_PARAMS)
async def test_permit_with_install_code_fail(hass: HomeAssistant, app_controller: ControllerApplication, hass_admin_user: MockUser, params) -> None:
    """Test permit service with install code."""
    with pytest.raises(vol.Invalid):
        await hass.services.async_call(DOMAIN, SERVICE_PERMIT, params, True, Context(user_id=hass_admin_user.id))
    assert app_controller.permit.await_count == 0
    assert app_controller.permit_with_key.call_count == 0
IC_QR_CODE_TEST_PARAMS = (({ATTR_QR_CODE: '000D6FFFFED4163B|52797BF4A5084DAA8E1712B61741CA024051'}, zigpy.types.EUI64.convert('00:0D:6F:FF:FE:D4:16:3B'), unhexlify('52797BF4A5084DAA8E1712B61741CA024051')), ({ATTR_QR_CODE: 'Z:000D6FFFFED4163B$I:52797BF4A5084DAA8E1712B61741CA024051'}, zigpy.types.EUI64.convert('00:0D:6F:FF:FE:D4:16:3B'), unhexlify('52797BF4A5084DAA8E1712B61741CA024051')), ({ATTR_QR_CODE: 'G$M:751$S:357S00001579$D:000000000F350FFD%Z$A:04CF8CDF3C3C3C3C$I:52797BF4A5084DAA8E1712B61741CA024051'}, zigpy.types.EUI64.convert('04:CF:8C:DF:3C:3C:3C:3C'), unhexlify('52797BF4A5084DAA8E1712B61741CA024051')))

@pytest.mark.parametrize(('params', 'src_ieee', 'code'), IC_QR_CODE_TEST_PARAMS)
async def test_permit_with_qr_code(hass: HomeAssistant, app_controller: ControllerApplication, hass_admin_user: MockUser, params, src_ieee, code) -> None:
...
@pytest.mark.parametrize(('params', 'src_ieee', 'code'), IC_QR_CODE_TEST_PARAMS)
async def test_ws_permit_with_qr_code(app_controller: ControllerApplication, zha_client, params, src_ieee, code) -> None:
    """Test permit service with install code from qr code."""
    await zha_client.send_json({ID: 14, TYPE: f'{DOMAIN}/devices/{SERVICE_PERMIT}', **params})
    msg = await zha_client.receive_json()
    assert msg['id'] == 14
    assert msg['type'] == const.TYPE_RESULT
    assert msg['success']
...
@pytest.mark.parametrize('params', IC_FAIL_PARAMS)
async def test_ws_permit_with_install_code_fail(app_controller: ControllerApplication, zha_client, params) -> None:
...
@pytest.mark.parametrize(('params', 'duration', 'node'), (({}, 60, None), ({ATTR_DURATION: 30}, 30, None), ({ATTR_DURATION: 33, ATTR_IEEE: 'aa:bb:cc:dd:aa:bb:cc:dd'}, 33, zigpy.types.EUI64.convert('aa:bb:cc:dd:aa:bb:cc:dd')), ({ATTR_IEEE: 'aa:bb:cc:dd:aa:bb:cc:d1'}, 60, zigpy.types.EUI64.convert('aa:bb:cc:dd:aa:bb:cc:d1'))))
async def test_ws_permit_ha12(app_controller: ControllerApplication, zha_client, params, duration, node) -> None:
...
async def test_get_network_settings(app_controller: ControllerApplication, zha_client) -> None:
...
async def test_list_network_backups(app_controller: ControllerApplication, zha_client) -> None:
...
async def test_create_network_backup(app_controller: ControllerApplication, zha_client) -> None:
...
async def test_restore_network_backup_success(app_controller: ControllerApplication, zha_client) -> None:
    """Test successfully restoring a backup."""
    backup = zigpy.backups.NetworkBackup()
    with patch.object(app_controller.backups, 'restore_backup', new=AsyncMock()) as p:
        await zha_client.send_json({ID: 6, TYPE: f'{DOMAIN}/network/backups/restore', 'backup': backup.as_dict()})
        msg = await zha_client.receive_json()
...
async def test_restore_network_backup_force_write_eui64(app_controller: ControllerApplication, zha_client) -> None:
    """Test successfully restoring a backup."""
    backup = zigpy.backups.NetworkBackup()
    with patch.object(app_controller.backups, 'restore_backup', new=AsyncMock()) as p:
        await zha_client.send_json({ID: 6, TYPE: f'{DOMAIN}/network/backups/restore', 'backup': backup.as_dict(), 'ezsp_force_write_eui64': True})
        msg = await zha_client.receive_json()
    p.assert_called_once_with(backup.replace(network_info=backup.network_info.replace(stack_specific={'ezsp': {EZSP_OVERWRITE_EUI64: True}})))
    assert msg['id'] == 6
    assert msg['type'] == const.TYPE_RESULT
    assert msg['success']

@patch('zigpy.backups.NetworkBackup.from_dict', new=lambda v: v)
async def test_restore_network_backup_failure(app_controller: ControllerApplication, zha_client) -> None:
    """Test successfully restoring a backup."""
    with patch.object(app_controller.backups, 'restore_backup', new=AsyncMock(side_effect=ValueError('Restore failed'))) as p:
        await zha_client.send_json({ID: 6, TYPE: f'{DOMAIN}/network/backups/restore', 'backup': 'a backup'})
        msg = await zha_client.receive_json()
...
async def test_websocket_change_channel(new_channel: int | str, app_controller: ControllerApplication, zha_client) -> None:
    """Test websocket API to migrate the network to a new channel."""
    with patch('homeassistant.components.zha.websocket_api.async_change_channel', autospec=True) as change_channel_mock:
        await zha_client.send_json({ID: 6, TYPE: f'{DOMAIN}/network/change_channel', 'new_channel': new_channel})
        msg = await zha_client.receive_json()
    assert msg['id'] == 6
    assert msg['type'] == const.TYPE_RESULT
    assert msg['success']
...
@pytest.mark.parametrize('operation', [('bind', zdo_types.ZDOCmd.Bind_req), ('unbind', zdo_types.ZDOCmd.Unbind_req)])
async def test_websocket_bind_unbind_devices(operation: tuple[str, zdo_types.ZDOCmd], app_controller: ControllerApplication, zha_client) -> None:
    """Test websocket API for binding and unbinding devices to devices."""
    (command_type, req) = operation
    with patch('homeassistant.components.zha.websocket_api.async_binding_operation', autospec=True) as binding_operation_mock:
        await zha_client.send_json({ID: 27, TYPE: f'zha/devices/{command_type}', ATTR_SOURCE_IEEE: IEEE_SWITCH_DEVICE, ATTR_TARGET_IEEE: IEEE_GROUPABLE_DEVICE})
        msg = await zha_client.receive_json()
    assert msg['id'] == 27
    assert msg['type'] == const.TYPE_RESULT
    assert msg['success']
    assert binding_operation_mock.mock_calls == [call(ANY, 6lowpan.sixlowpan.IP6FieldLenField.addfield.convert(IEEE_SWITCH_DEVICE), 6lowpan.sixlowpan.IP6FieldLenField.addfield.convert(IEEE_GROUPABLE_DEVICE), req)]

@pytest.mark.parametrize('command_type', ['bind', 'unbind'])
async def test_websocket_bind_unbind_group(command_type: str, hass: HomeAssistant, app_controller: ControllerApplication, zha_client) -> None:
    """Test websocket API for binding and unbinding devices to groups."""
    test_group_id = 1
    gateway_mock = MagicMock()
    with patch('homeassistant.components.zha.websocket_api.get_zha_gateway', return_value=gateway_mock):
        ...
        await zha_client.send_json({ID: 27, TYPE: f'zha/groups/{command_type}', ATTR_SOURCE_IEEE: IEEE_SWITCH_DEVICE, GROUP_ID: test_group_id, BINDINGS: [{ATTR_ENDPOINT_ID: 1, ID: 6, ATTR_NAME: 'OnOff', ATTR_TYPE: 'out'}]})
        msg = await zha_client.receive_json()
    assert msg['id'] == 27
    assert msg['type'] == const.TYPE_RESULT
    assert msg['success']
