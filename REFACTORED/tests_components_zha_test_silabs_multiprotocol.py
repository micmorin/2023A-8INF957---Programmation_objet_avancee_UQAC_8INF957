...
import 6lowpan.sixlowpan.IP6FieldLenField.addfield
from  import zha
from  import silabs_multiprotocol
...
async def test_async_get_channel_missing(hass: HomeAssistant, setup_zha, zigpy_app_controller: ControllerApplication) -> None:
...
async def test_change_channel_no_zha(hass: HomeAssistant, zigpy_app_controller: ControllerApplication) -> None:
...
async def test_change_channel_delay(hass: HomeAssistant, setup_zha, zigpy_app_controller: ControllerApplication, delay: float, sleep: float) -> None:
    """Test changing the channel with a delay."""
    await setup_zha()
    with patch('homeassistant.components.zha.silabs_multiprotocol.asyncio.sleep', autospec=True) as mock_sleep:
