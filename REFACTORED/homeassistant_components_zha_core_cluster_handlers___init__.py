...
def wrap_zigpy_exceptions() -> Iterator[None]:
    ...
    try:
        ...
        raise HomeAssistantError('Failed to send request: device did not respond') from exc
...
def parse_and_log_command(cluster_handler, tsn, command_id, args):
    ...
    cluster_handler.debug("received '%s' command with %s args on cluster_id '%s' tsn '%s'", name, args, cluster_handler.cluster.cluster_id, tsn)
    return name
...
class ClusterHandler(LogMixin):
    ...
    def __init__(self, cluster: zigpy.zcl.Cluster, endpoint: Endpoint) -> None:
        ...
        if not hasattr(self, '_value_attribute') and self.REPORT_CONFIG:
            attr_def: ZCLAttributeDef = self.cluster.attributes_by_name[self.REPORT_CONFIG[0]['attr']]
    ...
    async def bind(self):
        ...
        try:
            ...
            async_dispatcher_send(self._endpoint.device.hass, ZHA_CLUSTER_HANDLER_MSG, {ATTR_TYPE: ZHA_CLUSTER_HANDLER_MSG_BIND, ZHA_CLUSTER_HANDLER_MSG_DATA: {'cluster_name': self.cluster.name, 'cluster_id': self.cluster.cluster_id, 'success': res[0] == 0}})
        except (zigpy.exceptions.ZigbeeException, asyncio.TimeoutError) as ex:
            self.debug("Failed to bind '%s' cluster: %s", self.cluster.ep_attribute, str(ex), exc_info=ex)
            async_dispatcher_send(self._endpoint.device.hass, ZHA_CLUSTER_HANDLER_MSG, {ATTR_TYPE: ZHA_CLUSTER_HANDLER_MSG_BIND, ZHA_CLUSTER_HANDLER_MSG_DATA: {'cluster_name': self.cluster.name, 'cluster_id': self.cluster.cluster_id, 'success': False}})
    ...
    async def configure_reporting(self) -> None:
        ...
        event_data = {}
        kwargs = {}
        if self.cluster.cluster_id >= 64512 and self._endpoint.device.manufacturer_code:
        ...
        for attr_report in self.REPORT_CONFIG:
            ...
            try:
                ...
                attr_name = attr
            event_data[attr_name] = {'min': config[0], 'max': config[1], 'id': attr, 'name': attr_name, 'change': config[2], 'success': False}
        to_configure = [*self.REPORT_CONFIG]
        (chunk, rest) = (to_configure[:REPORT_CONFIG_ATTR_PER_REQ], to_configure[REPORT_CONFIG_ATTR_PER_REQ:])
        while chunk:
            ...
            try:
                ...
                break
            (chunk, rest) = (rest[:REPORT_CONFIG_ATTR_PER_REQ], rest[REPORT_CONFIG_ATTR_PER_REQ:])
        async_dispatcher_send(self._endpoint.device.hass, ZHA_CLUSTER_HANDLER_MSG, {ATTR_TYPE: ZHA_CLUSTER_HANDLER_MSG_CFG_RPT, ZHA_CLUSTER_HANDLER_MSG_DATA: {'cluster_name': self.cluster.name, 'cluster_id': self.cluster.cluster_id, 'attributes': event_data}})

    def _configure_reporting_status(self, attrs: dict[str, tuple[int, int, float | int]], res: list | tuple) -> None:
        ...
        if res[0].status == Status.SUCCESS and len(res) == 1:
            ...
            return
        failed = [self.cluster.find_attribute(record.attrid).name for record in res if record.status != Status.SUCCESS]
        self.debug("Successfully configured reporting for '%s' on '%s' cluster", set(attrs) - set(failed), self.name)
    ...
    async def async_configure(self) -> None:
        ...
        if not self._endpoint.device.skip_configuration:
            ...
            ch_specific_cfg = getattr(self, 'async_configure_cluster_handler_specific', None)
            if ch_specific_cfg:
    ...
    async def async_initialize(self, from_cache: bool) -> None:
        ...
        if cached:
            ...
            await self._get_attributes(True, cached, from_cache=True, only_cache=from_cache)
        if uncached:
            self.debug('initializing uncached cluster handler attributes: %s - from cache[%s]', uncached, from_cache)
            await self._get_attributes(True, uncached, from_cache=from_cache, only_cache=from_cache)
        ch_specific_init = getattr(self, 'async_initialize_cluster_handler_specific', None)
        if ch_specific_init:
            self.debug('Performing cluster handler specific initialization: %s', uncached)
    ...
    def attribute_updated(self, attrid: int, value: Any, _: Any) -> None:
        ...
        self.async_send_signal(f'{self.unique_id}_{SIGNAL_ATTR_UPDATED}', attrid, self._get_attribute_name(attrid), value)
    ...
    def zha_send_event(self, command: str, arg: list | dict | CommandSchema) -> None:
        ...
        self._endpoint.send_event({ATTR_UNIQUE_ID: self.unique_id, ATTR_CLUSTER_ID: self.cluster.cluster_id, ATTR_COMMAND: command, ATTR_ARGS: args, ATTR_PARAMS: params})
    ...
    async def get_attribute_value(self, attribute, from_cache=True):
        ...
        if self.cluster.cluster_id >= 64512 and manufacturer_code:
            manufacturer = manufacturer_code
        result = await safe_read(self._cluster, [attribute], allow_cache=from_cache, only_cache=from_cache, manufacturer=manufacturer)
        return result.get(attribute)

    async def _get_attributes(self, raise_exceptions: bool, attributes: list[str], from_cache: bool=True, only_cache: bool=True) -> dict[int | str, Any]:
        ...
        while chunk:
            try:
                ...
                (read, _) = await self.cluster.read_attributes(chunk, allow_cache=from_cache, only_cache=only_cache, manufacturer=manufacturer)
                result.update(read)
            except (asyncio.TimeoutError, zigpy.exceptions.ZigbeeException) as ex:
                self.debug("failed to get attributes '%s' on '%s' cluster: %s", chunk, self.cluster.ep_attribute, str(ex))
                if raise_exceptions:
                    raise
    ...
    async def write_attributes_safe(self, attributes: dict[str, Any], manufacturer: int | None=None) -> None:
        ...
        for record in res[0]:
            if record.status != Status.SUCCESS:
                try:
                    ...
                    value = 'unknown'
                raise HomeAssistantError(f'Failed to write attribute {name}={value}: {record.status}')
    ...
    def __getattr__(self, name):
        ...
        if hasattr(self._cluster, name) and callable(getattr(self._cluster, name)) and (name not in UNPROXIED_CLUSTER_METHODS):
...
class ClientClusterHandler(ClusterHandler):
    ...
    def cluster_command(self, tsn, command_id, args):
        ...
        if self._cluster.server_commands is not None and self._cluster.server_commands.get(command_id) is not None:
