...
class Endpoint:
    ...
    def zigbee_signature(self) -> tuple[int, dict[str, Any]]:
        ...
        return (self.id, {ATTR_PROFILE_ID: f'0x{self._zigpy_endpoint.profile_id:04x}' if self._zigpy_endpoint.profile_id is not None else '', ATTR_DEVICE_TYPE: f'0x{self._zigpy_endpoint.device_type:04x}' if self._zigpy_endpoint.device_type is not None else '', ATTR_IN_CLUSTERS: [f'0x{cluster_id:04x}' for cluster_id in sorted(self._zigpy_endpoint.in_clusters)], ATTR_OUT_CLUSTERS: [f'0x{cluster_id:04x}' for cluster_id in sorted(self._zigpy_endpoint.out_clusters)]})
    ...
    def add_all_cluster_handlers(self) -> None:
        ...
        for (cluster_id, cluster) in self.zigpy_endpoint.in_clusters.items():
            cluster_handler_class = registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.get(cluster_id, ClusterHandler)
            if not cluster_handler_class.matches(cluster, self):
                cluster_handler_class = ClusterHandler
            _LOGGER.info('Creating cluster handler for cluster id: %s class: %s', cluster_id, cluster_handler_class)
            if hasattr(cluster, 'ep_attribute') and cluster_id == zigpy.zcl.clusters.closures.DoorLock.cluster_id and (cluster.ep_attribute == 'multistate_input'):
                cluster_handler_class = MultistateInput
            try:
                cluster_handler = cluster_handler_class(cluster, self)
            except KeyError as err:
                _LOGGER.warning('Cluster handler %s for cluster %s on endpoint %s is invalid: %s', cluster_handler_class, cluster, self, err)
                continue
    ...
    def add_client_cluster_handlers(self) -> None:
        ...
        for (cluster_id, cluster_handler_class) in registries.CLIENT_CLUSTER_HANDLER_REGISTRY.items():
    ...
    async def _execute_handler_tasks(self, func_name: str, *args: Any) -> None:
        ...
        cluster_handlers = [*self.claimed_cluster_handlers.values(), *self.client_cluster_handlers.values()]
        ...
        for (cluster_handler, outcome) in zip(cluster_handlers, results):
            if isinstance(outcome, Exception):
                cluster_handler.warning("'%s' stage failed: %s", func_name, str(outcome), exc_info=outcome)
                continue
            cluster_handler.debug("'%s' stage succeeded", func_name)

    def async_new_entity(self, platform: Platform, entity_class: CALLABLE_T, unique_id: str, cluster_handlers: list[ClusterHandler]) -> None:
        """Create a new entity."""
        from  import DeviceStatus
        if self.device.status == DeviceStatus.INITIALIZED:
            return
        zha_data = get_zha_data(self.device.hass)
        zha_data.platforms[platform].append((entity_class, (unique_id, self.device, cluster_handlers)))
    ...
    def send_event(self, signal: dict[str, Any]) -> None:
        ...
        self.device.zha_send_event({const.ATTR_UNIQUE_ID: self.unique_id, const.ATTR_ENDPOINT_ID: self.id, **signal})
    ...
    def unclaimed_cluster_handlers(self) -> list[ClusterHandler]:
        ...
        return [self.all_cluster_handlers[cluster_id] for cluster_id in available - claimed]