...
from  import Status
from  import ATTR_CURRENT_POSITION, ATTR_POSITION, ATTR_TILT_POSITION, CoverDeviceClass, CoverEntity
from  import ConfigEntry
from  import STATE_CLOSED, STATE_CLOSING, STATE_OPEN, STATE_OPENING, Platform
...
from  import CLUSTER_HANDLER_COVER, CLUSTER_HANDLER_LEVEL, CLUSTER_HANDLER_ON_OFF, CLUSTER_HANDLER_SHADE, SIGNAL_ADD_ENTITIES, SIGNAL_ATTR_UPDATED, SIGNAL_SET_LEVEL
...
class ZhaCover(ZhaEntity, CoverEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self.async_accept_signal(self._cover_cluster_handler, SIGNAL_ATTR_UPDATED, self.async_set_position)
    ...
    def async_restore_last_state(self, last_state):
        ...
        if 'current_tilt_position' in last_state.attributes:
            self._tilt_position = last_state.attributes['current_tilt_position']
    ...
    async def async_set_cover_position(self, **kwargs: Any) -> None:
        ...
        self.async_update_state(STATE_CLOSING if new_pos < self._current_position else STATE_OPENING)
    ...
    async def async_set_cover_tilt_position(self, **kwargs: Any) -> None:
        ...
        self.async_update_state(STATE_CLOSING if new_pos < self._tilt_position else STATE_OPENING)
...
@MULTI_MATCH(cluster_handler_names={CLUSTER_HANDLER_LEVEL, CLUSTER_HANDLER_ON_OFF, CLUSTER_HANDLER_SHADE})
class Shade(ZhaEntity, CoverEntity):
    ...
    async def async_added_to_hass(self) -> None:
        """Run when about to be added to hass."""
        await super().async_added_to_hass()
        self.async_accept_signal(self._on_off_cluster_handler, SIGNAL_ATTR_UPDATED, self.async_set_open_closed)
        self.async_accept_signal(self._level_cluster_handler, SIGNAL_SET_LEVEL, self.async_set_level)
    ...
    async def async_set_cover_position(self, **kwargs: Any) -> None:
        """Move the roller shutter to a specific position."""
        new_pos = kwargs[ATTR_POSITION]
        res = await self._level_cluster_handler.move_to_level_with_on_off(new_pos * 255 / 100, 1)
        if res[1] != Status.SUCCESS:
            raise HomeAssistantError(f'Failed to set cover position: {res[1]}')
...
@MULTI_MATCH(cluster_handler_names={CLUSTER_HANDLER_LEVEL, CLUSTER_HANDLER_ON_OFF}, manufacturers='Keen Home Inc')
class KeenVent(Shade):
    ...
    async def async_open_cover(self, **kwargs: Any) -> None:
        ...
        await asyncio.gather(self._level_cluster_handler.move_to_level_with_on_off(position * 255 / 100, 1), self._on_off_cluster_handler.on())
        self._is_open = True
