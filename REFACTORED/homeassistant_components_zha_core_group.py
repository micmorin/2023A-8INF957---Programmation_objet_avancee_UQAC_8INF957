...
class ZHAGroupMember(LogMixin):
    ...
    def __init__(self, zha_group: ZHAGroup, zha_device: ZHADevice, endpoint_id: int) -> None:
    ...
    def associated_entities(self) -> list[dict[str, Any]]:
        ...
        for entity_ref in zha_device_registry.get(self.device.ieee):
            ...
            if entity is None or handler.cluster.endpoint.endpoint_id != self.endpoint_id:
                continue
            entity_info.append(GroupEntityReference(name=entity.name, original_name=entity.original_name, entity_id=entity_ref.reference_id)._asdict())
        return entity_info

    async def async_remove_from_group(self) -> None:
        """Remove the device endpoint from the provided zigbee group."""
        try:
            await self._zha_device.device.endpoints[self._endpoint_id].remove_from_group(self._zha_group.group_id)
        except (zigpy.exceptions.ZigbeeException, asyncio.TimeoutError) as ex:
            self.debug("Failed to remove endpoint: %s for device '%s' from group: 0x%04x ex: %s", self._endpoint_id, self._zha_device.ieee, self._zha_group.group_id, str(ex))
...
class ZHAGroup(LogMixin):
    ...
    def __init__(self, hass: HomeAssistant, zha_gateway: ZHAGateway, zigpy_group: zigpy.group.Group) -> None:
    ...
    def members(self) -> list[ZHAGroupMember]:
        ...
        return [ZHAGroupMember(self, self._zha_gateway.devices[member_ieee], endpoint_id) for (member_ieee, endpoint_id) in self._zigpy_group.members if member_ieee in self._zha_gateway.devices]
    ...
    async def async_add_members(self, members: list[GroupMember]) -> None:
        ...
        if len(members) > 1:
            ...
            for member in members:
                tasks.append(self._zha_gateway.devices[member.ieee].async_add_endpoint_to_group(member.endpoint_id, self.group_id))
            await asyncio.gather(*tasks)
        else:
            await self._zha_gateway.devices[members[0].ieee].async_add_endpoint_to_group(members[0].endpoint_id, self.group_id)
    ...
    async def async_remove_members(self, members: list[GroupMember]) -> None:
        ...
        if len(members) > 1:
            tasks = []
            for member in members:
                tasks.append(self._zha_gateway.devices[member.ieee].async_remove_endpoint_from_group(member.endpoint_id, self.group_id))
            await asyncio.gather(*tasks)
        else:
            await self._zha_gateway.devices[members[0].ieee].async_remove_endpoint_from_group(members[0].endpoint_id, self.group_id)
    ...
    def get_domain_entity_ids(self, domain: str) -> list[str]:
        ...
        for member in self.members:
            ...
            domain_entity_ids.extend([entity.entity_id for entity in entities if entity.domain == domain])
