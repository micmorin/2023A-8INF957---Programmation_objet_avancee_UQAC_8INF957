...
from  import CONF_DEFAULT_CONSIDER_UNAVAILABLE_BATTERY, CONF_DEFAULT_CONSIDER_UNAVAILABLE_MAINS
from  import STATE_OFF, STATE_UNAVAILABLE, Platform
from  import HomeAssistant
import homeassistant.helpers.device_registry as dr
import homeassistant.util.dt as dt_util
from  import async_enable_traffic, make_zcl_header
...
def required_platforms_only():
    ...
    with patch('homeassistant.components.zha.PLATFORMS', (Platform.DEVICE_TRACKER, Platform.SENSOR, Platform.SELECT, Platform.SWITCH, Platform.BINARY_SENSOR)):
        yield
...
def zigpy_device(zigpy_device_mock):
    ...
    def _dev(with_basic_cluster_handler: bool=True, **kwargs):
        ...
        endpoints = {3: {SIG_EP_INPUT: in_clusters, SIG_EP_OUTPUT: [], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.ON_OFF_SWITCH}}
...
def zigpy_device_mains(zigpy_device_mock):
    ...
    def _dev(with_basic_cluster_handler: bool=True):
        ...
        if with_basic_cluster_handler:
            in_clusters.append(general.Basic.cluster_id)
        endpoints = {3: {SIG_EP_INPUT: in_clusters, SIG_EP_OUTPUT: [], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.ON_OFF_SWITCH}}
        return zigpy_device_mock(endpoints, node_descriptor=b'\x02@\x84_\x11\x7fd\x00\x00,d\x00\x00')
    return _dev
...
async def ota_zha_device(zha_device_restored, zigpy_device_mock):
    ...
    zigpy_dev = zigpy_device_mock({1: {SIG_EP_INPUT: [general.Basic.cluster_id], SIG_EP_OUTPUT: [general.Ota.cluster_id], SIG_EP_TYPE: 4660}}, '00:11:22:33:44:55:66:77', 'test manufacturer', 'test model')
...
@patch('homeassistant.components.zha.core.cluster_handlers.general.BasicClusterHandler.async_initialize', new=mock.AsyncMock())
async def test_check_available_success(hass: HomeAssistant, device_with_basic_cluster_handler, zha_device_restored) -> None:
    ...
    device_with_basic_cluster_handler.last_seen = time.time() - zha_device.consider_unavailable_time - 2
...
@patch('homeassistant.components.zha.core.cluster_handlers.general.BasicClusterHandler.async_initialize', new=mock.AsyncMock())
async def test_check_available_unsuccessful(hass: HomeAssistant, device_with_basic_cluster_handler, zha_device_restored) -> None:
...
@patch('homeassistant.components.zha.core.cluster_handlers.general.BasicClusterHandler.async_initialize', new=mock.AsyncMock())
async def test_check_available_no_basic_cluster_handler(hass: HomeAssistant, device_without_basic_cluster_handler, zha_device_restored, caplog: pytest.LogCaptureFixture) -> None:
    ...
    await async_enable_traffic(hass, [zha_device])
    assert zha_device.available is True
    device_without_basic_cluster_handler.last_seen = time.time() - zha_device.consider_unavailable_time - 2
...
async def test_device_restore_availability(hass: HomeAssistant, request, device, last_seen_delta, is_available, zha_device_restored) -> None:
    """Test initial availability for restored devices."""
    zigpy_device = request.getfixturevalue(device)()
    zha_device = await zha_device_restored(zigpy_device, last_seen=time.time() - last_seen_delta)
...
async def test_device_is_active_coordinator(hass: HomeAssistant, zha_device_joined, zigpy_device) -> None:
    """Test that the current coordinator is uniquely detected."""
    current_coord_dev = zigpy_device(ieee='aa:bb:cc:dd:ee:ff:00:11', nwk=0)
    current_coord_dev.node_desc = current_coord_dev.node_desc.replace(logical_type=zdo_t.LogicalType.Coordinator)
    old_coord_dev = zigpy_device(ieee='aa:bb:cc:dd:ee:ff:00:12', nwk=0)
    old_coord_dev.node_desc = old_coord_dev.node_desc.replace(logical_type=zdo_t.LogicalType.Coordinator)
    ...
    current_coordinator.gateway.application_controller.state.node_info.ieee = current_coord_dev.ieee
