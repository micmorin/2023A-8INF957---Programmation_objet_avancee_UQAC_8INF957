...
from  import CLUSTER_HANDLER_HUE_OCCUPANCY, CLUSTER_HANDLER_IAS_WD, CLUSTER_HANDLER_INOVELLI, CLUSTER_HANDLER_ON_OFF, SIGNAL_ADD_ENTITIES, SIGNAL_ATTR_UPDATED, Strobe
...
CONFIG_DIAGNOSTIC_MATCH = functools.partial(ZHA_ENTITIES.config_diagnostic_match, Platform.SELECT)
...
class ZHAEnumSelectEntity(ZhaEntity, SelectEntity):
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        self._cluster_handler.data_cache[self._attribute_name] = self._enum[option.replace(' ', '_')]
    ...
    def async_restore_last_state(self, last_state) -> None:
        ...
        if last_state.state and last_state.state != STATE_UNKNOWN:
            self._cluster_handler.data_cache[self._attribute_name] = self._enum[last_state.state.replace(' ', '_')]
...
class ZCLEnumSelectEntity(ZhaEntity, SelectEntity):
    ...
    async def async_select_option(self, option: str) -> None:
        """Change the selected option."""
        await self._cluster_handler.write_attributes_safe({self._attribute_name: self._enum[option.replace(' ', '_')]})
        self.async_write_ha_state()
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names=CLUSTER_HANDLER_ON_OFF, models={'TS011F', 'TS0121', 'TS0001', 'TS0002', 'TS0003', 'TS0004'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names='opple_cluster', models={'lumi.motion.ac01', 'lumi.motion.ac02', 'lumi.motion.agl04'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names=CLUSTER_HANDLER_HUE_OCCUPANCY, manufacturers={'Philips', 'Signify Netherlands B.V.'}, models={'SML001'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names=CLUSTER_HANDLER_HUE_OCCUPANCY, manufacturers={'Philips', 'Signify Netherlands B.V.'}, models={'SML002', 'SML003', 'SML004'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names='window_covering', models={'lumi.curtain.agl001'})
