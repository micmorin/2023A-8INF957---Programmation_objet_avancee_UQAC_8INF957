...
from  import Any
from  import hvac
from  import ATTR_PERCENTAGE, ATTR_PRESET_MODE, FanEntity, FanEntityFeature, NotValidPresetModeError
from  import ConfigEntry
from  import STATE_UNAVAILABLE, Platform
from  import HomeAssistant, State, callback
from  import async_dispatcher_connect
from  import AddEntitiesCallback
from  import int_states_in_range, percentage_to_ranged_value, ranged_value_to_percentage
from  import discovery
from  import wrap_zigpy_exceptions
from  import CLUSTER_HANDLER_FAN, SIGNAL_ADD_ENTITIES, SIGNAL_ATTR_UPDATED
...
class BaseFan(FanEntity):
    ...
    async def async_set_preset_mode(self, preset_mode: str) -> None:
        ...
        if preset_mode not in self.preset_modes:
            raise NotValidPresetModeError(f'The preset_mode {preset_mode} is not a valid preset_mode: {self.preset_modes}')
...
class ZhaFan(BaseFan, ZhaEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self.async_accept_signal(self._fan_cluster_handler, SIGNAL_ATTR_UPDATED, self.async_set_state)

    @property
    def percentage(self) -> int | None:
        """Return the current speed percentage."""
        if self._fan_cluster_handler.fan_mode is None or self._fan_cluster_handler.fan_mode > self.speed_range[1]:
            return None
...
class FanGroup(BaseFan, ZhaGroupEntity):
    ...
    async def async_update(self) -> None:
        ...
        percentage_states: list[State] = [state for state in states if state.attributes.get(ATTR_PERCENTAGE)]
        preset_mode_states: list[State] = [state for state in states if state.attributes.get(ATTR_PRESET_MODE)]
    ...
    async def async_added_to_hass(self) -> None:
        ...
        await super().async_added_to_hass()
IKEA_SPEED_RANGE = (1, 10)
IKEA_PRESET_MODES_TO_NAME = {1: PRESET_MODE_AUTO, 2: 'Speed 1', 3: 'Speed 1.5', 4: 'Speed 2', 5: 'Speed 2.5', 6: 'Speed 3', 7: 'Speed 3.5', 8: 'Speed 4', 9: 'Speed 4.5', 10: 'Speed 5'}

@MULTI_MATCH(cluster_handler_names='ikea_airpurifier', models={'STARKVIND Air purifier', 'STARKVIND Air purifier table'})
class IkeaFan(ZhaFan):
    ...
    def default_on_percentage(self) -> int:
        """Return the default on percentage."""
        return int(100 / self.speed_count * self.preset_name_to_mode[PRESET_MODE_AUTO])

@MULTI_MATCH(cluster_handler_names=CLUSTER_HANDLER_FAN, models={'HBUniversalCFRemote', 'HDC52EastwindFan'})
class KofFan(ZhaFan):
