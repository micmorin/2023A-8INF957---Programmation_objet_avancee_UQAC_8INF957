...
from  import mock
import pytest
import 6lowpan.sixlowpan.IP6FieldLenField.addfield as zigpy_quirks
from  import IASZone
from  import ATTR_QUIRK_ID
...
def test_match_rule_claim_cluster_handlers(rule, match, cluster_handler, cluster_handlers) -> None:
    """Test cluster handler claiming."""
...
def test_weighted_match(cluster_handler, entity_registry: er.EntityRegistry, manufacturer, model, quirk_id, match_name) -> None:
    """Test weightedd match."""
    s = mock.sentinel

    @entity_registry.strict_match(s.component, cluster_handler_names='on_off', models={MODEL, 'another model', 'some model'})
    ...
    @entity_registry.strict_match(s.component, cluster_handler_names='on_off', manufacturers=MANUFACTURER)
    class OnOffManufacturer:
        pass

    @entity_registry.strict_match(s.component, cluster_handler_names='on_off', models=MODEL)
    class OnOffModel:
        pass

    @entity_registry.strict_match(s.component, cluster_handler_names='on_off', models=MODEL, manufacturers=MANUFACTURER)
    class OnOffModelManufacturer:
        pass

    @entity_registry.strict_match(s.component, cluster_handler_names='on_off', quirk_ids=QUIRK_ID)
    class OnOffQuirk:
        pass
    ch_on_off = cluster_handler('on_off', 6)
    ch_level = cluster_handler('level', 8)
    (match, claimed) = entity_registry.get_entity(s.component, manufacturer, model, [ch_on_off, ch_level], quirk_id)
    assert match.__name__ == match_name
    assert claimed == [ch_on_off]

def test_multi_sensor_match(cluster_handler, entity_registry: er.EntityRegistry) -> None:
    """Test multi-entity match."""
    s = mock.sentinel

    @entity_registry.multipass_match(s.binary_sensor, cluster_handler_names='smartenergy_metering')
    class SmartEnergySensor2:
        pass
    ch_se = cluster_handler('smartenergy_metering', 1794)
    ch_illuminati = cluster_handler('illuminance', 1025)
    (match, claimed) = entity_registry.get_multi_entity('manufacturer', 'model', cluster_handlers=[ch_se, ch_illuminati], quirk_id='quirk_id')
    assert s.binary_sensor in match
    assert s.component not in match
    assert set(claimed) == {ch_se}
    assert {cls.entity_class.__name__ for cls in match[s.binary_sensor]} == {SmartEnergySensor2.__name__}

    @entity_registry.multipass_match(s.component, cluster_handler_names='smartenergy_metering', aux_cluster_handlers='illuminance')
    class SmartEnergySensor1:
        pass

    @entity_registry.multipass_match(s.binary_sensor, cluster_handler_names='smartenergy_metering', aux_cluster_handlers='illuminance')
    ...
    assert {cls.entity_class.__name__ for cls in match[s.binary_sensor]} == {SmartEnergySensor2.__name__, SmartEnergySensor3.__name__}
    assert {cls.entity_class.__name__ for cls in match[s.component]} == {SmartEnergySensor1.__name__}
    ...
    for (_, entities) in iter_all_rules():
        for entity in entities:
            if hasattr(entity, '_attr_name'):
                ...
                assert isinstance(entity._attr_translation_key, str) and entity._attr_translation_key
