...
from  import setup as setup_quirks
from  import CONF_DEVICE_PATH
from  import NetworkSettingsInconsistent
from  import ConfigEntry
from  import CONF_TYPE, EVENT_HOMEASSISTANT_STOP
from  import Event, HomeAssistant
from  import ConfigEntryNotReady, HomeAssistantError
...
from  import BAUD_RATES, CONF_BAUDRATE, CONF_CUSTOM_QUIRKS_PATH, CONF_DEVICE_CONFIG, CONF_ENABLE_QUIRKS, CONF_RADIO_TYPE, CONF_USB_PATH, CONF_ZIGPY, DATA_ZHA, DOMAIN, PLATFORMS, SIGNAL_ADD_ENTITIES, RadioType
from  import get_device_automation_triggers
from  import GROUP_PROBE
from  import ZHAData, get_zha_data
from  import ZhaRadioManager
from  import warn_on_inconsistent_network_settings
from  import AlreadyRunningEZSP, warn_on_wrong_silabs_firmware
DEVICE_CONFIG_SCHEMA_ENTRY = vol.Schema({vol.Optional(CONF_TYPE): cv.string})
ZHA_CONFIG_SCHEMA = {vol.Optional(CONF_BAUDRATE): cv.positive_int, vol.Optional(CONF_DATABASE): cv.string, vol.Optional(CONF_DEVICE_CONFIG, default={}): vol.Schema({cv.string: DEVICE_CONFIG_SCHEMA_ENTRY}), vol.Optional(CONF_ENABLE_QUIRKS, default=True): cv.boolean, vol.Optional(CONF_ZIGPY): dict, vol.Optional(CONF_RADIO_TYPE): cv.enum(RadioType), vol.Optional(CONF_USB_PATH): cv.string, vol.Optional(CONF_CUSTOM_QUIRKS_PATH): cv.isdir}
CONFIG_SCHEMA = vol.Schema({DOMAIN: vol.Schema(vol.All(cv.deprecated(CONF_USB_PATH), cv.deprecated(CONF_BAUDRATE), cv.deprecated(CONF_RADIO_TYPE), ZHA_CONFIG_SCHEMA))}, extra=vol.ALLOW_EXTRA)
CENTICELSIUS = 'C-100'
...
def _clean_serial_port_path(path: str) -> str:
    ...
    if path.startswith('socket://'):
        path = path.strip()
    if re.match('^socket://\\[\\d+\\.\\d+\\.\\d+\\.\\d+\\]:\\d+$', path):
...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    if zha_data.yaml_config.get(CONF_ENABLE_QUIRKS, True):
        setup_quirks(custom_quirks_path=zha_data.yaml_config.get(CONF_CUSTOM_QUIRKS_PATH))
    ...
    async with radio_mgr.connect_zigpy_app() as app:
        for dev in app.devices.values():
            dev_entry = device_registry.async_get_device(identifiers={(DOMAIN, str(dev.ieee))}, connections={(dr.CONNECTION_ZIGBEE, str(dev.ieee))})
            if dev_entry is None:
                continue
            zha_data.device_trigger_cache[dev_entry.id] = (str(dev.ieee), get_device_automation_triggers(dev))
    ...
    try:
        ...
        await warn_on_inconsistent_network_settings(hass, config_entry=config_entry, old_state=exc.old_state, new_state=exc.new_state)
        raise HomeAssistantError('Network settings do not match most recent backup') from exc
    except Exception:
        if RadioType[config_entry.data[CONF_RADIO_TYPE]] == RadioType.ezsp:
            try:
                await warn_on_wrong_silabs_firmware(hass, config_entry.data[CONF_DEVICE][CONF_DEVICE_PATH])
            except AlreadyRunningEZSP as exc:
                raise ConfigEntryNotReady from exc
        raise
    repairs.async_delete_blocking_issues(hass)
    device_registry.async_get_or_create(config_entry_id=config_entry.entry_id, connections={(dr.CONNECTION_ZIGBEE, str(zha_gateway.coordinator_ieee))}, identifiers={(DOMAIN, str(zha_gateway.coordinator_ieee))}, name='Zigbee Coordinator', manufacturer='ZHA', model=zha_gateway.radio_description)
    websocket_api.async_load_api(hass)

    async def async_shutdown(_: Event) -> None:
        await zha_gateway.shutdown()
    config_entry.async_on_unload(hass.bus.async_listen_once(EVENT_HOMEASSISTANT_STOP, async_shutdown))
...
async def async_unload_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    await asyncio.gather(*(hass.config_entries.async_forward_entry_unload(config_entry, platform) for platform in PLATFORMS))
    return True
...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    if config_entry.version == 1:
        data = {CONF_RADIO_TYPE: config_entry.data[CONF_RADIO_TYPE], CONF_DEVICE: {CONF_DEVICE_PATH: config_entry.data[CONF_USB_PATH]}}
