...
from  import Any
from  import mock
from  import AsyncMock, Mock, patch
import pytest
from  import SIG_NODE_DESC
...
from  import DEV_SIG_ATTRIBUTES, DEV_SIG_CLUSTER_HANDLERS, DEV_SIG_ENT_MAP, DEV_SIG_ENT_MAP_CLASS, DEV_SIG_ENT_MAP_ID, DEV_SIG_EVT_CLUSTER_HANDLERS, DEVICES
NO_TAIL_ID = re.compile('_\\d$')
UNIQUE_ID_HD = re.compile('^(([\\da-fA-F]{2}:){7}[\\da-fA-F]{2}-\\d{1,3})', re.X)
IGNORE_SUFFIXES = [zigpy.zcl.clusters.general.OnOff.StartUpOnOff.__name__, 'on_off_transition_time', 'on_level', 'on_transition_time', 'off_transition_time', 'default_move_rate', 'start_up_current_level']
...
@patch('zigpy.zcl.clusters.general.Identify.request', new=AsyncMock(return_value=[mock.sentinel.data, zcl_f.Status.SUCCESS]))
@patch('homeassistant.components.zha.entity.ZhaEntity.entity_registry_enabled_default', new=Mock(return_value=True))
@pytest.mark.parametrize('device', DEVICES)
async def test_devices(device, hass_disable_services, zigpy_device_mock, zha_device_joined_restored) -> None:
    """Test device discovery."""
    entity_registry = er.async_get(hass_disable_services)
    zigpy_device = zigpy_device_mock(endpoints=device[SIG_ENDPOINTS], ieee='00:11:22:33:44:55:66:77', manufacturer=device[SIG_MANUFACTURER], model=device[SIG_MODEL], node_descriptor=device[SIG_NODE_DESC], attributes=device.get(DEV_SIG_ATTRIBUTES), patch_cluster=False)
    ...
    if cluster_identify:
        should_identify = zha_device_joined_restored.name == 'zha_device_joined' and (not zigpy_device.skip_configuration)
        if should_identify:
            assert cluster_identify.request.mock_calls == [mock.call(False, cluster_identify.commands_by_name['trigger_effect'].id, cluster_identify.commands_by_name['trigger_effect'].schema, effect_id=zigpy.zcl.clusters.general.Identify.EffectIdentifier.Okay, effect_variant=zigpy.zcl.clusters.general.Identify.EffectVariant.Default, expect_reply=True, manufacturer=None, tsn=None)]
        else:
            assert cluster_identify.request.mock_calls == []
    event_cluster_handlers = {ch.id for endpoint in zha_dev._endpoints.values() for ch in endpoint.client_cluster_handlers.values()}
    ...
    for call in _dispatch.call_args_list:
        ...
        if response and (not contains_ignored_suffix(response.unique_id)):
            ...
            unique_id_head = UNIQUE_ID_HD.match(unique_id).group(0)
            ha_ent_info[unique_id_head, entity_cls.__name__] = (platform, unique_id, cluster_handlers)
    for (comp_id, ent_info) in device[DEV_SIG_ENT_MAP].items():
        ...
        (ha_comp, ha_unique_id, ha_cluster_handlers) = ha_ent_info[test_unique_id_head, test_ent_class]
        assert platform is ha_comp.value
        assert unique_id.startswith(ha_unique_id)
        assert {ch.name for ch in ha_cluster_handlers} == set(ent_info[DEV_SIG_CLUSTER_HANDLERS])
    assert created_entity_count == len(device[DEV_SIG_ENT_MAP])
    entity_ids = hass_disable_services.states.async_entity_ids()
    await hass_disable_services.async_block_till_done()
    zha_entity_ids = {ent for ent in entity_ids if not contains_ignored_suffix(ent) and ent.split('.')[0] in zha_const.PLATFORMS}
    assert zha_entity_ids == {e[DEV_SIG_ENT_MAP_ID] for e in device[DEV_SIG_ENT_MAP].values()}
...
@mock.patch('homeassistant.components.zha.core.discovery.ProbeEndpoint.discover_by_device_type')
@mock.patch('homeassistant.components.zha.core.discovery.ProbeEndpoint.discover_by_cluster_id')
...
@pytest.mark.parametrize(('device_type', 'platform', 'hit'), [(zigpy.profiles.zha.DeviceType.ON_OFF_LIGHT, Platform.LIGHT, True), (zigpy.profiles.zha.DeviceType.ON_OFF_BALLAST, Platform.SWITCH, True), (zigpy.profiles.zha.DeviceType.SMART_PLUG, Platform.SWITCH, True), (65535, None, False)])
def test_discover_by_device_type(device_type, platform, hit) -> None:
    ...
    ep_mock.return_value.profile_id = 260
    ep_mock.return_value.device_type = device_type
    type(endpoint).zigpy_endpoint = ep_mock
    get_entity_mock = mock.MagicMock(return_value=(mock.sentinel.entity_cls, mock.sentinel.claimed))
...
def test_discover_by_device_type_override() -> None:
    ...
    ep_mock = mock.PropertyMock()
    ep_mock.return_value.profile_id = 260
    ep_mock.return_value.device_type = 256
    type(endpoint).zigpy_endpoint = ep_mock
    overrides = {endpoint.unique_id: {'type': Platform.SWITCH}}
    get_entity_mock = mock.MagicMock(return_value=(mock.sentinel.entity_cls, mock.sentinel.claimed))
    with mock.patch('homeassistant.components.zha.core.registries.ZHA_ENTITIES.get_entity', get_entity_mock), mock.patch.dict(disc.PROBE._device_configs, overrides, clear=True):
        disc.PROBE.discover_by_device_type(endpoint)
        assert get_entity_mock.call_count == 1
        assert endpoint.claim_cluster_handlers.call_count == 1
        assert endpoint.claim_cluster_handlers.call_args[0][0] is mock.sentinel.claimed
...
async def test_discover_endpoint(device_info: dict[str, Any], zha_device_mock: Callable[..., ZHADevice], hass: HomeAssistant) -> None:
    """Test device discovery."""
    with mock.patch('homeassistant.components.zha.core.endpoint.Endpoint.async_new_entity') as new_ent:
        device = zha_device_mock(device_info[SIG_ENDPOINTS], manufacturer=device_info[SIG_MANUFACTURER], model=device_info[SIG_MODEL], node_desc=device_info[SIG_NODE_DESC], patch_cluster=True)
    assert device_info[DEV_SIG_EVT_CLUSTER_HANDLERS] == sorted((ch.id for endpoint in device._endpoints.values() for ch in endpoint.client_cluster_handlers.values()))
    ha_ent_info = {}
    ...
    for (platform_id, ent_info) in device_info[DEV_SIG_ENT_MAP].items():
        ...
        test_unique_id_head = UNIQUE_ID_HD.match(unique_id).group(0)
        assert (test_unique_id_head, test_ent_class) in ha_ent_info
        (entity_platform, entity_unique_id, entity_cluster_handlers) = ha_ent_info[test_unique_id_head, test_ent_class]
        assert platform is entity_platform.value
        assert unique_id.startswith(entity_unique_id)
        assert {ch.name for ch in entity_cluster_handlers} == set(ent_info[DEV_SIG_CLUSTER_HANDLERS])
...
def _ch_mock(cluster):
    ...
    type(cluster_handler).cluster = mock.PropertyMock(return_value=cluster(mock.MagicMock()))
    return cluster_handler

@mock.patch('homeassistant.components.zha.core.discovery.ProbeEndpoint.handle_on_off_output_cluster_exception', new=mock.MagicMock())
@mock.patch('homeassistant.components.zha.core.discovery.ProbeEndpoint.probe_single_cluster')
def _test_single_input_cluster_device_class(probe_mock):
    ...
    endpoint = mock.MagicMock(spec_set=Endpoint)
    endpoint.unclaimed_cluster_handlers.return_value = [door_ch, cover_ch, multistate_ch, ias_ch]
    disc.ProbeEndpoint().discover_by_cluster_id(endpoint)
    assert probe_mock.call_count == len(endpoint.unclaimed_cluster_handlers())
    probes = ((Platform.LOCK, door_ch), (Platform.COVER, cover_ch), (Platform.SENSOR, multistate_ch), (Platform.BINARY_SENSOR, ias_ch), (Platform.SENSOR, analog_ch))
...
def test_single_input_cluster_device_class_by_cluster_class() -> None:
    """Test SINGLE_INPUT_CLUSTER_DEVICE_CLASS matching by cluster id or class."""
    mock_reg = {zigpy.zcl.clusters.closures.DoorLock.cluster_id: Platform.LOCK, zigpy.zcl.clusters.closures.WindowCovering.cluster_id: Platform.COVER, zigpy.zcl.clusters.general.AnalogInput: Platform.SENSOR, zigpy.zcl.clusters.general.MultistateInput: Platform.SENSOR, zigpy.zcl.clusters.security.IasZone: Platform.BINARY_SENSOR}
    with mock.patch.dict(zha_regs.SINGLE_INPUT_CLUSTER_DEVICE_CLASS, mock_reg, clear=True):
        _test_single_input_cluster_device_class()

@pytest.mark.parametrize(('override', 'entity_id'), [(None, 'light.manufacturer_model_light'), ('switch', 'switch.manufacturer_model_switch')])
async def test_device_override(hass_disable_services, zigpy_device_mock, setup_zha, override, entity_id) -> None:
    """Test device discovery override."""
    zigpy_device = zigpy_device_mock({1: {SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.COLOR_DIMMABLE_LIGHT, 'endpoint_id': 1, SIG_EP_INPUT: [0, 3, 4, 5, 6, 8, 768, 2821, 64513], SIG_EP_OUTPUT: [25], SIG_EP_PROFILE: 260}}, '00:11:22:33:44:55:66:77', 'manufacturer', 'model', patch_cluster=False)
...
async def test_group_probe_cleanup_called(hass_disable_services, setup_zha, config_entry) -> None:
