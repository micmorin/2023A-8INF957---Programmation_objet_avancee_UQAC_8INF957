...
import re
import time
from  import TYPE_CHECKING, Any, NamedTuple
from  import ControllerApplication
from  import CONF_NWK_VALIDATE_SETTINGS
import 6lowpan.sixlowpan.IP6FieldLenField.addfield
import 6lowpan.sixlowpan.IP6FieldLenField.addfield
from  import TransientConnectionError
import 6lowpan.sixlowpan.IP6FieldLenField.addfield
from  import EUI64
from  import __path__ as HOMEASSISTANT_PATH
from  import LogEntry, _figure_out_source
from  import ConfigEntry
from  import HomeAssistant, callback
from  import ConfigEntryNotReady
from  import device_registry as dr, entity_registry as er
from  import DeviceInfo
from  import async_dispatcher_send
from  import ConfigType
from  import discovery
from  import ATTR_IEEE, ATTR_MANUFACTURER, ATTR_MODEL, ATTR_NWK, ATTR_SIGNATURE, ATTR_TYPE, CONF_RADIO_TYPE, CONF_USE_THREAD, CONF_ZIGPY, DEBUG_COMP_BELLOWS, DEBUG_COMP_ZHA, DEBUG_COMP_ZIGPY, DEBUG_COMP_ZIGPY_DECONZ, DEBUG_COMP_ZIGPY_XBEE, DEBUG_COMP_ZIGPY_ZIGATE, DEBUG_COMP_ZIGPY_ZNP, DEBUG_LEVEL_CURRENT, DEBUG_LEVEL_ORIGINAL, DEBUG_LEVELS, DEBUG_RELAY_LOGGERS, DEFAULT_DATABASE_NAME, DEVICE_PAIRING_STATUS, DOMAIN, SIGNAL_ADD_ENTITIES, SIGNAL_GROUP_MEMBERSHIP_CHANGE, SIGNAL_REMOVE, STARTUP_FAILURE_DELAY_S, STARTUP_RETRIES, UNKNOWN_MANUFACTURER, UNKNOWN_MODEL, ZHA_GW_MSG, ZHA_GW_MSG_DEVICE_FULL_INIT, ZHA_GW_MSG_DEVICE_INFO, ZHA_GW_MSG_DEVICE_JOINED, ZHA_GW_MSG_DEVICE_REMOVED, ZHA_GW_MSG_GROUP_ADDED, ZHA_GW_MSG_GROUP_INFO, ZHA_GW_MSG_GROUP_MEMBER_ADDED, ZHA_GW_MSG_GROUP_MEMBER_REMOVED, ZHA_GW_MSG_GROUP_REMOVED, ZHA_GW_MSG_LOG_ENTRY, ZHA_GW_MSG_LOG_OUTPUT, ZHA_GW_MSG_RAW_INIT, RadioType
from  import DeviceStatus, ZHADevice
from  import GroupMember, ZHAGroup
from  import get_zha_data
from  import GROUP_ENTITY_DOMAINS
if TYPE_CHECKING:
    from  import Filter, LogRecord
    from  import ZhaEntity
...
class ZHAGateway:
    ...
    def __init__(self, hass: HomeAssistant, config: ConfigType, config_entry: ConfigEntry) -> None:
        ...
        self._device_registry: collections.defaultdict[EUI64, list[EntityReference]] = collections.defaultdict(list)
        self._log_levels: dict[str, dict[str, int]] = {DEBUG_LEVEL_ORIGINAL: async_capture_log_levels(), DEBUG_LEVEL_CURRENT: async_capture_log_levels()}
    ...
    def get_application_controller_data(self) -> tuple[ControllerApplication, dict]:
        ...
        database = self._config.get(CONF_DATABASE, self.hass.config.path(DEFAULT_DATABASE_NAME))
        ...
        if CONF_USE_THREAD not in app_config and RadioType[radio_type] is RadioType.ezsp and app_config[CONF_DEVICE][CONF_DEVICE_PATH].startswith('socket://'):
            app_config[CONF_USE_THREAD] = False
        from  import is_multiprotocol_url
        if is_multiprotocol_url(app_config[CONF_DEVICE][CONF_DEVICE_PATH]) and app_config.get(CONF_NWK, {}).get(CONF_NWK_CHANNEL) is None:
    ...
    async def async_initialize(self) -> None:
        ...
        self.application_controller = await app_controller_cls.new(config=app_config, auto_form=False, start_radio=False)
        try:
            for attempt in range(STARTUP_RETRIES):
                try:
                    ...
                    _LOGGER.debug("Couldn't start %s coordinator (attempt %s of %s)", self.radio_description, attempt + 1, STARTUP_RETRIES, exc_info=exc)
        ...
        self.coordinator_zha_device = self._async_get_or_create_device(self._find_coordinator_device(), restored=True)
    ...
    def _find_coordinator_device(self) -> zigpy.device.Device:
        zigpy_coordinator = self.application_controller.get_device(nwk=0)
        if (last_backup := self.application_controller.backups.most_recent_backup()):
            with suppress(KeyError):
                zigpy_coordinator = self.application_controller.get_device(ieee=last_backup.node_info.ieee)
    ...
    def async_load_devices(self) -> None:
        ...
        for zigpy_device in self.application_controller.devices.values():
            ...
            _LOGGER.debug("[%s](%s) restored as '%s', last seen: %s, consider_unavailable_time: %s seconds", zha_device.nwk, zha_device.name, 'available' if zha_device.available else 'unavailable', delta_msg, zha_device.consider_unavailable_time)
    ...
    async def async_initialize_devices_and_entities(self) -> None:
        ...
        await asyncio.gather(*(dev.async_initialize(from_cache=True) for dev in self.devices.values()))
        ...
        async def fetch_updated_state() -> None:
            ...
            await asyncio.gather(*(dev.async_initialize(from_cache=False) for dev in self.devices.values() if dev.is_mains_powered))
        self.config_entry.async_create_background_task(self.hass, fetch_updated_state(), 'zha.gateway-fetch_updated_state')
    ...
    def device_joined(self, device: zigpy.device.Device) -> None:
        ...
        address
        """
        async_dispatcher_send(self.hass, ZHA_GW_MSG, {ATTR_TYPE: ZHA_GW_MSG_DEVICE_JOINED, ZHA_GW_MSG_DEVICE_INFO: {ATTR_NWK: device.nwk, ATTR_IEEE: str(device.ieee), DEVICE_PAIRING_STATUS: DevicePairingStatus.PAIRED.name}})

    def raw_device_initialized(self, device: zigpy.device.Device) -> None:
        """Handle a device initialization without quirks loaded."""
        manuf = device.manufacturer
        async_dispatcher_send(self.hass, ZHA_GW_MSG, {ATTR_TYPE: ZHA_GW_MSG_RAW_INIT, ZHA_GW_MSG_DEVICE_INFO: {ATTR_NWK: device.nwk, ATTR_IEEE: str(device.ieee), DEVICE_PAIRING_STATUS: DevicePairingStatus.INTERVIEW_COMPLETE.name, ATTR_MODEL: device.model if device.model else UNKNOWN_MODEL, ATTR_MANUFACTURER: manuf if manuf else UNKNOWN_MANUFACTURER, ATTR_SIGNATURE: device.get_signature()}})
    ...
    def group_member_removed(self, zigpy_group: zigpy.group.Group, endpoint: zigpy.endpoint.Endpoint) -> None:
        ...
        async_dispatcher_send(self.hass, f'{SIGNAL_GROUP_MEMBERSHIP_CHANGE}_0x{zigpy_group.group_id:04x}')

    def group_member_added(self, zigpy_group: zigpy.group.Group, endpoint: zigpy.endpoint.Endpoint) -> None:
    ...
    def _send_group_gateway_message(self, zigpy_group: zigpy.group.Group, gateway_message_type: str) -> None:
        """Send the gateway event for a zigpy group event."""
        zha_group = self._groups.get(zigpy_group.group_id)
        if zha_group is not None:
            async_dispatcher_send(self.hass, ZHA_GW_MSG, {ATTR_TYPE: gateway_message_type, ZHA_GW_MSG_GROUP_INFO: zha_group.group_info})

    async def _async_remove_device(self, device: ZHADevice, entity_refs: list[EntityReference] | None) -> None:
    ...
    def device_removed(self, device: zigpy.device.Device) -> None:
        ...
        if zha_device is not None:
            ...
            self.hass.async_create_task(self._async_remove_device(zha_device, entity_refs), 'ZHAGateway._async_remove_device')
            if device_info is not None:
                async_dispatcher_send(self.hass, ZHA_GW_MSG, {ATTR_TYPE: ZHA_GW_MSG_DEVICE_REMOVED, ZHA_GW_MSG_DEVICE_INFO: device_info})
    ...
    def get_entity_reference(self, entity_id: str) -> EntityReference | None:
        ...
        for entity_reference in itertools.chain.from_iterable(self.device_registry.values()):
    ...
    def remove_entity_reference(self, entity: ZhaEntity) -> None:
        ...
        if entity.zha_device.ieee in self.device_registry:
            ...
            self.device_registry[entity.zha_device.ieee] = [e for e in entity_refs if e.reference_id != entity.entity_id]

    def _cleanup_group_entity_registry_entries(self, zigpy_group: zigpy.group.Group) -> None:
        """Remove entity registry entries for group entities when the groups are removed from HA."""
        possible_entity_unique_ids = [f'{domain}_zha_group_0x{zigpy_group.group_id:04x}' for domain in GROUP_ENTITY_DOMAINS]
        entity_registry = er.async_get(self.hass)
        assert self.coordinator_zha_device
        all_group_entity_entries = er.async_entries_for_device(entity_registry, self.coordinator_zha_device.device_id, include_disabled_entities=True)
        entries_to_remove = [entry for entry in all_group_entity_entries if entry.unique_id in possible_entity_unique_ids]
        for entry in entries_to_remove:
            _LOGGER.debug('cleaning up entity registry entry for entity: %s', entry.entity_id)
    ...
    def register_entity_reference(self, ieee: EUI64, reference_id: str, zha_device: ZHADevice, cluster_handlers: dict[str, ClusterHandler], device_info: DeviceInfo, remove_future: asyncio.Future[Any]):
        """Record the creation of a hass entity associated with ieee."""
        self._device_registry[ieee].append(EntityReference(reference_id=reference_id, zha_device=zha_device, cluster_handlers=cluster_handlers, device_info=device_info, remove_future=remove_future))
    ...
    @callback
    def _async_get_or_create_device(self, zigpy_device: zigpy.device.Device, restored: bool=False) -> ZHADevice:
        ...
        if (zha_device := self._devices.get(zigpy_device.ieee)) is None:
            ...
            device_registry = dr.async_get(self.hass)
            device_registry_device = device_registry.async_get_or_create(config_entry_id=self.config_entry.entry_id, connections={(dr.CONNECTION_ZIGBEE, str(zha_device.ieee))}, identifiers={(DOMAIN, str(zha_device.ieee))}, name=zha_device.name, manufacturer=zha_device.manufacturer, model=zha_device.model)
    ...
    @callback
    def async_update_device(self, sender: zigpy.device.Device, available: bool=True) -> None:
    ...
    async def async_device_initialized(self, device: zigpy.device.Device) -> None:
        ...
        _LOGGER.debug('device - %s:%s entering async_device_initialized - is_new_join: %s', device.nwk, device.ieee, zha_device.status is not DeviceStatus.INITIALIZED)
        if zha_device.status is DeviceStatus.INITIALIZED:
            _LOGGER.debug('device - %s:%s has been reset and re-added or its nwk address changed', device.nwk, device.ieee)
            await self._async_device_rejoined(zha_device)
        else:
            _LOGGER.debug('device - %s:%s has joined the ZHA zigbee network', device.nwk, device.ieee)
            await self._async_device_joined(zha_device)
        device_info = zha_device.zha_device_info
        device_info[DEVICE_PAIRING_STATUS] = DevicePairingStatus.INITIALIZED.name
        async_dispatcher_send(self.hass, ZHA_GW_MSG, {ATTR_TYPE: ZHA_GW_MSG_DEVICE_FULL_INIT, ZHA_GW_MSG_DEVICE_INFO: device_info})
    ...
    async def _async_device_rejoined(self, zha_device: ZHADevice) -> None:
        _LOGGER.debug('skipping discovery for previously discovered device - %s:%s', zha_device.nwk, zha_device.ieee)
        await zha_device.async_configure()
        device_info = zha_device.device_info
        device_info[DEVICE_PAIRING_STATUS] = DevicePairingStatus.CONFIGURED.name
        async_dispatcher_send(self.hass, ZHA_GW_MSG, {ATTR_TYPE: ZHA_GW_MSG_DEVICE_FULL_INIT, ZHA_GW_MSG_DEVICE_INFO: device_info})
    ...
    async def async_create_zigpy_group(self, name: str, members: list[GroupMember] | None, group_id: int | None=None) -> ZHAGroup | None:
        ...
        if self.async_get_group_by_name(name) is None:
            ...
            if members is not None:
                ...
                for member in members:
                    _LOGGER.debug('Adding member with IEEE: %s and endpoint ID: %s to group: %s:0x%04x', member.ieee, member.endpoint_id, name, group_id)
                    tasks.append(self.devices[member.ieee].async_add_endpoint_to_group(member.endpoint_id, group_id))
    ...
    async def shutdown(self) -> None:
        ...
        if hasattr(self, 'application_controller') and self.application_controller is not None:
            await self.application_controller.shutdown()

    def handle_message(self, sender: zigpy.device.Device, profile: int, cluster: int, src_ep: int, dst_ep: int, message: bytes) -> None:
...
def async_capture_log_levels() -> dict[str, int]:
    ...
    return {DEBUG_COMP_BELLOWS: logging.getLogger(DEBUG_COMP_BELLOWS).getEffectiveLevel(), DEBUG_COMP_ZHA: logging.getLogger(DEBUG_COMP_ZHA).getEffectiveLevel(), DEBUG_COMP_ZIGPY: logging.getLogger(DEBUG_COMP_ZIGPY).getEffectiveLevel(), DEBUG_COMP_ZIGPY_ZNP: logging.getLogger(DEBUG_COMP_ZIGPY_ZNP).getEffectiveLevel(), DEBUG_COMP_ZIGPY_DECONZ: logging.getLogger(DEBUG_COMP_ZIGPY_DECONZ).getEffectiveLevel(), DEBUG_COMP_ZIGPY_XBEE: logging.getLogger(DEBUG_COMP_ZIGPY_XBEE).getEffectiveLevel(), DEBUG_COMP_ZIGPY_ZIGATE: logging.getLogger(DEBUG_COMP_ZIGPY_ZIGATE).getEffectiveLevel()}
...
class LogRelayHandler(logging.Handler):
    ...
    def __init__(self, hass: HomeAssistant, gateway: ZHAGateway) -> None:
        ...
        self.paths_re = re.compile('(?:{})/(.*)'.format('|'.join([re.escape(x) for x in (hass_path, config_dir)])))
    ...
    def emit(self, record: LogRecord) -> None:
        ...
        async_dispatcher_send(self.hass, ZHA_GW_MSG, {ATTR_TYPE: ZHA_GW_MSG_LOG_OUTPUT, ZHA_GW_MSG_LOG_ENTRY: entry.to_dict()})