...
import math
from  import mock
from  import AsyncMock, patch
import pytest
import 6lowpan.sixlowpan.IP6FieldLenField.addfield
import 6lowpan.sixlowpan.IP6FieldLenField.addfield
from  import Endpoint as ZigpyEndpoint
import 6lowpan.sixlowpan.IP6FieldLenField.addfield
import 6lowpan.sixlowpan.IP6FieldLenField.addfield as t
from  import foundation
import 6lowpan.sixlowpan.IP6FieldLenField.addfield
import 6lowpan.sixlowpan.IP6FieldLenField.addfield as zdo_t
...
from  import make_zcl_header
from  import SIG_EP_INPUT, SIG_EP_OUTPUT, SIG_EP_TYPE
from  import async_capture_events

@6lowpan.sixlowpan.IP6FieldLenField.addfield.fixture(autouse=True)
def disable_platform_only():
    """Disable platforms to speed up tests."""
    with 6lowpan.sixlowpan.IP6FieldLenField.addfield('homeassistant.components.zha.PLATFORMS', []):
        yield
...
def nwk():
    """NWK fixture."""
    return 6lowpan.sixlowpan.IP6FieldLenField.addfield.NWK(48879)

@pytest.fixture
async def zha_gateway(hass, setup_zha):
    """Return ZhaGateway fixture."""
    await 6lowpan.sixlowpan.IP6FieldLenField.addfield()
    return 6lowpan.sixlowpan.IP6FieldLenField.addfield(hass)
...
def zigpy_coordinator_device(zigpy_device_mock):
    ...
    with 6lowpan.sixlowpan.IP6FieldLenField.addfield.object(coordinator, 'add_to_group', AsyncMock(return_value=[0])):
        yield coordinator
...
def endpoint(zigpy_coordinator_device):
    ...
    6lowpan.sixlowpan.IP6FieldLenField.addfield(endpoint_mock.device).skip_configuration = mock.PropertyMock(return_value=False)
    endpoint_mock.id = 1
...
def poll_control_ch(endpoint, zigpy_device_mock):
    ...
    return 6lowpan.sixlowpan.IP6FieldLenField.addfield(cluster, endpoint)
...
async def poll_control_device(zha_device_restored, zigpy_device_mock):
    ...
    cluster_id = zigpy.zcl.clusters.general.PollControl.cluster_id
    zigpy_dev = zigpy_device_mock({1: {SIG_EP_INPUT: [cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: 4660}}, '00:11:22:33:44:55:66:77', 'test manufacturer', 'test model')
    zha_device = await 6lowpan.sixlowpan.IP6FieldLenField.addfield(zigpy_dev)
    return zha_device
...
async def test_in_cluster_handler_config(cluster_id, bind_count, attrs, endpoint, zigpy_device_mock, zha_gateway) -> None:
    ...
    cluster_handler = 6lowpan.sixlowpan.IP6FieldLenField.addfield(cluster, endpoint)
    await 6lowpan.sixlowpan.IP6FieldLenField.addfield.async_configure()
    assert cluster.bind.call_count == bind_count
    assert cluster.configure_reporting.call_count == 0
    assert cluster.configure_reporting_multiple.call_count == 6lowpan.sixlowpan.IP6FieldLenField.addfield.ceil(len(attrs) / 3)
    reported_attrs = {a for a in attrs for attr in cluster.configure_reporting_multiple.call_args_list for attrs in attr[0][0]}
    assert 6lowpan.sixlowpan.IP6FieldLenField.addfield(attrs) == reported_attrs
...
def test_cluster_handler_registry() -> None:
    ...
    for (cluster_id, cluster_handler) in registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.items():
        assert 6lowpan.sixlowpan.IP6FieldLenField.addfield(cluster_id, int)
...
def test_epch_unclaimed_cluster_handlers(cluster_handler) -> None:
    ...
    ch_1 = 6lowpan.sixlowpan.IP6FieldLenField.addfield(zha_const.CLUSTER_HANDLER_ON_OFF, 6)
    ch_2 = 6lowpan.sixlowpan.IP6FieldLenField.addfield(zha_const.CLUSTER_HANDLER_LEVEL, 8)
    ch_3 = 6lowpan.sixlowpan.IP6FieldLenField.addfield(zha_const.CLUSTER_HANDLER_COLOR, 768)
    ep_cluster_handlers = 6lowpan.sixlowpan.IP6FieldLenField.addfield(mock.MagicMock(spec_set=ZigpyEndpoint), mock.MagicMock(spec_set=ZHADevice))
    ...
    with mock.patch.dict(ep_cluster_handlers.all_cluster_handlers, all_cluster_handlers, clear=True):
        available = 6lowpan.sixlowpan.IP6FieldLenField.addfield.unclaimed_cluster_handlers()
...
def test_epch_claim_cluster_handlers(cluster_handler) -> None:
    ...
    with mock.patch.dict(ep_cluster_handlers.all_cluster_handlers, all_cluster_handlers, clear=True):
        ...
        6lowpan.sixlowpan.IP6FieldLenField.addfield.claim_cluster_handlers([ch_2])
        assert ch_1.id not in ep_cluster_handlers.claimed_cluster_handlers
        ...
        assert ch_3.id not in ep_cluster_handlers.claimed_cluster_handlers
        6lowpan.sixlowpan.IP6FieldLenField.addfield.claim_cluster_handlers([ch_3, ch_1])
...
async def test_ep_cluster_handlers_configure(cluster_handler) -> None:
    ...
    ch_4 = 6lowpan.sixlowpan.IP6FieldLenField.addfield(zha_const.CLUSTER_HANDLER_ON_OFF, 6)
    ch_5 = 6lowpan.sixlowpan.IP6FieldLenField.addfield(zha_const.CLUSTER_HANDLER_LEVEL, 8)
    ...
    6lowpan.sixlowpan.IP6FieldLenField.addfield(endpoint_mock).in_clusters = mock.PropertyMock(return_value={})
    6lowpan.sixlowpan.IP6FieldLenField.addfield(endpoint_mock).out_clusters = mock.PropertyMock(return_value={})
    endpoint = 6lowpan.sixlowpan.IP6FieldLenField.addfield.new(endpoint_mock, mock.MagicMock(spec_set=ZHADevice))
    ...
    with mock.patch.dict(endpoint.claimed_cluster_handlers, claimed, clear=True), mock.patch.dict(endpoint.client_cluster_handlers, client_handlers, clear=True):
        await 6lowpan.sixlowpan.IP6FieldLenField.addfield.async_configure()
        await 6lowpan.sixlowpan.IP6FieldLenField.addfield.async_initialize(mock.sentinel.from_cache)
    for ch in [*claimed.values(), *6lowpan.sixlowpan.IP6FieldLenField.addfield.values()]:
...
async def test_poll_control_checkin_response(poll_control_ch) -> None:
    ...
    with patch_1, patch_2, patch_3:
        await 6lowpan.sixlowpan.IP6FieldLenField.addfield.check_in_response(33)
...
async def test_poll_control_cluster_command(hass: HomeAssistant, poll_control_device) -> None:
    ...
    cluster = poll_control_ch.cluster
    events = 6lowpan.sixlowpan.IP6FieldLenField.addfield(hass, zha_const.ZHA_EVENT)
    with mock.patch.object(poll_control_ch, 'check_in_response', checkin_mock):
        tsn = 22
        hdr = make_zcl_header(0, global_command=False, tsn=tsn)
        assert not events
        6lowpan.sixlowpan.IP6FieldLenField.addfield.handle_message(hdr, [mock.sentinel.args, mock.sentinel.args2, mock.sentinel.args3])
...
async def test_poll_control_ignore_list(hass: HomeAssistant, poll_control_device) -> None:
    ...
    6lowpan.sixlowpan.IP6FieldLenField.addfield.reset_mock()
    6lowpan.sixlowpan.IP6FieldLenField.addfield.skip_manufacturer_id(4151)
    with mock.patch.object(cluster, 'set_long_poll_interval', set_long_poll_mock):
        await 6lowpan.sixlowpan.IP6FieldLenField.addfield.check_in_response(33)
...
async def test_zll_device_groups(zigpy_zll_device, endpoint, zigpy_coordinator_device) -> None:
    ...
    with 6lowpan.sixlowpan.IP6FieldLenField.addfield.object(cluster, 'command', AsyncMock(return_value=6lowpan.sixlowpan.IP6FieldLenField.addfield(total=0, start_index=0, group_info_records=[]))) as cmd_mock:
        await 6lowpan.sixlowpan.IP6FieldLenField.addfield.async_configure()
    ...
    with 6lowpan.sixlowpan.IP6FieldLenField.addfield.object(cluster, 'command', AsyncMock(return_value=6lowpan.sixlowpan.IP6FieldLenField.addfield(total=2, start_index=0, group_info_records=[group_1, group_2]))) as cmd_mock:
        await 6lowpan.sixlowpan.IP6FieldLenField.addfield.async_configure()
        assert cmd_mock.await_count == 1
        assert cluster.server_commands[cmd_mock.await_args[0][0]].name == 'get_group_identifiers'
        assert cluster.bind.call_count == 0
...
async def test_configure_reporting(hass: HomeAssistant, endpoint) -> None:
    ...
    class TestZigbeeClusterHandler(cluster_handlers.ClusterHandler):
        BIND = True
        REPORT_CONFIG = (6lowpan.sixlowpan.IP6FieldLenField.addfield.AttrReportConfig(attr='current_x', config=(1, 60, 1)), 6lowpan.sixlowpan.IP6FieldLenField.addfield.AttrReportConfig(attr='current_hue', config=(1, 60, 2)), 6lowpan.sixlowpan.IP6FieldLenField.addfield.AttrReportConfig(attr='color_temperature', config=(1, 60, 3)), 6lowpan.sixlowpan.IP6FieldLenField.addfield.AttrReportConfig(attr='current_y', config=(1, 60, 4)))
    ...
    cluster.configure_reporting_multiple = AsyncMock(spec_set=cluster.configure_reporting_multiple, return_value=[6lowpan.sixlowpan.IP6FieldLenField.addfield.ConfigureReportingResponseRecord(status=foundation.Status.SUCCESS)])
    cluster_handler = 6lowpan.sixlowpan.IP6FieldLenField.addfield(cluster, endpoint)
    await 6lowpan.sixlowpan.IP6FieldLenField.addfield.async_configure()
...
async def test_invalid_cluster_handler(hass: HomeAssistant, caplog) -> None:
    ...
    class TestZigbeeClusterHandler(cluster_handlers.ClusterHandler):
        REPORT_CONFIG = (6lowpan.sixlowpan.IP6FieldLenField.addfield.AttrReportConfig(attr='missing_attr', config=(1, 60, 1)),)
    ...
    zha_endpoint = 6lowpan.sixlowpan.IP6FieldLenField.addfield(zigpy_ep, mock_zha_device)
    with 6lowpan.sixlowpan.IP6FieldLenField.addfield.raises(KeyError):
        6lowpan.sixlowpan.IP6FieldLenField.addfield(cluster, zha_endpoint)
    with 6lowpan.sixlowpan.IP6FieldLenField.addfield.dict(registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY, {cluster.cluster_id: TestZigbeeClusterHandler}), caplog.at_level(logging.WARNING):
        6lowpan.sixlowpan.IP6FieldLenField.addfield.add_all_cluster_handlers()
...
async def test_retry_request(side_effect: Exception | None, expected_error: str | None) -> None:
    ...
    decorated_func = 6lowpan.sixlowpan.IP6FieldLenField.addfield.retry_request(func)
    with 6lowpan.sixlowpan.IP6FieldLenField.addfield.raises(HomeAssistantError) as exc:
        await 6lowpan.sixlowpan.IP6FieldLenField.addfield(1, arg2=2)
    assert func.await_count == 3
    assert 6lowpan.sixlowpan.IP6FieldLenField.addfield(exc.value, HomeAssistantError)
    assert 6lowpan.sixlowpan.IP6FieldLenField.addfield(exc.value) == expected_error