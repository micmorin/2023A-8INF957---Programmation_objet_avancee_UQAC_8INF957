...
from  import STATE_ON, STATE_UNAVAILABLE, EntityCategory, Platform
from  import HomeAssistant, State, callback
from  import async_dispatcher_connect
from  import AddEntitiesCallback
from  import discovery
from  import CLUSTER_HANDLER_BASIC, CLUSTER_HANDLER_INOVELLI, CLUSTER_HANDLER_ON_OFF, SIGNAL_ADD_ENTITIES, SIGNAL_ATTR_UPDATED
...
CONFIG_DIAGNOSTIC_MATCH = functools.partial(ZHA_ENTITIES.config_diagnostic_match, Platform.SWITCH)
...
class ZHASwitchConfigurationEntity(ZhaEntity, SwitchEntity):
    ...
    def create_entity(cls, unique_id: str, zha_device: ZHADevice, cluster_handlers: list[ClusterHandler], **kwargs: Any) -> Self | None:
        ...
        if cls._attribute_name in cluster_handler.cluster.unsupported_attributes or cls._attribute_name not in cluster_handler.cluster.attributes_by_name or cluster_handler.cluster.get(cls._attribute_name) is None:
            _LOGGER.debug('%s is not supported - skipping %s entity creation', cls._attribute_name, cls.__name__)
            return None
    ...
    async def async_turn_on_off(self, state: bool) -> None:
        ...
        await self._cluster_handler.write_attributes_safe({self._attribute_name: not state if self.inverted else state})
        self.async_write_ha_state()
    ...
    async def async_update(self) -> None:
        ...
        if self._cluster_handler:
            value = await self._cluster_handler.get_attribute_value(self._attribute_name, from_cache=False)
            await self._cluster_handler.get_attribute_value(self._inverter_attribute_name, from_cache=False)
            self.debug('read value=%s, inverted=%s', value, self.inverted)

@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names='tuya_manufacturer', manufacturers={'_TZE200_b6wax7g0'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names='opple_cluster', models={'lumi.motion.ac02'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names='opple_cluster', models={'lumi.plug.mmeu01', 'lumi.plug.maeu01'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names=CLUSTER_HANDLER_BASIC, manufacturers={'Philips', 'Signify Netherlands B.V.'}, models={'SML001', 'SML002', 'SML003', 'SML004'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names='ikea_airpurifier', models={'STARKVIND Air purifier', 'STARKVIND Air purifier table'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names='opple_cluster', models={'aqara.feeder.acn001'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names=CLUSTER_HANDLER_ON_OFF, models={'TS011F'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names='opple_cluster', models={'lumi.airrtc.agl001'})
...
@CONFIG_DIAGNOSTIC_MATCH(cluster_handler_names='opple_cluster', models={'lumi.sensor_smoke.acn03'})
