...
from  import Status
from  import STATE_LOCKED, STATE_UNLOCKED, LockEntity
from  import ConfigEntry
from  import Platform
from  import HomeAssistant, callback
from  import config_validation as cv
from  import async_dispatcher_connect
from  import AddEntitiesCallback, async_get_current_platform
from  import StateType
from  import discovery
from  import CLUSTER_HANDLER_DOORLOCK, SIGNAL_ADD_ENTITIES, SIGNAL_ATTR_UPDATED
...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    platform.async_register_entity_service(SERVICE_SET_LOCK_USER_CODE, {vol.Required('code_slot'): vol.Coerce(int), vol.Required('user_code'): cv.string}, 'async_set_lock_user_code')
    platform.async_register_entity_service(SERVICE_ENABLE_LOCK_USER_CODE, {vol.Required('code_slot'): vol.Coerce(int)}, 'async_enable_lock_user_code')
    platform.async_register_entity_service(SERVICE_DISABLE_LOCK_USER_CODE, {vol.Required('code_slot'): vol.Coerce(int)}, 'async_disable_lock_user_code')
    platform.async_register_entity_service(SERVICE_CLEAR_LOCK_USER_CODE, {vol.Required('code_slot'): vol.Coerce(int)}, 'async_clear_lock_user_code')
...
class ZhaDoorLock(ZhaEntity, LockEntity):
    ...
    def __init__(self, unique_id, zha_device, cluster_handlers, **kwargs):
        ...
        self._doorlock_cluster_handler = self.cluster_handlers.get(CLUSTER_HANDLER_DOORLOCK)

    async def async_added_to_hass(self) -> None:
        """Run when about to be added to hass."""
        await super().async_added_to_hass()
        self.async_accept_signal(self._doorlock_cluster_handler, SIGNAL_ATTR_UPDATED, self.async_set_state)
    ...
    async def async_get_state(self, from_cache=True):
        ...
        if self._doorlock_cluster_handler:
            state = await self._doorlock_cluster_handler.get_attribute_value('lock_state', from_cache=from_cache)
    ...
    async def async_set_lock_user_code(self, code_slot: int, user_code: str) -> None:
        ...
        if self._doorlock_cluster_handler:
            await self._doorlock_cluster_handler.async_set_user_code(code_slot, user_code)
