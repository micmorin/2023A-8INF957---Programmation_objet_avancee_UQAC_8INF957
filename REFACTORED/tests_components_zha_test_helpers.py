...
from  import cluster_command_schema_to_vol_schema, convert_to_zcl_values
...
def light_platform_only():
    ...
    with patch('homeassistant.components.zha.PLATFORMS', (Platform.BUTTON, Platform.LIGHT, Platform.NUMBER, Platform.SELECT)):
        yield

@pytest.fixture
async def device_light(hass, zigpy_device_mock, zha_device_joined):
    """Test light."""
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [general.OnOff.cluster_id, general.LevelControl.cluster_id, lighting.Color.cluster_id, general.Groups.cluster_id, general.Identify.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zha.DeviceType.COLOR_DIMMABLE_LIGHT, SIG_EP_PROFILE: zha.PROFILE_ID}})
...
async def test_zcl_schema_conversions(hass: HomeAssistant, device_light) -> None:
    ...
    raw_data = {'update_flags': [1, 8], 'action': 2, 'direction': 1, 'time': 20, 'start_hue': 196}
    converted_data = convert_to_zcl_values(raw_data, command_schema)
    assert isinstance(converted_data['update_flags'], lighting.Color.ColorLoopUpdateFlags)
    assert lighting.Color.ColorLoopUpdateFlags.Action in converted_data['update_flags']
    assert lighting.Color.ColorLoopUpdateFlags.Start_Hue in converted_data['update_flags']
    ...
    assert isinstance(converted_data['start_hue'], uint16_t)
    assert converted_data['start_hue'] == 196
    raw_data = {'update_flags': [], 'action': 2, 'direction': 1, 'time': 20, 'start_hue': 196}
    converted_data = convert_to_zcl_values(raw_data, command_schema)
