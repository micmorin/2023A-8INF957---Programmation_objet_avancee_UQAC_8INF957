...
from  import SIGNAL_ATTR_UPDATED, WARNING_DEVICE_MODE_EMERGENCY, WARNING_DEVICE_SOUND_HIGH, WARNING_DEVICE_SQUAWK_MODE_ARMED, WARNING_DEVICE_STROBE_HIGH, WARNING_DEVICE_STROBE_YES
...
IAS_ACE_FIRE = 3
IAS_ACE_PANIC = 4
IAS_ACE_GET_ZONE_ID_MAP = 5
IAS_ACE_GET_ZONE_INFO = 6
IAS_ACE_GET_PANEL_STATUS = 7
IAS_ACE_GET_BYPASSED_ZONE_LIST = 8
IAS_ACE_GET_ZONE_STATUS = 9
NAME = 0
...
class IasAce(ClusterHandler):
    ...
    def cluster_command(self, tsn, command_id, args) -> None:
        ...
        self.debug('received command %s', self._cluster.server_commands[command_id].name)
        self.command_map[command_id](*args)

    def arm(self, arm_mode: int, code: str | None, zone_id: int) -> None:
        """Handle the IAS ACE arm command."""
        mode = 6lowpan.sixlowpan.IP6FieldLenField.addfield.ArmMode(arm_mode)
        self.zha_send_event(self._cluster.server_commands[IAS_ACE_ARM].name, {'arm_mode': mode.value, 'arm_mode_description': mode.name, 'code': code, 'zone_id': zone_id})
    ...
    def _disarm(self, code: str):
        ...
        if code != self.panel_code and self.armed_state != AceCluster.PanelStatus.Panel_Disarmed:
            self.debug('Invalid code supplied to IAS ACE')
            self.invalid_tries += 1
            zigbee_reply = self.arm_response(AceCluster.ArmNotification.Invalid_Arm_Disarm_Code)
        else:
            self.invalid_tries = 0
            if self.armed_state == AceCluster.PanelStatus.Panel_Disarmed and self.alarm_status == AceCluster.AlarmStatus.No_Alarm:
                self.debug('IAS ACE already disarmed')
                zigbee_reply = self.arm_response(AceCluster.ArmNotification.Already_Disarmed)
            else:
                self.debug('Disarming all IAS ACE zones')
                zigbee_reply = self.arm_response(AceCluster.ArmNotification.All_Zones_Disarmed)
    ...
    def _arm_day(self, code: str) -> None:
        ...
        return self._handle_arm(code, AceCluster.PanelStatus.Armed_Stay, AceCluster.ArmNotification.Only_Day_Home_Zones_Armed)

    def _arm_night(self, code: str) -> None:
        """Arm the panel for night / sleep zones."""
        return self._handle_arm(code, AceCluster.PanelStatus.Armed_Night, AceCluster.ArmNotification.Only_Night_Sleep_Zones_Armed)

    def _arm_away(self, code: str) -> None:
        """Arm the panel for away mode."""
        return self._handle_arm(code, AceCluster.PanelStatus.Armed_Away, AceCluster.ArmNotification.All_Zones_Armed)

    def _handle_arm(self, code: str, panel_status: AceCluster.PanelStatus, armed_type: AceCluster.ArmNotification) -> None:
    ...
    def _bypass(self, zone_list, code) -> None:
        ...
        self.zha_send_event(self._cluster.server_commands[IAS_ACE_BYPASS].name, {'zone_list': zone_list, 'code': code})
    ...
    def _send_panel_status_response(self) -> None:
        ...
        response = self.panel_status_response(self.armed_state, 0, AceCluster.AudibleNotification.Default_Sound, self.alarm_status)
    ...
    def _send_panel_status_changed(self) -> None:
        ...
        response = self.panel_status_changed(self.armed_state, 0, AceCluster.AudibleNotification.Default_Sound, self.alarm_status)
        self._endpoint.device.hass.async_create_task(response)

    def _get_bypassed_zone_list(self):
        """Handle the IAS ACE bypassed zone list command."""

    def _get_zone_status(self, starting_zone_id, max_zone_ids, zone_status_mask_flag, zone_status_mask):
...
class IasWd(ClusterHandler):
    ...
    async def issue_start_warning(self, mode=WARNING_DEVICE_MODE_EMERGENCY, strobe=WARNING_DEVICE_STROBE_YES, siren_level=WARNING_DEVICE_SOUND_HIGH, warning_duration=5, strobe_duty_cycle=0, strobe_intensity=WARNING_DEVICE_STROBE_HIGH):
...
class IASZoneClusterHandler(ClusterHandler):
    ...
    def cluster_command(self, tsn, command_id, args):
        ...
        if command_id == 0:
            zone_status = args[0]
            self.cluster.update_attribute(IasZone.attributes_by_name['zone_status'].id, zone_status)
            self.debug('Updated alarm state: %s', zone_status)
        elif command_id == 1:
            self.debug('Enroll requested')
            self._cluster.create_catching_task(self.enroll_response(enroll_response_code=IasZone.EnrollResponse.Success, zone_id=0))
    ...
    async def async_configure(self):
        ...
        try:
            ...
            self.debug("wrote cie_addr: %s to '%s' cluster", str(ieee), self._cluster.ep_attribute)
    ...
    def attribute_updated(self, attrid: int, value: Any, _: Any) -> None:
        ...
        if attrid == IasZone.attributes_by_name['zone_status'].id:
            self.async_send_signal(f'{self.unique_id}_{SIGNAL_ATTR_UPDATED}', attrid, 'zone_status', value)