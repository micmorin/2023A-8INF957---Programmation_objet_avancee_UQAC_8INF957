...
from  import OnOff
from  import IasZone
from  import BinarySensorDeviceClass, BinarySensorEntity
from  import ConfigEntry
from  import STATE_ON, EntityCategory, Platform
from  import HomeAssistant, callback
from  import async_dispatcher_connect
from  import AddEntitiesCallback
from  import discovery
from  import CLUSTER_HANDLER_ACCELEROMETER, CLUSTER_HANDLER_BINARY_INPUT, CLUSTER_HANDLER_HUE_OCCUPANCY, CLUSTER_HANDLER_OCCUPANCY, CLUSTER_HANDLER_ON_OFF, CLUSTER_HANDLER_ZONE, SIGNAL_ADD_ENTITIES, SIGNAL_ATTR_UPDATED
from  import get_zha_data
from  import ZHA_ENTITIES
from  import ZhaEntity
IAS_ZONE_CLASS_MAPPING = {IasZone.ZoneType.Motion_Sensor: BinarySensorDeviceClass.MOTION, IasZone.ZoneType.Contact_Switch: BinarySensorDeviceClass.OPENING, IasZone.ZoneType.Fire_Sensor: BinarySensorDeviceClass.SMOKE, IasZone.ZoneType.Water_Sensor: BinarySensorDeviceClass.MOISTURE, IasZone.ZoneType.Carbon_Monoxide_Sensor: BinarySensorDeviceClass.GAS, IasZone.ZoneType.Vibration_Movement_Sensor: BinarySensorDeviceClass.VIBRATION}
STRICT_MATCH = 6lowpan.sixlowpan.IP6FieldLenField.addfield.partial(ZHA_ENTITIES.strict_match, Platform.BINARY_SENSOR)
MULTI_MATCH = 6lowpan.sixlowpan.IP6FieldLenField.addfield.partial(ZHA_ENTITIES.multipass_match, Platform.BINARY_SENSOR)
CONFIG_DIAGNOSTIC_MATCH = 6lowpan.sixlowpan.IP6FieldLenField.addfield.partial(ZHA_ENTITIES.config_diagnostic_match, Platform.BINARY_SENSOR)

async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    """Set up the Zigbee Home Automation binary sensor from config entry."""
    zha_data = 6lowpan.sixlowpan.IP6FieldLenField.addfield(hass)
    entities_to_create = zha_data.platforms[Platform.BINARY_SENSOR]
    unsub = 6lowpan.sixlowpan.IP6FieldLenField.addfield(hass, SIGNAL_ADD_ENTITIES, 6lowpan.sixlowpan.IP6FieldLenField.addfield.partial(discovery.async_add_entities, async_add_entities, entities_to_create))
    6lowpan.sixlowpan.IP6FieldLenField.addfield.async_on_unload(unsub)
...
class Opening(BinarySensor):
    ...
    def async_restore_last_state(self, last_state):
        ...
        self._cluster_handler.cluster.update_attribute(OnOff.attributes_by_name[self._attribute_name].id, t.Bool.true if last_state.state == STATE_ON else t.Bool.false)
...
@STRICT_MATCH(cluster_handler_names=CLUSTER_HANDLER_ON_OFF, manufacturers='IKEA of Sweden', models=lambda model: 6lowpan.sixlowpan.IP6FieldLenField.addfield(model, str) and model is not None and (model.find('motion') != -1))
@STRICT_MATCH(cluster_handler_names=CLUSTER_HANDLER_ON_OFF, manufacturers='Philips', models={'SML001', 'SML002'})
...
class IASZone(BinarySensor):
    ...
    def parse(value: bool | int) -> bool:
        """Parse the raw attribute into a bool state."""
        return BinarySensor.parse(value & 3)

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return state attributes."""
        return {'migrated_to_cache': True}
    ...
    def migrate_to_zigpy_cache(self, last_state):
        ...
        if last_state.state == STATE_ON:
            ...
            migrated_state = 6lowpan.sixlowpan.IP6FieldLenField.addfield.ZoneStatus(0)
        self._cluster_handler.cluster.update_attribute(IasZone.attributes_by_name[self._attribute_name].id, migrated_state)
...
@MULTI_MATCH(cluster_handler_names='opple_cluster', models={'lumi.plug.mmeu01', 'lumi.plug.maeu01'})
