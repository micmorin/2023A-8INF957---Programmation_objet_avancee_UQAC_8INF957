...
from  import ListPortInfo
import voluptuous as vol
import 6lowpan.sixlowpan.IP6FieldLenField.addfield
from  import CONF_DEVICE_PATH
from  import config_entries
from  import onboarding, usb, zeroconf
from  import process_uploaded_file
from  import AddonError, AddonState
from  import silabs_multiprotocol_addon
from  import hardware as yellow_hardware
from  import CONF_NAME
from  import HomeAssistant, callback
from  import FlowHandler, FlowResult
from  import HomeAssistantError
from  import FileSelector, FileSelectorConfig
from  import dt as dt_util
from  import CONF_BAUDRATE, CONF_FLOWCONTROL, CONF_RADIO_TYPE, DOMAIN, RadioType
from  import HARDWARE_DISCOVERY_SCHEMA, RECOMMENDED_RADIOS, ProbeResult, ZhaRadioManager
CONF_MANUAL_PATH = 'Enter Manually'
SUPPORTED_PORT_SETTINGS = (CONF_BAUDRATE, CONF_FLOWCONTROL)
...
def _format_backup_choice(backup: zigpy.backups.NetworkBackup, *, pan_ids: bool=True) -> str:
    """Format network backup info into a short piece of text."""
    if not pan_ids:
        return dt_util.as_local(backup.backup_time).strftime('%c')
    identifier = f"{str(backup.network_info.pan_id)[2:]}:{str(backup.network_info.extended_pan_id).replace(':', '')}".lower()
...
async def list_serial_ports(hass: HomeAssistant) -> list[ListPortInfo]:
    ...
    if addon_info is not None and addon_info.state != AddonState.NOT_INSTALLED:
        addon_port = ListPortInfo(device=silabs_multiprotocol_addon.get_zigbee_socket(), skip_link_detection=True)
...
class BaseZhaFlow(FlowHandler):
    ...
    async def _async_create_radio_entry(self) -> FlowResult:
        ...
        device_settings[CONF_DEVICE_PATH] = await self.hass.async_add_executor_job(usb.get_serial_by_id, self._radio_mgr.device_path)
        return self.async_create_entry(title=self._title, data={CONF_DEVICE: device_settings, CONF_RADIO_TYPE: self._radio_mgr.radio_type.name})
    ...
    async def async_step_choose_serial_port(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        list_of_ports = [f"{p}{(', s/n: ' + p.serial_number if p.serial_number else '')}" + (f' - {p.manufacturer}' if p.manufacturer else '') for p in ports]
        ...
        if user_input is not None:
            ...
            if probe_result == ProbeResult.WRONG_FIRMWARE_INSTALLED:
                return self.async_abort(reason='wrong_firmware_installed', description_placeholders={'repair_url': REPAIR_MY_URL})
            if probe_result == ProbeResult.PROBING_FAILED:
                return await self.async_step_manual_pick_radio_type()
            self._title = f"{port.description}{(', s/n: ' + port.serial_number if port.serial_number else '')} - {port.manufacturer}" if port.manufacturer else ''
        ...
        schema = vol.Schema({vol.Required(CONF_DEVICE_PATH, default=default_port): vol.In(list_of_ports)})
        return self.async_show_form(step_id='choose_serial_port', data_schema=schema)

    async def async_step_manual_pick_radio_type(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        """Manually select the radio type."""
        if user_input is not None:
            self._radio_mgr.radio_type = RadioType.get_by_description(user_input[CONF_RADIO_TYPE])
        ...
        schema = {vol.Required(CONF_RADIO_TYPE, default=default): vol.In(RadioType.list())}
        return self.async_show_form(step_id='manual_pick_radio_type', data_schema=vol.Schema(schema))

    async def async_step_manual_port_config(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        schema = {vol.Required(CONF_DEVICE_PATH, default=self._radio_mgr.device_path or vol.UNDEFINED): str}
        source = self.context.get('source')
        for (param, value) in self._radio_mgr.radio_type.controller.SCHEMA_DEVICE.schema.items():
            ...
            elif self._radio_mgr.device_settings is not None and param in self._radio_mgr.device_settings:
                param = vol.Required(str(param), default=self._radio_mgr.device_settings[param])
            schema[param] = value
        return self.async_show_form(step_id='manual_port_config', data_schema=vol.Schema(schema), errors=errors)

    async def async_step_verify_radio(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        return self.async_show_form(step_id='verify_radio', description_placeholders={CONF_NAME: self._radio_mgr.radio_type.description, 'docs_recommended_adapters_url': 'https://www.home-assistant.io/integrations/zha/#recommended-zigbee-radio-adapters-and-modules'})

    async def async_step_choose_formation_strategy(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        """Choose how to deal with the current radio's settings."""
        await self._radio_mgr.async_load_network_settings()
        strategies = []
        if self._radio_mgr.backups and (self._radio_mgr.current_settings is None or any((not backup.is_compatible_with(self._radio_mgr.current_settings) for backup in self._radio_mgr.backups))):
        ...
        if not onboarding.async_is_onboarded(self.hass) and set(strategies) == {FORMATION_UPLOAD_MANUAL_BACKUP, FORMATION_FORM_INITIAL_NETWORK}:
            return await self.async_step_form_initial_network()
        return self.async_show_menu(step_id='choose_formation_strategy', menu_options=strategies)

    async def async_step_reuse_settings(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        """Reuse the existing network settings on the stick."""
        return await self._async_create_radio_entry()

    async def async_step_form_initial_network(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        """Form an initial network."""
        return await self.async_step_form_new_network(user_input)

    async def async_step_form_new_network(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        """Form a brand-new network."""
        await self._radio_mgr.async_form_network()
        return await self._async_create_radio_entry()

    def _parse_uploaded_backup(self, uploaded_file_id: str) -> zigpy.backups.NetworkBackup:
    ...
    async def async_step_upload_manual_backup(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        """Upload and restore a coordinator backup JSON file."""
        errors = {}
        if user_input is not None:
            try:
                self._radio_mgr.chosen_backup = await self.hass.async_add_executor_job(self._parse_uploaded_backup, user_input[UPLOADED_BACKUP_FILE])
            except ValueError:
                errors['base'] = 'invalid_backup_json'
            else:
                return await self.async_step_maybe_confirm_ezsp_restore()
        return self.async_show_form(step_id='upload_manual_backup', data_schema=vol.Schema({vol.Required(UPLOADED_BACKUP_FILE): FileSelector(FileSelectorConfig(accept='.json,application/json'))}), errors=errors)

    async def async_step_choose_automatic_backup(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        """Choose an automatic backup."""
        if self.show_advanced_options:
            choices = [_format_backup_choice(backup, pan_ids=True) for backup in self._radio_mgr.backups]
        else:
            num_backups_on_date = collections.Counter((backup.backup_time.date() for backup in self._radio_mgr.backups))
            choices = [_format_backup_choice(backup, pan_ids=num_backups_on_date[backup.backup_time.date()] > 1) for backup in self._radio_mgr.backups]
        if user_input is not None:
            index = choices.index(user_input[CHOOSE_AUTOMATIC_BACKUP])
            self._radio_mgr.chosen_backup = self._radio_mgr.backups[index]
            return await self.async_step_maybe_confirm_ezsp_restore()
        return self.async_show_form(step_id='choose_automatic_backup', data_schema=vol.Schema({vol.Required(CHOOSE_AUTOMATIC_BACKUP, default=choices[0]): vol.In(choices)}))

    async def async_step_maybe_confirm_ezsp_restore(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if not call_step_2:
            return await self._async_create_radio_entry()
        if user_input is not None:
            await self._radio_mgr.async_restore_backup_step_2(user_input[OVERWRITE_COORDINATOR_IEEE])
            return await self._async_create_radio_entry()
        return self.async_show_form(step_id='maybe_confirm_ezsp_restore', data_schema=vol.Schema({vol.Required(OVERWRITE_COORDINATOR_IEEE, default=True): bool}))

class ZhaConfigFlowHandler(BaseZhaFlow, config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow."""
    VERSION = 3

    async def _set_unique_id_or_update_path(self, unique_id: str, device_path: str) -> None:
        ...
        if not current_entry:
            return
        self._abort_if_unique_id_configured(updates={CONF_DEVICE: {**current_entry.data.get(CONF_DEVICE, {}), CONF_DEVICE_PATH: device_path}})
    ...
    async def async_step_confirm(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if user_input is not None or not onboarding.async_is_onboarded(self.hass):
            ...
            if self._radio_mgr.device_settings is None:
                return await self.async_step_manual_port_config()
            return await self.async_step_verify_radio()
        return self.async_show_form(step_id='confirm', description_placeholders={CONF_NAME: self._title})
    ...
    async def async_step_usb(self, discovery_info: usb.UsbServiceInfo) -> FlowResult:
        ...
        await self._set_unique_id_or_update_path(unique_id=f'{vid}:{pid}_{serial_number}_{manufacturer}_{description}', device_path=dev_path)
        ...
        self._title = description or usb.human_readable_device_name(dev_path, serial_number, manufacturer, description, vid, pid)
    ...
    async def async_step_zeroconf(self, discovery_info: zeroconf.ZeroconfServiceInfo) -> FlowResult:
        ...
        if 'radio_type' in discovery_info.properties:
            self._radio_mgr.radio_type = self._radio_mgr.parse_radio_type(discovery_info.properties['radio_type'])
        ...
        await self._set_unique_id_or_update_path(unique_id=node_name, device_path=device_path)
        self.context['title_placeholders'] = {CONF_NAME: node_name}
        self._title = device_path
        self._radio_mgr.device_path = device_path
        return await self.async_step_confirm()

    async def async_step_hardware(self, data: dict[str, Any] | None=None) -> FlowResult:
        ...
        try:
            device_settings = radio_type.controller.SCHEMA_DEVICE(discovery_data['port'])
        except vol.Invalid:
            return self.async_abort(reason='invalid_hardware_data')
        await self._set_unique_id_or_update_path(unique_id=f'{name}_{radio_type.name}_{device_settings[CONF_DEVICE_PATH]}', device_path=device_settings[CONF_DEVICE_PATH])
        self._title = name
...
class ZhaOptionsFlowHandler(BaseZhaFlow, config_entries.OptionsFlow):
    ...
    async def async_step_prompt_migrate_or_reconfigure(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        """Confirm if we are migrating adapters or just re-configuring."""
        return self.async_show_menu(step_id='prompt_migrate_or_reconfigure', menu_options=[OPTIONS_INTENT_RECONFIGURE, OPTIONS_INTENT_MIGRATE])

    async def async_step_intent_reconfigure(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        """Virtual step for when the user is reconfiguring the integration."""
        return await self.async_step_choose_serial_port()

    async def async_step_intent_migrate(self, user_input: dict[str, Any] | None=None) -> FlowResult:
    ...
    async def async_step_instruct_unplug(self, user_input: dict[str, Any] | None=None) -> FlowResult:
    ...
    async def _async_create_radio_entry(self):
        ...
        device_settings = self._radio_mgr.device_settings.copy()
        device_settings[CONF_DEVICE_PATH] = await self.hass.async_add_executor_job(usb.get_serial_by_id, self._radio_mgr.device_path)
        self.hass.config_entries.async_update_entry(entry=self.config_entry, data={CONF_DEVICE: device_settings, CONF_RADIO_TYPE: self._radio_mgr.radio_type.name}, options=self.config_entry.options)
    ...
    def async_remove(self):
        ...
        if self.config_entry.state not in (config_entries.ConfigEntryState.SETUP_ERROR, config_entries.ConfigEntryState.NOT_LOADED):
            return
        self.hass.async_create_task(self.hass.config_entries.async_setup(self.config_entry.entry_id))