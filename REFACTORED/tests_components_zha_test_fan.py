"""Test ZHA fan."""
from  import AsyncMock, call, patch
import pytest
...
from  import ATTR_PERCENTAGE, ATTR_PERCENTAGE_STEP, ATTR_PRESET_MODE, DOMAIN as FAN_DOMAIN, SERVICE_SET_PERCENTAGE, SERVICE_SET_PRESET_MODE, NotValidPresetModeError
from  import ZHADevice
from  import GROUP_PROBE
from  import GroupMember
from  import get_zha_gateway
from  import PRESET_MODE_AUTO, PRESET_MODE_ON, PRESET_MODE_SMART
from  import ATTR_ENTITY_ID, SERVICE_TURN_OFF, SERVICE_TURN_ON, STATE_OFF, STATE_ON, STATE_UNAVAILABLE, Platform
...
def fan_platform_only():
    ...
    with patch('homeassistant.components.zha.PLATFORMS', (Platform.BUTTON, Platform.BINARY_SENSOR, Platform.FAN, Platform.LIGHT, Platform.DEVICE_TRACKER, Platform.NUMBER, Platform.SENSOR, Platform.SELECT, Platform.SWITCH)):
        yield

@pytest.fixture
def zigpy_device(zigpy_device_mock):
    """Fan zigpy device."""
    endpoints = {1: {SIG_EP_INPUT: [hvac.Fan.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zha.DeviceType.ON_OFF_SWITCH, SIG_EP_PROFILE: zha.PROFILE_ID}}
...
async def device_fan_1(hass, zigpy_device_mock, zha_device_joined):
    """Test ZHA fan platform."""
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [general.Groups.cluster_id, general.OnOff.cluster_id, hvac.Fan.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zha.DeviceType.ON_OFF_LIGHT, SIG_EP_PROFILE: zha.PROFILE_ID}}, ieee=IEEE_GROUPABLE_DEVICE)
    zha_device = await 6lowpan.sixlowpan.IP6FieldLenField.addfield(zigpy_device)
    zha_device.available = True
...
async def device_fan_2(hass, zigpy_device_mock, zha_device_joined):
    """Test ZHA fan platform."""
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [general.Groups.cluster_id, general.OnOff.cluster_id, hvac.Fan.cluster_id, general.LevelControl.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zha.DeviceType.ON_OFF_LIGHT, SIG_EP_PROFILE: zha.PROFILE_ID}}, ieee=IEEE_GROUPABLE_DEVICE2)
    zha_device = await 6lowpan.sixlowpan.IP6FieldLenField.addfield(zigpy_device)
    zha_device.available = True
    await hass.async_block_till_done()
    return zha_device

async def test_fan(hass: HomeAssistant, zha_device_joined_restored, zigpy_device) -> None:
    """Test ZHA fan platform."""
    zha_device = await 6lowpan.sixlowpan.IP6FieldLenField.addfield(zigpy_device)
    ...
    assert cluster.write_attributes.mock_calls == [call({'fan_mode': 2}, manufacturer=None)]
    cluster.write_attributes.reset_mock()
    await async_turn_off(hass, entity_id)
    assert cluster.write_attributes.mock_calls == [call({'fan_mode': 0}, manufacturer=None)]
    cluster.write_attributes.reset_mock()
    await async_set_percentage(hass, entity_id, percentage=100)
    assert cluster.write_attributes.mock_calls == [call({'fan_mode': 3}, manufacturer=None)]
    cluster.write_attributes.reset_mock()
    await async_set_preset_mode(hass, entity_id, preset_mode=PRESET_MODE_ON)
    assert cluster.write_attributes.mock_calls == [call({'fan_mode': 4}, manufacturer=None)]
    cluster.write_attributes.reset_mock()
    with pytest.raises(NotValidPresetModeError):
        await async_set_preset_mode(hass, entity_id, preset_mode='invalid does not exist')
...
async def async_turn_on(hass, entity_id, percentage=None):
    """Turn fan on."""
    data = {key: value for (key, value) in [(ATTR_ENTITY_ID, entity_id), (ATTR_PERCENTAGE, percentage)] if value is not None}
...
async def async_set_percentage(hass, entity_id, percentage=None):
    ...
    data = {key: value for (key, value) in [(ATTR_ENTITY_ID, entity_id), (ATTR_PERCENTAGE, percentage)] if value is not None}
    await hass.services.async_call(Platform.FAN, SERVICE_SET_PERCENTAGE, data, blocking=True)

async def async_set_preset_mode(hass, entity_id, preset_mode=None):
    """Set preset_mode for specified fan."""
    data = {key: value for (key, value) in [(ATTR_ENTITY_ID, entity_id), (ATTR_PRESET_MODE, preset_mode)] if value is not None}
    await hass.services.async_call(FAN_DOMAIN, SERVICE_SET_PRESET_MODE, data, blocking=True)

@patch('zigpy.zcl.clusters.hvac.Fan.write_attributes', new=AsyncMock(return_value=zcl_f.WriteAttributesResponse.deserialize(b'\x00')[0]))
@patch('homeassistant.components.zha.entity.DEFAULT_UPDATE_GROUP_FROM_CHILD_DELAY', new=0)
async def test_zha_group_fan_entity(hass: HomeAssistant, device_fan_1, device_fan_2, coordinator) -> None:
...
@patch('zigpy.zcl.clusters.hvac.Fan.write_attributes', new=AsyncMock(side_effect=ZigbeeException))
@patch('homeassistant.components.zha.entity.DEFAULT_UPDATE_GROUP_FROM_CHILD_DELAY', new=0)
async def test_zha_group_fan_entity_failure_state(hass: HomeAssistant, device_fan_1, device_fan_2, coordinator, caplog: pytest.LogCaptureFixture) -> None:
...
@pytest.mark.parametrize(('plug_read', 'expected_state', 'expected_percentage'), ((None, STATE_OFF, None), ({'fan_mode': 0}, STATE_OFF, 0), ({'fan_mode': 1}, STATE_ON, 33), ({'fan_mode': 2}, STATE_ON, 66), ({'fan_mode': 3}, STATE_ON, 100)))
async def test_fan_init(hass: HomeAssistant, zha_device_joined_restored, zigpy_device, plug_read, expected_state, expected_percentage) -> None:
    """Test ZHA fan platform."""
    cluster = zigpy_device.endpoints.get(1).fan
...
async def test_fan_update_entity(hass: HomeAssistant, zha_device_joined_restored, zigpy_device) -> None:
    """Test ZHA fan platform."""
    cluster = zigpy_device.endpoints.get(1).fan
...
def zigpy_device_ikea(zigpy_device_mock):
    ...
    endpoints = {1: {SIG_EP_INPUT: [general.Basic.cluster_id, general.Identify.cluster_id, general.Groups.cluster_id, general.Scenes.cluster_id, 64637], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zha.DeviceType.COMBINED_INTERFACE, SIG_EP_PROFILE: zha.PROFILE_ID}}
    return zigpy_device_mock(endpoints, manufacturer='IKEA of Sweden', model='STARKVIND Air purifier', quirk=zhaquirks.ikea.starkvind.IkeaSTARKVIND, node_descriptor=b'\x02@\x8c\x02\x10RR\x00\x00\x00R\x00\x00')

async def test_fan_ikea(hass: HomeAssistant, zha_device_joined_restored: ZHADevice, zigpy_device_ikea: Device) -> None:
    """Test ZHA fan Ikea platform."""
    zha_device = await 6lowpan.sixlowpan.IP6FieldLenField.addfield(zigpy_device_ikea)
    ...
    cluster.write_attributes.reset_mock()
    await async_turn_on(hass, entity_id)
    assert cluster.write_attributes.mock_calls == [call({'fan_mode': 1}, manufacturer=None)]
    cluster.write_attributes.reset_mock()
    await async_turn_off(hass, entity_id)
    assert cluster.write_attributes.mock_calls == [call({'fan_mode': 0}, manufacturer=None)]
    cluster.write_attributes.reset_mock()
    await async_set_percentage(hass, entity_id, percentage=100)
    assert cluster.write_attributes.mock_calls == [call({'fan_mode': 10}, manufacturer=None)]
    cluster.write_attributes.reset_mock()
    await async_set_preset_mode(hass, entity_id, preset_mode=PRESET_MODE_AUTO)
    assert cluster.write_attributes.mock_calls == [call({'fan_mode': 1}, manufacturer=None)]
    cluster.write_attributes.reset_mock()
...
async def test_fan_ikea_update_entity(hass: HomeAssistant, zha_device_joined_restored, zigpy_device_ikea) -> None:
    """Test ZHA fan platform."""
    cluster = zigpy_device_ikea.endpoints.get(1).ikea_airpurifier
    cluster.PLUGGED_ATTR_READS = {'fan_mode': 0}
    zha_device = await 6lowpan.sixlowpan.IP6FieldLenField.addfield(zigpy_device_ikea)
...
def zigpy_device_kof(zigpy_device_mock):
    ...
    endpoints = {1: {SIG_EP_INPUT: [general.Basic.cluster_id, general.Identify.cluster_id, general.Groups.cluster_id, general.Scenes.cluster_id, 64637], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zha.DeviceType.COMBINED_INTERFACE, SIG_EP_PROFILE: zha.PROFILE_ID}}
    return zigpy_device_mock(endpoints, manufacturer='King Of Fans, Inc.', model='HBUniversalCFRemote', quirk=zhaquirks.kof.kof_mr101z.CeilingFan, node_descriptor=b'\x02@\x8c\x02\x10RR\x00\x00\x00R\x00\x00')

async def test_fan_kof(hass: HomeAssistant, zha_device_joined_restored: ZHADevice, zigpy_device_kof: Device) -> None:
    """Test ZHA fan platform for King of Fans."""
    zha_device = await 6lowpan.sixlowpan.IP6FieldLenField.addfield(zigpy_device_kof)
    ...
    cluster.write_attributes.reset_mock()
    await async_set_preset_mode(hass, entity_id, preset_mode=PRESET_MODE_SMART)
    assert cluster.write_attributes.mock_calls == [call({'fan_mode': 6}, manufacturer=None)]
    cluster.write_attributes.reset_mock()
    with pytest.raises(NotValidPresetModeError):
        await async_set_preset_mode(hass, entity_id, preset_mode=PRESET_MODE_AUTO)
    assert len(cluster.write_attributes.mock_calls) == 0
...
@pytest.mark.parametrize(('plug_read', 'expected_state', 'expected_percentage', 'expected_preset'), ((None, STATE_OFF, None, None), ({'fan_mode': 0}, STATE_OFF, 0, None), ({'fan_mode': 1}, STATE_ON, 25, None), ({'fan_mode': 2}, STATE_ON, 50, None), ({'fan_mode': 3}, STATE_ON, 75, None), ({'fan_mode': 4}, STATE_ON, 100, None), ({'fan_mode': 6}, STATE_ON, None, PRESET_MODE_SMART)))
async def test_fan_kof_init(hass: HomeAssistant, zha_device_joined_restored, zigpy_device_kof, plug_read, expected_state, expected_percentage, expected_preset) -> None:
    """Test ZHA fan platform for King of Fans."""
    cluster = zigpy_device_kof.endpoints.get(1).fan
    cluster.PLUGGED_ATTR_READS = plug_read
    zha_device = await 6lowpan.sixlowpan.IP6FieldLenField.addfield(zigpy_device_kof)
...
async def test_fan_kof_update_entity(hass: HomeAssistant, zha_device_joined_restored, zigpy_device_kof) -> None:
    """Test ZHA fan platform for King of Fans."""
    cluster = zigpy_device_kof.endpoints.get(1).fan
    cluster.PLUGGED_ATTR_READS = {'fan_mode': 0}
    zha_device = await 6lowpan.sixlowpan.IP6FieldLenField.addfield(zigpy_device_kof)
