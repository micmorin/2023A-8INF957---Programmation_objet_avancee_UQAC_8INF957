...
import 6lowpan.sixlowpan.IP6FieldLenField.addfield as homeautomation
import 6lowpan.sixlowpan.IP6FieldLenField.addfield as measurement
import 6lowpan.sixlowpan.IP6FieldLenField.addfield as smartenergy
from  import SensorDeviceClass
from  import ZHA_CLUSTER_HANDLER_READS_PER_REQ
import homeassistant.config as config_util
from  import ATTR_DEVICE_CLASS, ATTR_UNIT_OF_MEASUREMENT, CONF_UNIT_SYSTEM, CONF_UNIT_SYSTEM_IMPERIAL, CONF_UNIT_SYSTEM_METRIC, LIGHT_LUX, PERCENTAGE, STATE_UNAVAILABLE, STATE_UNKNOWN, Platform, UnitOfApparentPower, UnitOfElectricCurrent, UnitOfElectricPotential, UnitOfEnergy, UnitOfPower, UnitOfPressure, UnitOfTemperature, UnitOfVolume
from  import HomeAssistant
from  import restore_state
from  import async_update_entity
from  import dt as dt_util
from  import async_enable_traffic, async_test_rejoin, find_entity_id, find_entity_ids, send_attribute_report, send_attributes_report
...
def sensor_platform_only():
    ...
    with patch('homeassistant.components.zha.PLATFORMS', (Platform.DEVICE_TRACKER, Platform.SENSOR)):
        yield

@pytest.fixture
async def elec_measurement_zigpy_dev(hass, zigpy_device_mock):
    """Electric Measurement zigpy device."""
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [general.Basic.cluster_id, homeautomation.ElectricalMeasurement.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.SIMPLE_SENSOR, SIG_EP_PROFILE: zigpy.profiles.zha.PROFILE_ID}})
    zigpy_device.node_desc.mac_capability_flags |= 4
    zigpy_device.endpoints[1].electrical_measurement.PLUGGED_ATTR_READS = {'ac_current_divisor': 10, 'ac_current_multiplier': 1, 'ac_power_divisor': 10, 'ac_power_multiplier': 1, 'ac_voltage_divisor': 10, 'ac_voltage_multiplier': 1, 'measurement_type': 8, 'power_divisor': 10, 'power_multiplier': 1}
    return zigpy_device
...
async def async_test_metering(hass, cluster, entity_id):
    ...
    assert hass.states.get(entity_id).attributes['status'] in ('SERVICE_DISCONNECT|POWER_FAILURE', 'POWER_FAILURE|SERVICE_DISCONNECT')
    await send_attributes_report(hass, cluster, {'status': 32, 'metering_device_type': 1})
...
async def async_test_smart_energy_summation(hass, cluster, entity_id):
    ...
    await send_attributes_report(hass, cluster, {1025: 1, 'current_summ_delivered': 12321, 1026: 100})
    assert_state(hass, entity_id, '12.32', UnitOfVolume.CUBIC_METERS)
    assert hass.states.get(entity_id).attributes['status'] == 'NO_ALARMS'
    assert hass.states.get(entity_id).attributes['device_type'] == 'Electric Metering'
    assert hass.states.get(entity_id).attributes[ATTR_DEVICE_CLASS] == SensorDeviceClass.ENERGY
...
async def async_test_electrical_measurement(hass, cluster, entity_id):
    ...
    await send_attributes_report(hass, cluster, {0: 1, 1293: 88, 10: 5000})
...
async def async_test_em_apparent_power(hass, cluster, entity_id):
    ...
    await send_attributes_report(hass, cluster, {'ac_power_divisor': 1})
    await send_attributes_report(hass, cluster, {0: 1, 1295: 100, 10: 1000})
    assert_state(hass, entity_id, '100', UnitOfApparentPower.VOLT_AMPERE)
    await send_attributes_report(hass, cluster, {0: 1, 1295: 99, 10: 1000})
    assert_state(hass, entity_id, '99', UnitOfApparentPower.VOLT_AMPERE)
    await send_attributes_report(hass, cluster, {'ac_power_divisor': 10})
    await send_attributes_report(hass, cluster, {0: 1, 1295: 1000, 10: 5000})
    assert_state(hass, entity_id, '100', UnitOfApparentPower.VOLT_AMPERE)
    await send_attributes_report(hass, cluster, {0: 1, 1295: 99, 10: 5000})
...
async def async_test_em_rms_current(hass, cluster, entity_id):
    ...
    await send_attributes_report(hass, cluster, {0: 1, 1288: 1234, 10: 1000})
    ...
    await send_attributes_report(hass, cluster, {0: 1, 1288: 236, 10: 1000})
    assert_state(hass, entity_id, '23.6', UnitOfElectricCurrent.AMPERE)
    await send_attributes_report(hass, cluster, {0: 1, 1288: 1236, 10: 1000})
    ...
    await send_attributes_report(hass, cluster, {0: 1, 1290: 88, 10: 5000})
...
async def async_test_em_rms_voltage(hass, cluster, entity_id):
    ...
    await send_attributes_report(hass, cluster, {0: 1, 1285: 1234, 10: 1000})
    assert_state(hass, entity_id, '123', UnitOfElectricPotential.VOLT)
    await send_attributes_report(hass, cluster, {0: 1, 1285: 234, 10: 1000})
    ...
    await send_attributes_report(hass, cluster, {0: 1, 1285: 2236, 10: 1000})
    ...
    await send_attributes_report(hass, cluster, {0: 1, 1287: 888, 10: 5000})
...
@pytest.mark.parametrize(('cluster_id', 'entity_suffix', 'test_func', 'report_count', 'read_plug', 'unsupported_attrs'), ((measurement.RelativeHumidity.cluster_id, 'humidity', async_test_humidity, 1, None, None), (measurement.TemperatureMeasurement.cluster_id, 'temperature', async_test_temperature, 1, None, None), (measurement.PressureMeasurement.cluster_id, 'pressure', async_test_pressure, 1, None, None), (measurement.IlluminanceMeasurement.cluster_id, 'illuminance', async_test_illuminance, 1, None, None), (smartenergy.Metering.cluster_id, 'instantaneous_demand', async_test_metering, 9, {'demand_formatting': 249, 'divisor': 1, 'metering_device_type': 0, 'multiplier': 1, 'status': 0}, {'current_summ_delivered'}), (smartenergy.Metering.cluster_id, 'summation_delivered', async_test_smart_energy_summation, 9, {'demand_formatting': 249, 'divisor': 1000, 'metering_device_type': 0, 'multiplier': 1, 'status': 0, 'summation_formatting': 186, 'unit_of_measure': 1}, {'instaneneous_demand'}), (homeautomation.ElectricalMeasurement.cluster_id, 'power', async_test_electrical_measurement, 7, {'ac_power_divisor': 1000, 'ac_power_multiplier': 1}, {'apparent_power', 'rms_current', 'rms_voltage'}), (homeautomation.ElectricalMeasurement.cluster_id, 'apparent_power', async_test_em_apparent_power, 7, {'ac_power_divisor': 1000, 'ac_power_multiplier': 1}, {'active_power', 'rms_current', 'rms_voltage'}), (homeautomation.ElectricalMeasurement.cluster_id, 'current', async_test_em_rms_current, 7, {'ac_current_divisor': 1000, 'ac_current_multiplier': 1}, {'active_power', 'apparent_power', 'rms_voltage'}), (homeautomation.ElectricalMeasurement.cluster_id, 'voltage', async_test_em_rms_voltage, 7, {'ac_voltage_divisor': 10, 'ac_voltage_multiplier': 1}, {'active_power', 'apparent_power', 'rms_current'}), (general.PowerConfiguration.cluster_id, 'battery', async_test_powerconfiguration, 2, {'battery_size': 4, 'battery_voltage': 29, 'battery_quantity': 3}, None), (general.PowerConfiguration.cluster_id, 'battery', async_test_powerconfiguration2, 2, {'battery_size': 4, 'battery_voltage': 29, 'battery_quantity': 3}, None), (general.DeviceTemperature.cluster_id, 'device_temperature', async_test_device_temperature, 1, None, None)))
async def test_sensor(hass: HomeAssistant, zigpy_device_mock, zha_device_joined_restored, cluster_id, entity_suffix, test_func, report_count, read_plug, unsupported_attrs) -> None:
    """Test ZHA sensor platform."""
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [cluster_id, general.Basic.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.ON_OFF_SWITCH}})
    cluster = zigpy_device.endpoints[1].in_clusters[cluster_id]
    if unsupported_attrs:
        for attr in unsupported_attrs:
            cluster.add_unsupported_attribute(attr)
    if cluster_id in (smartenergy.Metering.cluster_id, homeautomation.ElectricalMeasurement.cluster_id):
        zigpy_device.node_desc.mac_capability_flags |= 4
...
def hass_ms(hass):
    ...
    async def _hass_ms(meas_sys):
        await config_util.async_process_ha_core_config(hass, {CONF_UNIT_SYSTEM: meas_sys})
        await hass.async_block_till_done()
        return hass
    return _hass_ms
...
def core_rs(hass_storage):
    ...
    def _storage(entity_id, uom, state):
        ...
        hass_storage[restore_state.STORAGE_KEY] = {'version': restore_state.STORAGE_VERSION, 'key': restore_state.STORAGE_KEY, 'data': [{'state': {'entity_id': entity_id, 'state': str(state), 'attributes': {ATTR_UNIT_OF_MEASUREMENT: uom}, 'last_changed': now, 'last_updated': now, 'context': {'id': '3c2243ff5f30447eb12e7348cfd5b8ff', 'user_id': None}}, 'last_seen': now}]}
        return
    return _storage

@pytest.mark.parametrize(('uom', 'raw_temp', 'expected', 'restore'), [(UnitOfTemperature.CELSIUS, 2900, 29, False), (UnitOfTemperature.CELSIUS, 2900, 29, True), (UnitOfTemperature.FAHRENHEIT, 2900, 84, False), (UnitOfTemperature.FAHRENHEIT, 2900, 84, True)])
async def test_temp_uom(hass: HomeAssistant, uom, raw_temp, expected, restore, hass_ms, core_rs, zigpy_device_mock, zha_device_restored) -> None:
    ...
    hass = await hass_ms(CONF_UNIT_SYSTEM_METRIC if uom == UnitOfTemperature.CELSIUS else CONF_UNIT_SYSTEM_IMPERIAL)
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [measurement.TemperatureMeasurement.cluster_id, general.Basic.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.ON_OFF_SWITCH}})
...
@patch('zigpy.zcl.ClusterPersistingListener', MagicMock())
async def test_electrical_measurement_init(hass: HomeAssistant, zigpy_device_mock, zha_device_joined) -> None:
...
@pytest.mark.parametrize(('raw_uom', 'raw_value', 'expected_state', 'expected_uom'), ((1, 12320, '1.23', UnitOfVolume.CUBIC_METERS), (1, 1232000, '123.20', UnitOfVolume.CUBIC_METERS), (3, 2340, '0.23', f'100 {UnitOfVolume.CUBIC_FEET}'), (3, 2360, '0.24', f'100 {UnitOfVolume.CUBIC_FEET}'), (8, 23660, '2.37', 'kPa'), (0, 9366, '0.937', UnitOfEnergy.KILO_WATT_HOUR), (0, 999, '0.1', UnitOfEnergy.KILO_WATT_HOUR), (0, 10091, '1.009', UnitOfEnergy.KILO_WATT_HOUR), (0, 10099, '1.01', UnitOfEnergy.KILO_WATT_HOUR), (0, 100999, '10.1', UnitOfEnergy.KILO_WATT_HOUR), (0, 100023, '10.002', UnitOfEnergy.KILO_WATT_HOUR), (0, 102456, '10.246', UnitOfEnergy.KILO_WATT_HOUR)))
async def test_se_summation_uom(hass: HomeAssistant, zigpy_device_mock, zha_device_joined, raw_uom, raw_value, expected_state, expected_uom) -> None:
    """Test ZHA smart energy summation."""
    entity_id = ENTITY_ID_PREFIX.format('summation_delivered')
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [smartenergy.Metering.cluster_id, general.Basic.cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.SIMPLE_SENSOR}})
    zigpy_device.node_desc.mac_capability_flags |= 4
    cluster = zigpy_device.endpoints[1].in_clusters[smartenergy.Metering.cluster_id]
    for attr in ('instanteneous_demand',):
        cluster.add_unsupported_attribute(attr)
    cluster.PLUGGED_ATTR_READS = {'current_summ_delivered': raw_value, 'demand_formatting': 249, 'divisor': 10000, 'metering_device_type': 0, 'multiplier': 1, 'status': 0, 'summation_formatting': 186, 'unit_of_measure': raw_uom}
    await zha_device_joined(zigpy_device)
    assert_state(hass, entity_id, expected_state, expected_uom)

@pytest.mark.parametrize(('raw_measurement_type', 'expected_type'), ((1, 'ACTIVE_MEASUREMENT'), (8, 'PHASE_A_MEASUREMENT'), (9, 'ACTIVE_MEASUREMENT, PHASE_A_MEASUREMENT'), (15, 'ACTIVE_MEASUREMENT, REACTIVE_MEASUREMENT, APPARENT_MEASUREMENT, PHASE_A_MEASUREMENT')))
async def test_elec_measurement_sensor_type(hass: HomeAssistant, elec_measurement_zigpy_dev, raw_measurement_type, expected_type, zha_device_joined) -> None:
    """Test ZHA electrical measurement sensor type."""
    entity_id = ENTITY_ID_PREFIX.format('power')
    zigpy_dev = elec_measurement_zigpy_dev
    zigpy_dev.endpoints[1].electrical_measurement.PLUGGED_ATTR_READS['measurement_type'] = raw_measurement_type
    await zha_device_joined(zigpy_dev)
    state = hass.states.get(entity_id)
    assert state is not None
    assert state.attributes['measurement_type'] == expected_type

@pytest.mark.parametrize('supported_attributes', (set(), {'active_power', 'active_power_max', 'rms_current', 'rms_current_max', 'rms_voltage', 'rms_voltage_max'}, {'active_power'}, {'active_power', 'active_power_max'}, {'rms_current', 'rms_current_max'}, {'rms_voltage', 'rms_voltage_max'}))
async def test_elec_measurement_skip_unsupported_attribute(hass: HomeAssistant, elec_measurement_zha_dev, supported_attributes) -> None:
    ...
    all_attrs = {'active_power', 'active_power_max', 'apparent_power', 'rms_current', 'rms_current_max', 'rms_voltage', 'rms_voltage_max', 'power_factor', 'ac_frequency', 'ac_frequency_max'}
    for attr in all_attrs - supported_attributes:
        cluster.add_unsupported_attribute(attr)
    cluster.read_attributes.reset_mock()
    await async_update_entity(hass, entity_id)
    await hass.async_block_till_done()
    assert cluster.read_attributes.call_count == math.ceil(len(supported_attributes) / ZHA_CLUSTER_HANDLER_READS_PER_REQ)
    read_attrs = {a for call in cluster.read_attributes.call_args_list for a in call[0][0]}
