...
from  import AsyncMock, MagicMock, patch
import pytest
...
def mock_usb_serial_by_id_fixture() -> Generator[MagicMock, None, None]:
    ...
    with patch('homeassistant.components.zha.config_flow.usb.get_serial_by_id') as mock_usb_serial_by_id:
...
def mock_zha():
    ...
    with patch('homeassistant.components.zha.radio_manager.ZhaRadioManager.connect_zigpy_app', return_value=mock_connect_app), patch('homeassistant.components.zha.async_setup_entry', return_value=True):
        yield

@pytest.fixture(autouse=True)
def mock_zha_get_last_network_settings() -> Generator[None, None, None]:
    """Mock zha.api.async_get_last_network_settings."""
    with patch('homeassistant.components.zha.api.async_get_last_network_settings', AsyncMock(return_value=None)):
        yield

@pytest.fixture(name='addon_running')
def mock_addon_running(addon_store_info, addon_info):
    """Mock add-on already running."""
    addon_store_info.return_value = {'installed': '1.0.0', 'state': 'started', 'version': '1.0.0'}
...
def mock_addon_installed(addon_store_info, addon_info):
    ...
    addon_store_info.return_value = {'installed': '1.0.0', 'state': 'stopped', 'version': '1.0.0'}
    addon_info.return_value['hostname'] = 'core-silabs-multiprotocol'
...
def addon_store_info_fixture():
    ...
    with patch('homeassistant.components.hassio.addon_manager.async_get_addon_store_info') as addon_store_info:
        addon_store_info.return_value = {'available': True, 'installed': None, 'state': None, 'version': '1.0.0'}
...
def addon_info_fixture():
    ...
    with patch('homeassistant.components.hassio.addon_manager.async_get_addon_info') as addon_info:
        addon_info.return_value = {'available': True, 'hostname': None, 'options': {}, 'state': None, 'update_available': False, 'version': None}
        yield addon_info
...
def mock_install_addon(install_addon_side_effect):
    """Mock install add-on."""
    with patch('homeassistant.components.hassio.addon_manager.async_install_addon', side_effect=install_addon_side_effect) as install_addon:
        yield install_addon
