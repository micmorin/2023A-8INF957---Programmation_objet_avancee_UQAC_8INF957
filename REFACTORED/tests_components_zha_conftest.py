...
import time
from  import Any
from  import AsyncMock, MagicMock, create_autospec, patch
import warnings
import pytest
...
from  import patch_cluster as common_patch_cluster
from  import MockConfigEntry
from  import mock_light_profiles
FIXTURE_GRP_ID = 4097
...
def disable_request_retry_delay():
    ...
    with patch('homeassistant.components.zha.core.cluster_handlers.RETRYABLE_REQUEST_DECORATOR', zigpy.util.retryable_request(tries=3, delay=0)):
        yield
...
class _FakeApp(ControllerApplication):
    ...
    async def permit_with_key(self, node: zigpy.types.EUI64, code: bytes, time_s: int=60):
        pass
...
async def zigpy_app_controller():
    ...
    app = _FakeApp({zigpy.config.CONF_DATABASE: None, zigpy.config.CONF_DEVICE: {zigpy.config.CONF_DEVICE_PATH: '/dev/null'}, zigpy.config.CONF_STARTUP_ENERGY_SCAN: False, zigpy.config.CONF_NWK_BACKUP_ENABLED: False, zigpy.config.CONF_TOPO_SCAN_ENABLED: False})
    ...
    dev.node_desc = 6lowpan.sixlowpan.IP6FieldLenField.addfield.NodeDescriptor()
...
async def config_entry_fixture(hass) -> MockConfigEntry:
    ...
    return MockConfigEntry(version=3, domain=zha_const.DOMAIN, data={zigpy.config.CONF_DEVICE: {zigpy.config.CONF_DEVICE_PATH: '/dev/ttyUSB0'}, zha_const.CONF_RADIO_TYPE: 'ezsp'}, options={zha_const.CUSTOM_CONFIGURATION: {zha_const.ZHA_OPTIONS: {zha_const.CONF_ENABLE_ENHANCED_LIGHT_TRANSITION: True, zha_const.CONF_GROUP_MEMBERS_ASSUME_STATE: False}, zha_const.ZHA_ALARM_OPTIONS: {zha_const.CONF_ALARM_ARM_REQUIRES_CODE: False, zha_const.CONF_ALARM_MASTER_CODE: '4321', zha_const.CONF_ALARM_FAILED_TRIES: 2}}})
...
@pytest.fixture
def setup_zha(hass, config_entry: MockConfigEntry, mock_zigpy_connect: ControllerApplication):
...
@pytest.fixture
def zha_device_mock(hass, zigpy_device_mock) -> Callable[..., zha_core_device.ZHADevice]:
    """Return a ZHA Device factory."""

    def _zha_device(endpoints=None, ieee='00:11:22:33:44:55:66:77', manufacturer='mock manufacturer', model='mock model', node_desc=b'\x02@\x807\x10\x7fd\x00\x00*d\x00\x00', patch_cluster=True) -> zha_core_device.ZHADevice:
        if endpoints is None:
            endpoints = {1: {'in_clusters': [0, 1, 8, 768], 'out_clusters': [25], 'device_type': 261}, 2: {'in_clusters': [0], 'out_clusters': [6, 8, 25, 768], 'device_type': 2064}}
        zigpy_device = 6lowpan.sixlowpan.IP6FieldLenField.addfield(endpoints, ieee, manufacturer, model, node_desc, patch_cluster=patch_cluster)
...
def hass_disable_services(hass):
    """Mock services."""
    with patch.object(hass, 'services', MagicMock(has_service=MagicMock(return_value=True))):
        yield hass
