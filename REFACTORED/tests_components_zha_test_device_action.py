...
import pytest
from  import unordered
from  import InovelliVZM31SNv11
...
from  import async_get_device_automations, async_mock_service
...
def required_platforms_only():
    ...
    with patch('homeassistant.components.zha.PLATFORMS', (Platform.BINARY_SENSOR, Platform.BUTTON, Platform.DEVICE_TRACKER, Platform.LIGHT, Platform.NUMBER, Platform.SELECT, Platform.SENSOR, Platform.SWITCH, Platform.SIREN)):
        yield
...
async def device_ias(hass, zigpy_device_mock, zha_device_joined_restored):
    ...
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [c.cluster_id for c in clusters], SIG_EP_OUTPUT: [general.OnOff.cluster_id], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.ON_OFF_SWITCH}})
...
async def device_inovelli(hass, zigpy_device_mock, zha_device_joined):
    ...
    zigpy_device = zigpy_device_mock({1: {SIG_EP_INPUT: [general.Basic.cluster_id, general.Identify.cluster_id, general.OnOff.cluster_id, general.LevelControl.cluster_id, 64561], SIG_EP_OUTPUT: [], SIG_EP_TYPE: zigpy.profiles.zha.DeviceType.DIMMABLE_LIGHT}}, ieee='00:1d:8f:08:0c:90:69:6b', manufacturer='Inovelli', model='VZM31-SN', quirk=InovelliVZM31SNv11)
...
async def test_get_actions(hass: HomeAssistant, device_ias) -> None:
    ...
    siren_level_select = entity_registry.async_get('select.fakemanufacturer_fakemodel_default_siren_level')
    siren_tone_select = entity_registry.async_get('select.fakemanufacturer_fakemodel_default_siren_tone')
    strobe_level_select = entity_registry.async_get('select.fakemanufacturer_fakemodel_default_strobe_level')
    strobe_select = entity_registry.async_get('select.fakemanufacturer_fakemodel_default_strobe')
    actions = await async_get_device_automations(hass, DeviceAutomationType.ACTION, reg_device.id)
    expected_actions = [{'domain': DOMAIN, 'type': 'squawk', 'device_id': reg_device.id, 'metadata': {}}, {'domain': DOMAIN, 'type': 'warn', 'device_id': reg_device.id, 'metadata': {}}]
    expected_actions.extend([{'domain': Platform.SELECT, 'type': action, 'device_id': reg_device.id, 'entity_id': entity_id, 'metadata': {'secondary': True}} for action in ['select_first', 'select_last', 'select_next', 'select_option', 'select_previous'] for entity_id in [siren_level_select.id, siren_tone_select.id, strobe_level_select.id, strobe_select.id]])
...
async def test_get_inovelli_actions(hass: HomeAssistant, device_inovelli) -> None:
    ...
    device_registry = dr.async_get(hass)
    inovelli_reg_device = device_registry.async_get_device(identifiers={(DOMAIN, inovelli_ieee_address)})
    entity_registry = er.async_get(hass)
...
async def test_action(hass: HomeAssistant, device_ias, device_inovelli) -> None:
    ...
    with patch('zigpy.zcl.Cluster.request', return_value=[0, zcl_f.Status.SUCCESS]):
        ...
        assert call(False, cluster.commands_by_name['led_effect'].id, cluster.commands_by_name['led_effect'].schema, 6, 41, 10, 5, expect_reply=False, manufacturer=4151, tsn=None) in cluster.request.call_args_list
        assert call(False, cluster.commands_by_name['individual_led_effect'].id, cluster.commands_by_name['individual_led_effect'].schema, 1, 6, 41, 10, 5, expect_reply=False, manufacturer=4151, tsn=None) in cluster.request.call_args_list
