...
import logging
import random
from  import TYPE_CHECKING, Any, cast
from  import OnOff
from  import Color
from  import Status
from  import light
from  import ColorMode, LightEntityFeature, brightness_supported, filter_supported_color_modes
from  import ConfigEntry
from  import ATTR_SUPPORTED_FEATURES, STATE_ON, STATE_UNAVAILABLE, Platform
from  import CALLBACK_TYPE, HomeAssistant, State, callback
from  import Debouncer
from  import async_dispatcher_connect, async_dispatcher_send
from  import AddEntitiesCallback
from  import async_call_later, async_track_time_interval
from  import discovery, helpers
from  import CLUSTER_HANDLER_COLOR, CLUSTER_HANDLER_LEVEL, CLUSTER_HANDLER_ON_OFF, CONF_ALWAYS_PREFER_XY_COLOR_MODE, CONF_DEFAULT_LIGHT_TRANSITION, CONF_ENABLE_ENHANCED_LIGHT_TRANSITION, CONF_ENABLE_LIGHT_TRANSITIONING_FLAG, CONF_GROUP_MEMBERS_ASSUME_STATE, SIGNAL_ADD_ENTITIES, SIGNAL_ATTR_UPDATED, SIGNAL_SET_LEVEL, ZHA_OPTIONS
from  import LogMixin, async_get_zha_config_value, get_zha_data
from  import ZHA_ENTITIES
from  import ZhaEntity, ZhaGroupEntity
if TYPE_CHECKING:
    from  import ZHADevice
_LOGGER = logging.getLogger(__name__)
DEFAULT_ON_OFF_TRANSITION = 1
...
FLASH_EFFECTS = {light.FLASH_SHORT: Identify.EffectIdentifier.Blink, light.FLASH_LONG: Identify.EffectIdentifier.Breathe}
...
SUPPORT_GROUP_LIGHT = light.LightEntityFeature.EFFECT | light.LightEntityFeature.FLASH | light.LightEntityFeature.TRANSITION

async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    """Set up the Zigbee Home Automation light from config entry."""
    zha_data = get_zha_data(hass)
    entities_to_create = zha_data.platforms[Platform.LIGHT]
    unsub = async_dispatcher_connect(hass, SIGNAL_ADD_ENTITIES, functools.partial(discovery.async_add_entities, async_add_entities, entities_to_create))
...
class BaseLight(LogMixin, light.LightEntity):
    ...
    def set_level(self, value: int) -> None:
        ...
        if self.is_transitioning:
            self.debug('received level %s while transitioning - skipping update', value)
            return
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        duration = (transition if transition is not None else self._zha_config_transition) or self._DEFAULT_MIN_TRANSITION_TIME
        ...
        execute_if_off_supported = self._GROUP_SUPPORTS_EXECUTE_IF_OFF if isinstance(self, LightGroup) else self._color_cluster_handler and self._color_cluster_handler.execute_if_off_supported
        set_transition_flag = (brightness_supported(self._attr_supported_color_modes) or temperature is not None or xy_color is not None or (hs_color is not None)) and self._zha_config_enable_light_transitioning_flag
        transition_time = (duration + DEFAULT_EXTRA_TRANSITION_DELAY_SHORT if (brightness is not None or transition is not None) and brightness_supported(self._attr_supported_color_modes) or (self._off_with_transition and self._off_brightness is not None) or temperature is not None or (xy_color is not None) or (hs_color is not None) else DEFAULT_ON_OFF_TRANSITION + DEFAULT_EXTRA_TRANSITION_DELAY_SHORT) if set_transition_flag else 0
        if set_transition_flag:
            self.async_transition_set_flag()
        new_color_provided_while_off = self._zha_config_enhanced_light_transition and (not self._FORCE_ON) and (not self._attr_state) and (temperature is not None and (self._attr_color_temp != temperature or self._attr_color_mode != ColorMode.COLOR_TEMP) or (xy_color is not None and (self._attr_xy_color != xy_color or self._attr_color_mode != ColorMode.XY)) or (hs_color is not None and (self._attr_hs_color != hs_color or self._attr_color_mode != ColorMode.HS))) and brightness_supported(self._attr_supported_color_modes) and (not execute_if_off_supported)
        if brightness is None and (self._off_with_transition or new_color_provided_while_off) and (self._off_brightness is not None):
        ...
        if new_color_provided_while_off:
            result = await self._level_cluster_handler.move_to_level_with_on_off(level=DEFAULT_MIN_BRIGHTNESS, transition_time=int(10 * self._DEFAULT_MIN_TRANSITION_TIME))
        ...
        if execute_if_off_supported:
            ...
            if not await self.async_handle_color_commands(temperature, duration, hs_color, xy_color, new_color_provided_while_off, t_log):
                if set_transition_flag and (not self._transition_listener):
                    self.async_transition_complete()
                self.debug('turned on: %s', t_log)
                return
        if (brightness is not None or transition is not None) and (not new_color_provided_while_off) and brightness_supported(self._attr_supported_color_modes):
            result = await self._level_cluster_handler.move_to_level_with_on_off(level=level, transition_time=int(10 * duration))
            t_log['move_to_level_with_on_off'] = result
            if result[1] is not Status.SUCCESS:
                if set_transition_flag and (not self._transition_listener):
                    self.async_transition_complete()
        ...
        if (brightness is None and transition is None) and (not new_color_provided_while_off) or (self._FORCE_ON and brightness != 0):
        ...
        if not execute_if_off_supported:
            ...
            if not await self.async_handle_color_commands(temperature, duration, hs_color, xy_color, new_color_provided_while_off, t_log):
                ...
                return
        if new_color_provided_while_off:
            result = await self._level_cluster_handler.move_to_level(level=level, transition_time=int(10 * duration))
        ...
        if effect == light.EFFECT_COLORLOOP:
            result = await self._color_cluster_handler.color_loop_set(update_flags=Color.ColorLoopUpdateFlags.Action | Color.ColorLoopUpdateFlags.Direction | Color.ColorLoopUpdateFlags.Time, action=Color.ColorLoopAction.Activate_from_current_hue, direction=Color.ColorLoopDirection.Increment, time=transition if transition else 7, start_hue=0)
            t_log['color_loop_set'] = result
            self._attr_effect = light.EFFECT_COLORLOOP
        elif self._attr_effect == light.EFFECT_COLORLOOP and effect != light.EFFECT_COLORLOOP:
            result = await self._color_cluster_handler.color_loop_set(update_flags=Color.ColorLoopUpdateFlags.Action, action=Color.ColorLoopAction.Deactivate, direction=Color.ColorLoopDirection.Decrement, time=0, start_hue=0)
            t_log['color_loop_set'] = result
            self._attr_effect = None
        if flash is not None:
            result = await self._identify_cluster_handler.trigger_effect(effect_id=FLASH_EFFECTS[flash], effect_variant=Identify.EffectVariant.Default)
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        transition_time = (transition or self._DEFAULT_MIN_TRANSITION_TIME if transition is not None else DEFAULT_ON_OFF_TRANSITION) + DEFAULT_EXTRA_TRANSITION_DELAY_SHORT
        if self._zha_config_enable_light_transitioning_flag:
            self.async_transition_set_flag()
        if transition is not None and supports_level:
            result = await self._level_cluster_handler.move_to_level_with_on_off(level=0, transition_time=int(10 * (transition or self._DEFAULT_MIN_TRANSITION_TIME)))
        else:
    ...
    async def async_handle_color_commands(self, temperature, duration, hs_color, xy_color, new_color_provided_while_off, t_log):
        """Process ZCL color commands."""
        transition_time = self._DEFAULT_MIN_TRANSITION_TIME if new_color_provided_while_off else duration
        if temperature is not None:
            result = await self._color_cluster_handler.move_to_color_temp(color_temp_mireds=temperature, transition_time=int(10 * transition_time))
        ...
        if hs_color is not None:
            if not isinstance(self, LightGroup) and self._color_cluster_handler.enhanced_hue_supported:
                result = await self._color_cluster_handler.enhanced_move_to_hue_and_saturation(enhanced_hue=int(hs_color[0] * 65535 / 360), saturation=int(hs_color[1] * 2.54), transition_time=int(10 * transition_time))
                t_log['enhanced_move_to_hue_and_saturation'] = result
            else:
                result = await self._color_cluster_handler.move_to_hue_and_saturation(hue=int(hs_color[0] * 254 / 360), saturation=int(hs_color[1] * 2.54), transition_time=int(10 * transition_time))
        ...
        if xy_color is not None:
            result = await self._color_cluster_handler.move_to_color(color_x=int(xy_color[0] * 65535), color_y=int(xy_color[1] * 65535), transition_time=int(10 * transition_time))
    ...
    def async_transition_set_flag(self) -> None:
        ...
        if isinstance(self, LightGroup):
            async_dispatcher_send(self.hass, SIGNAL_LIGHT_GROUP_TRANSITION_START, {'entity_ids': self._entity_ids})
        self._async_unsub_transition_listener()
    ...
    def async_transition_start_timer(self, transition_time) -> None:
        ...
        self._transition_listener = async_call_later(self._zha_device.hass, transition_time, self.async_transition_complete)
    ...
    def async_transition_complete(self, _=None) -> None:
        ...
        self.async_write_ha_state()
        if isinstance(self, LightGroup):
            async_dispatcher_send(self.hass, SIGNAL_LIGHT_GROUP_TRANSITION_FINISHED, {'entity_ids': self._entity_ids})
            if self._debounced_member_refresh is not None:
                self.debug('transition complete - refreshing group member states')
                assert self.platform.config_entry
                self.platform.config_entry.async_create_background_task(self.hass, self._debounced_member_refresh.async_call(), 'zha.light-refresh-debounced-member')

@STRICT_MATCH(cluster_handler_names=CLUSTER_HANDLER_ON_OFF, aux_cluster_handlers={CLUSTER_HANDLER_COLOR, CLUSTER_HANDLER_LEVEL})
class Light(BaseLight, ZhaEntity):
    ...
    def __init__(self, unique_id, zha_device: ZHADevice, cluster_handlers, **kwargs) -> None:
        ...
        effect_list = []
        self._zha_config_always_prefer_xy_color_mode = async_get_zha_config_value(zha_device.gateway.config_entry, ZHA_OPTIONS, CONF_ALWAYS_PREFER_XY_COLOR_MODE, True)
        ...
        if self._color_cluster_handler:
            ...
            if self._color_cluster_handler.xy_supported and (self._zha_config_always_prefer_xy_color_mode or not self._color_cluster_handler.hs_supported):
            ...
            if self._color_cluster_handler.hs_supported and (not self._zha_config_always_prefer_xy_color_mode):
                self._attr_supported_color_modes.add(ColorMode.HS)
                if self._color_cluster_handler.enhanced_hue_supported and self._color_cluster_handler.enhanced_current_hue is not None:
                    curr_hue = self._color_cluster_handler.enhanced_current_hue * 65535 / 360
                elif self._color_cluster_handler.current_hue is not None:
                    curr_hue = self._color_cluster_handler.current_hue * 254 / 360
                else:
                    curr_hue = 0
                if (curr_saturation := self._color_cluster_handler.current_saturation) is None:
                    curr_saturation = 0
                self._attr_hs_color = (int(curr_hue), int(curr_saturation * 2.54))
            if self._color_cluster_handler.color_loop_supported:
                ...
                if self._color_cluster_handler.color_loop_active == 1:
                    self._attr_effect = light.EFFECT_COLORLOOP
        self._attr_supported_color_modes = filter_supported_color_modes(self._attr_supported_color_modes)
        if len(self._attr_supported_color_modes) == 1:
            self._attr_color_mode = next(iter(self._attr_supported_color_modes))
        else:
            assert self._color_cluster_handler
            if self._color_cluster_handler.color_mode == Color.ColorMode.Color_temperature:
                self._attr_color_mode = ColorMode.COLOR_TEMP
            else:
                self._attr_color_mode = ColorMode.XY
        ...
        self._zha_config_transition = async_get_zha_config_value(zha_device.gateway.config_entry, ZHA_OPTIONS, CONF_DEFAULT_LIGHT_TRANSITION, 0)
        self._zha_config_enhanced_light_transition = async_get_zha_config_value(zha_device.gateway.config_entry, ZHA_OPTIONS, CONF_ENABLE_ENHANCED_LIGHT_TRANSITION, False)
        self._zha_config_enable_light_transitioning_flag = async_get_zha_config_value(zha_device.gateway.config_entry, ZHA_OPTIONS, CONF_ENABLE_LIGHT_TRANSITIONING_FLAG, True)

    @callback
    def async_set_state(self, attr_id, attr_name, value):
        """Set the state."""
        if self.is_transitioning:
            self.debug('received onoff %s while transitioning - skipping update', value)
            return
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self.async_accept_signal(self._on_off_cluster_handler, SIGNAL_ATTR_UPDATED, self.async_set_state)
        if self._level_cluster_handler:
            self.async_accept_signal(self._level_cluster_handler, SIGNAL_SET_LEVEL, self.set_level)
        refresh_interval = random.randint(*(x * 60 for x in self._REFRESH_INTERVAL))
        self._cancel_refresh_handle = async_track_time_interval(self.hass, self._refresh, timedelta(seconds=refresh_interval))
        self.async_accept_signal(None, SIGNAL_LIGHT_GROUP_STATE_CHANGED, self._maybe_force_refresh, signal_override=True)
        ...
        self.async_accept_signal(None, SIGNAL_LIGHT_GROUP_TRANSITION_START, transition_on, signal_override=True)
        ...
        def transition_off(signal):
            ...
            if self.entity_id in signal['entity_ids']:
                ...
                self._transitioning_group = False
        self.async_accept_signal(None, SIGNAL_LIGHT_GROUP_TRANSITION_FINISHED, transition_off, signal_override=True)
        self.async_accept_signal(None, SIGNAL_LIGHT_GROUP_ASSUME_GROUP_STATE, self._assume_group_state, signal_override=True)
    ...
    def async_restore_last_state(self, last_state):
        ...
        if (color_mode := last_state.attributes.get('color_mode')) is not None:
            self._attr_color_mode = 6lowpan.sixlowpan.IP6FieldLenField.addfield(color_mode)
    ...
    async def async_get_state(self) -> None:
        ...
        if self._on_off_cluster_handler:
            state = await self._on_off_cluster_handler.get_attribute_value('on_off', from_cache=False)
            if self.is_transitioning:
                return
            if state is not None:
                ...
                if state:
                    ...
                    self._off_brightness = None
        if self._level_cluster_handler:
            level = await self._level_cluster_handler.get_attribute_value('current_level', from_cache=False)
            if self.is_transitioning:
                return
            if level is not None:
                self._attr_brightness = level
        if self._color_cluster_handler:
            attributes = ['color_mode', 'current_x', 'current_y']
            if not self._zha_config_always_prefer_xy_color_mode and self._color_cluster_handler.enhanced_hue_supported:
                attributes.append('enhanced_current_hue')
                attributes.append('current_saturation')
            if self._color_cluster_handler.hs_supported and (not self._color_cluster_handler.enhanced_hue_supported) and (not self._zha_config_always_prefer_xy_color_mode):
            ...
            results = await self._color_cluster_handler.get_attributes(attributes, from_cache=False, only_cache=False)
            if self.is_transitioning:
                return
            if (color_mode := results.get('color_mode')) is not None:
                if color_mode == Color.ColorMode.Color_temperature:
                    ...
                    if color_temp is not None and color_mode:
                        ...
                        self._attr_xy_color = None
                        self._attr_hs_color = None
                elif color_mode == Color.ColorMode.Hue_and_saturation and (not self._zha_config_always_prefer_xy_color_mode):
                    self._attr_color_mode = ColorMode.HS
                    ...
                    if current_hue is not None and current_saturation is not None:
                        self._attr_hs_color = (int(current_hue * 360 / 65535) if self._color_cluster_handler.enhanced_hue_supported else int(current_hue * 360 / 254), int(current_saturation / 2.54))
                        self._attr_xy_color = None
                        self._attr_color_temp = None
                else:
                    self._attr_color_mode = ColorMode.XY
    ...
    def _assume_group_state(self, signal, update_params) -> None:
        ...
        if self.entity_id in signal['entity_ids'] and self._attr_available:
            ...
            if state and (not self._attr_state):
                self._off_with_transition = False
                self._off_brightness = None
            elif not state and self._attr_state and brightness_supported(supported_modes):
                self._off_brightness = self._attr_brightness
...
@STRICT_MATCH(cluster_handler_names=CLUSTER_HANDLER_ON_OFF, aux_cluster_handlers={CLUSTER_HANDLER_COLOR, CLUSTER_HANDLER_LEVEL}, manufacturers={'Philips', 'Signify Netherlands B.V.'})
class HueLight(Light):
    """Representation of a HUE light which does not report attributes."""
    _REFRESH_INTERVAL = (3, 5)

@STRICT_MATCH(cluster_handler_names=CLUSTER_HANDLER_ON_OFF, aux_cluster_handlers={CLUSTER_HANDLER_COLOR, CLUSTER_HANDLER_LEVEL}, manufacturers={'Jasco', 'Quotra-Vision', 'eWeLight', 'eWeLink'})
class ForceOnLight(Light):
    """Representation of a light which does not respect on/off for move_to_level_with_on_off commands."""
    _FORCE_ON = True

@STRICT_MATCH(cluster_handler_names=CLUSTER_HANDLER_ON_OFF, aux_cluster_handlers={CLUSTER_HANDLER_COLOR, CLUSTER_HANDLER_LEVEL}, manufacturers=DEFAULT_MIN_TRANSITION_MANUFACTURERS)
...
class LightGroup(BaseLight, ZhaGroupEntity):
    ...
    def __init__(self, entity_ids: list[str], unique_id: str, group_id: int, zha_device: ZHADevice, **kwargs: Any) -> None:
        ...
        for member in group.members:
            if member.device.manufacturer in DEFAULT_MIN_TRANSITION_MANUFACTURERS:
                self._DEFAULT_MIN_TRANSITION_TIME = MinTransitionLight._DEFAULT_MIN_TRANSITION_TIME
            for endpoint in member.device._endpoints.values():
                for cluster_handler in endpoint.all_cluster_handlers.values():
                    if cluster_handler.name == CLUSTER_HANDLER_COLOR and (not cluster_handler.execute_if_off_supported):
        ...
        self._zha_config_enable_light_transitioning_flag = async_get_zha_config_value(zha_device.gateway.config_entry, ZHA_OPTIONS, CONF_ENABLE_LIGHT_TRANSITIONING_FLAG, True)
        self._zha_config_always_prefer_xy_color_mode = async_get_zha_config_value(zha_device.gateway.config_entry, ZHA_OPTIONS, CONF_ALWAYS_PREFER_XY_COLOR_MODE, True)
        self._zha_config_group_members_assume_state = async_get_zha_config_value(zha_device.gateway.config_entry, ZHA_OPTIONS, CONF_GROUP_MEMBERS_ASSUME_STATE, True)
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if self._debounced_member_refresh is None:
            force_refresh_debouncer = Debouncer(self.hass, _LOGGER, cooldown=3, immediate=True, function=self._force_member_updates)
    ...
    async def async_update(self) -> None:
        ...
        self._attr_brightness = helpers.reduce_attribute(on_states, light.ATTR_BRIGHTNESS)
        self._attr_xy_color = helpers.reduce_attribute(on_states, light.ATTR_XY_COLOR, reduce=helpers.mean_tuple)
        if not self._zha_config_always_prefer_xy_color_mode:
            self._attr_hs_color = helpers.reduce_attribute(on_states, light.ATTR_HS_COLOR, reduce=helpers.mean_tuple)
        self._attr_color_temp = helpers.reduce_attribute(on_states, light.ATTR_COLOR_TEMP)
        self._attr_min_mireds = helpers.reduce_attribute(states, light.ATTR_MIN_MIREDS, default=153, reduce=min)
        self._attr_max_mireds = helpers.reduce_attribute(states, light.ATTR_MAX_MIREDS, default=500, reduce=max)
        self._attr_effect_list = None
        all_effect_lists = list(helpers.find_state_attributes(states, light.ATTR_EFFECT_LIST))
        if all_effect_lists:
        ...
        self._attr_color_mode = None
        all_color_modes = list(helpers.find_state_attributes(on_states, light.ATTR_COLOR_MODE))
        if all_color_modes:
            ...
            if self._attr_color_mode == ColorMode.HS and (color_mode_count[ColorMode.HS] != len(self._group.members) or self._zha_config_always_prefer_xy_color_mode):
                self._attr_color_mode = ColorMode.XY
        self._attr_supported_color_modes = None
        all_supported_color_modes = list(helpers.find_state_attributes(states, light.ATTR_SUPPORTED_COLOR_MODES))
    ...
    async def _force_member_updates(self) -> None:
        ...
        async_dispatcher_send(self.hass, SIGNAL_LIGHT_GROUP_STATE_CHANGED, {'entity_ids': self._entity_ids})

    def _send_member_assume_state_event(self, state, service_kwargs, off_brightness=None) -> None:
        """Send an assume event to all members of the group."""
        update_params = {'state': state, 'off_with_transition': self._off_with_transition}
        ...
        async_dispatcher_send(self.hass, SIGNAL_LIGHT_GROUP_ASSUME_GROUP_STATE, {'entity_ids': self._entity_ids}, update_params)